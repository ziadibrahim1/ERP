...
public class frmUnFinishedSteps : frmReportTree2010
{
	private IContainer components = null;

	public frmUnFinishedSteps()
	{
		dtItems = BusinessLayer.Clinics.Clinics.FillCombo("-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		TreeItemsParentIDCol = "ParentID";
		TreeItemsIDCol = "ClinicID";
		TreeItemsNameCol = "ClinicName";
		TreeItemsIsMainCol = "IsMain";
		dtItems2 = Patients.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		TreeItems2ParentIDCol = "ParentID";
		TreeItems2IDCol = "PatientID";
		TreeItems2NameCol = "PatientName";
	...
	public override void ShowReport()
	{
		...
		if (Items.Equals(","))
		{
			GlobalVariables.InformationMB.Show("لم تقم باختيار اى عيادة  ", "There is no chosen Clinic to be shown in the report, please check Clinics to be shown in report");
			return;
		}
		if (Items2.Equals(","))
		{
			GlobalVariables.InformationMB.Show("لم تقم باختيار اى مريض  ", "There is no chosen Patient to be shown in the report, please check Patients to be shown in report");
			return;
		}
		frmReporViwer frmReporViwer2 = new frmReporViwer();
		GlobalVariables.ReportDocument.SetParameterValue("@ClinicIDs", Items);
		GlobalVariables.ReportDocument.SetParameterValue("@PatientIDs", Items2);
	...
	{
		dtSearchResult = SearchFunctions.ClinicsSearchReport(IsFromServer: false);
	...
	{
		dtSearchResult = SearchFunctions.PatientsSearchReport(IsFromServer: false);
		for (int i = 0; i < dtSearchResult.Rows.Count; i++)
		{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Clinics.Reports.frmUnFinishedSteps));
