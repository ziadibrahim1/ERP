...
public class frmUnApprovingSendLabOrders : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingSendLabOrders()
	{
		InitializeComponent();
		NoCol = "LabOrderNo";
		((Control)(object)btnPost).Text = (GlobalVariables.IsArabic ? "الغاء الارسال" : "Cancel Sending");
	}

	public override void FillGrid()
	{
		dtsource = LabOrders.SelectByLabOrdersSent(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["LabName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ClientName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SentDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsSent"].Header).Caption = (GlobalVariables.IsArabic ? "الغاء الارسال" : "Cancel Sending");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsSent"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IsSent")
		{
	...
	public override void btnPost_Click(object sender, EventArgs e)
	{
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			try
			{
				LabOrders.SetSentToLab(((UltraGridBase)ULGData).Rows[i].Cells["LabOrderID"].Value.ToString(), bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsSent"].Value.ToString()) ? "0" : "1", DateTime.Parse(((UltraGridBase)ULGData).Rows[i].Cells["SentDate"].Value.ToString()).ToString(GlobalVariables.DateLongFormate));
				...
				if (((UltraGridBase)ULGData).Rows[i].Cells["SalesJVID"].Value != DBNull.Value)
				{
					JV.DeleteVirtual(((UltraGridBase)ULGData).Rows[i].Cells["SalesJVID"].Value.ToString(), GlobalVariables.UserID, IsFromServer: false);
					JVDetails.DeleteVirtualByJVID(((UltraGridBase)ULGData).Rows[i].Cells["SalesJVID"].Value.ToString(), GlobalVariables.UserID, IsFromServer: false);
				}
				if (((UltraGridBase)ULGData).Rows[i].Cells["BranchPurchasesJVID"].Value != DBNull.Value)
				{
					JV.DeleteVirtual(((UltraGridBase)ULGData).Rows[i].Cells["BranchPurchasesJVID"].Value.ToString(), GlobalVariables.UserID, IsFromServer: false);
					JVDetails.DeleteVirtualByJVID(((UltraGridBase)ULGData).Rows[i].Cells["BranchPurchasesJVID"].Value.ToString(), GlobalVariables.UserID, IsFromServer: false);
				}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Lenses.Approved.frmUnApprovingSendLabOrders));
		...
		base.Name = "frmUnApprovingSendLabOrders";
