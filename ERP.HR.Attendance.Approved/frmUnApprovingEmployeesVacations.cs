...
public class frmUnApprovingEmployeesVacations : frmPosted
{
	private DataTable dtUser = new DataTable();

	private IContainer components = null;

	public frmUnApprovingEmployeesVacations()
	{
		InitializeComponent();
	...
	public override void FillGrid()
	{
		if (ViewAllEmployees)
		{
			dtsource = EmployeesVacations.SelectByApproved("-1", "-1", "1", "0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		}
		else if (!ViewAllEmployees && dtUser.Rows[0]["SubAccountID"] != DBNull.Value)
		{
			dtsource = EmployeesVacations.SelectByApproved("-1", dtUser.Rows[0]["SubAccountID"].ToString(), "1", "0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		}
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["Approved"].Value.Equals(true))
			{
				...
				EmployeesVacations.SetApprove("0", string.Concat(",", ((UltraGridBase)ULGData).Rows[i].Cells["EmployeeVacationID"].Value, ","), GlobalVariables.UserID, IsFromServer: true);
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات اجازة لإلغاء إعتمادها " : "There are No Vacation Request to UnApprove");
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.HR.Attendance.Approved.frmUnApprovingEmployeesVacations));
		...
		base.Name = "frmUnApprovingEmployeesVacations";
