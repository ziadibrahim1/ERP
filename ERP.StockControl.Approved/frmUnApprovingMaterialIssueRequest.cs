...
public class frmUnApprovingMaterialIssueRequest : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingMaterialIssueRequest()
	{
		InitializeComponent();
		NoCol = "MaterialIssueRequestNo";
		((Control)(object)btnPost).Text = (GlobalVariables.IsArabic ? "فك إعتماد" : "Unapprove");
	}

	public override void FillGrid()
	{
		dtsource = MaterialIssueRequest.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " EXEC SC_MaterialIssueRequest_SetApprove  0," + (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsRefused"].Value.ToString()) ? "1" : "0") + "," + ((UltraGridBase)ULGData).Rows[i].Cells["MaterialIssueRequestID"].Value.ToString() + " ; ";
			}
		}
		if (text != "")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات صرف لفك إعتمادها " : "There are No Material Request to UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmMaterialIssueRequest frmMaterialIssueRequest2 = new frmMaterialIssueRequest(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["MaterialIssueRequestID"].Value.ToString()));
			frmMaterialIssueRequest2.Size = new Size(base.Width, base.Height);
			frmMaterialIssueRequest2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmMaterialIssueRequest2.lblTitle).Text = (GlobalVariables.IsArabic ? "طلب صرف لقسم" : "Material Issue Request");
			frmMaterialIssueRequest2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.MaterialIssueRequestReport(1, 0, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["MaterialIssueRequestID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["MaterialIssueRequestID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.Approved.frmUnApprovingMaterialIssueRequest));
		...
		base.Name = "frmUnApprovingMaterialIssueRequest";
