...
public class frmUnApprovingSlicingAndRecipes : frmPosted
{
	...
	public frmUnApprovingSlicingAndRecipes()
	{
		InitializeComponent();
		NoCol = "VoucherNo";
	...
	public override void FillGrid()
	{
		...
		{
			dtsource = BusinessLayer.StockControl.Slicing.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
		}
		else if (rbrecipes.Checked)
		{
			dtsource = RecipeManufacturing.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
		}
	...
	public override void SaveData()
	{
		...
		if (text != ",")
		{
			if (rbSlicing.Checked)
			{
				BusinessLayer.StockControl.Slicing.SetApprove("0", text);
			}
			else if (rbrecipes.Checked)
			{
				RecipeManufacturing.SetApprove("0", text);
			}
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد أذونات لفك إعتمادها " : "There are No Vouchers To UnApprove");
		}
	}
	...
	public override void Search()
	{
		if (rbSlicing.Checked)
		{
			DataTable dataTable = SearchFunctions.SlicingReport(1);
		...
		else
		{
			...
			DataTable dataTable2 = SearchFunctions.RecipeManufacturingReport(1);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.Approved.frmUnApprovingSlicingAndRecipes));
		...
		base.Name = "frmUnApprovingSlicingAndRecipes";
