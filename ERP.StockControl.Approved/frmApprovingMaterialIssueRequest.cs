...
public class frmApprovingMaterialIssueRequest : frmPosted
{
	private IContainer components = null;

	public frmApprovingMaterialIssueRequest()
	{
		InitializeComponent();
	...
	{
		dtsource = MaterialIssueRequest.SelectByApproved(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " EXEC SC_MaterialIssueRequest_SetApprove  1," + (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsRefused"].Value.ToString()) ? "1" : "0") + "," + ((UltraGridBase)ULGData).Rows[i].Cells["MaterialIssueRequestID"].Value.ToString() + " ; ";
			}
		}
		if (text != "")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات صرف لإعتمادها " : "There are No Material Request to Approve");
		}
	}
	...
	{
		DataTable dataTable = SearchFunctions.MaterialIssueRequestReport(0, 0, -1);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.Approved.frmApprovingMaterialIssueRequest));
		...
		base.Name = "frmApprovingMaterialIssueRequest";
