...
public class frmApprovingStoreTransferRequest : frmPosted
{
	...
	public frmApprovingStoreTransferRequest()
	{
	...
	{
		dtsource = StoreTransferRequest.SelectByApproved(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " EXEC SC_StoreTransferRequest_SetApprove  1," + (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsRefused"].Value.ToString()) ? "1" : "0") + "," + ((UltraGridBase)ULGData).Rows[i].Cells["StoreTransferRequestID"].Value.ToString() + " ; ";
			}
		}
		if (text != "")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات تحويل لإعتمادها " : "There are No Store Transfer Request to Approve");
		}
	}
	...
	{
		DataTable dataTable = SearchFunctions.StoreTransferRequestReport(GlobalVariables.StoreIDs, "-1", 0, 0, -1);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.Approved.frmApprovingStoreTransferRequest));
		...
		base.Name = "frmApprovingStoreTransferRequest";
