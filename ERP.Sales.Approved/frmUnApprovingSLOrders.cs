...
public class frmUnApprovingSLOrders : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingSLOrders()
	{
		InitializeComponent();
		NoCol = "SLOrderNo";
	...
	{
		dtsource = SLOrders.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SLOrderNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "SLOrder No");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["SLOrderID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			SLOrders.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد أوامر بيع لفك إعتمادها " : "There are No Orders to  UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmSLOrders frmSLOrders2 = new frmSLOrders(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["SLOrderID"].Value.ToString()));
			frmSLOrders2.Size = new Size(base.Width, base.Height);
			frmSLOrders2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmSLOrders2.lblTitle).Text = (GlobalVariables.IsArabic ? "اوامــر  البيع" : "Sales Orders");
			frmSLOrders2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.SLOrdersReport(1, 0, -1, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["SLOrderID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["SLOrderID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sales.Approved.frmUnApprovingSLOrders));
		...
		base.Name = "frmUnApprovingSLOrders";
