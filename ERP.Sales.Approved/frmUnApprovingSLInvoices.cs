...
public class frmUnApprovingSLInvoices : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingSLInvoices()
	{
		InitializeComponent();
		NoCol = "SLInvoiceNo";
	...
	{
		dtsource = SLInvoices.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["SLInvoiceID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			SLInvoices.SetApprove("0", text);
			FillGrid();
		}
		else
		{
	...
	public override void ClickCellButton()
	{
		...
		{
			frmSLInvoices frmSLInvoices3 = new frmSLInvoices(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["SLInvoiceID"].Value.ToString()));
			frmSLInvoices3.Size = new Size(base.Width, base.Height);
			frmSLInvoices3.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmSLInvoices3.lblTitle).Text = (GlobalVariables.IsArabic ? "فاتورة المبيعات" : "Sales Invoice");
	...
	{
		DataTable dataTable = SearchFunctions.SLInvoicesReport(1, 0, -1, -1, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["SLInvoiceID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["SLInvoiceID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sales.Approved.frmUnApprovingSLInvoices));
		...
		base.Name = "frmUnApprovingSLInvoices";
