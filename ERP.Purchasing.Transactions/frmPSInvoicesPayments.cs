...
public class frmPSInvoicesPayments : frmHeaderDetails
{
	...
	private DataTable dtBankOut;

	private DataTable dtSafeOut;
	...
	public frmPSInvoicesPayments()
	{
		InitializeComponent();
		TableName = "PS_PSInvoicesPayments";
		IDCol = "PSInvoicePaymentID";
		NoCol = "PSInvoicePaymentNo";
		DateCol = "PSInvoicePaymentDate";
		((Control)(object)btnJV3).Text = (((Control)(object)btnJV2).Text = (((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? "قيد " : "JV")));
	}

	public frmPSInvoicesPayments(int ID)
		: this()
	{
		RowID = ID.ToString();
	}

	public frmPSInvoicesPayments(string subaccountid)
		: this()
	{
	...
	public override void PrepareData()
	{
		...
		dtDetails = PSInvoicesPaymentsDetails.SelectByPSInvoicePaymentID("0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void FillData()
	{
		if (SubAccountID != "0")
		{
			drMaster = null;
			btnAddClick();
			((TextEditorControlBase)cboSuppliers).Value = SubAccountID;
			DisplayData();
			bool adding = Adding;
		...
		{
			DataTable dataTable = PSInvoicesPayments.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((TextEditorControlBase)cboSuppliers).ValueChanged -= cboSuppliers_ValueChanged;
			dtSafeOut = null;
			dtBankOut = null;
			CanModifyOtherBranch = drMaster["BranchID"].ToString() == GlobalVariables.CurrentBranchID;
			((Control)(object)txtCode).Text = drMaster["PSInvoicePaymentNo"].ToString();
			dtpDate.Value = (DateTime)drMaster["PSInvoicePaymentDate"];
			((TextEditorControlBase)cboSuppliers).Value = drMaster["SubAccountID"];
			rbPayable.Checked = bool.Parse(drMaster["IsPayable"].ToString());
			...
			if (rbIsSafe.Checked && drMaster["SafeOutID"] != DBNull.Value)
			{
				dtSafeOut = SafeOut.Select(drMaster["SafeOutID"].ToString(), "-1", GlobalVariables.IsArabic ? "1" : "0");
			}
			else if (!rbIsSafe.Checked && drMaster["BankOutID"] != DBNull.Value)
			{
				dtBankOut = BankOut.Select(drMaster["BankOutID"].ToString(), "-1", GlobalVariables.IsArabic ? "1" : "0");
			}
			if (dtSafeOut != null && dtSafeOut.Rows.Count > 0)
			{
				((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + dtSafeOut.Rows[0]["JVNo"].ToString() + ")قيد") : (" No( " + dtSafeOut.Rows[0]["JVNo"].ToString() + " )"));
				((Control)(object)btnJV).Visible = ((dtSafeOut.Rows[0]["JVNo"] != DBNull.Value) ? true : false) && CanViewJV;
			}
			else if (dtBankOut != null && dtBankOut.Rows.Count > 0)
			{
				((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + dtBankOut.Rows[0]["JVNo"].ToString() + ")قيد ") : ("JV( " + dtBankOut.Rows[0]["JVNo"].ToString() + " )"));
				((Control)(object)btnJV2).Text = (GlobalVariables.IsArabic ? ("(" + dtBankOut.Rows[0]["JVNo2"].ToString() + ")قيد ") : ("JV( " + dtBankOut.Rows[0]["JVNo2"].ToString() + " )"));
				((Control)(object)btnJV3).Text = (GlobalVariables.IsArabic ? ("(" + dtBankOut.Rows[0]["JVNo3"].ToString() + ")قيد ") : ("JV( " + dtBankOut.Rows[0]["JVNo3"].ToString() + " )"));
				((Control)(object)btnJV).Visible = ((dtBankOut.Rows[0]["JVNo"] != DBNull.Value) ? true : false) && CanViewJV;
				((Control)(object)btnJV2).Visible = ((dtBankOut.Rows[0]["JVNo2"] != DBNull.Value) ? true : false) && CanViewJV;
				((Control)(object)btnJV3).Visible = ((dtBankOut.Rows[0]["JVNo3"] != DBNull.Value) ? true : false) && CanViewJV;
				((Control)(object)lblCheckStatus).Text = dtBankOut.Rows[0]["CheckStatus"].ToString();
				((Control)(object)lblCheckStatus).Visible = ((dtBankOut.Rows[0]["CheckStatus"] != DBNull.Value) ? true : false);
				((Control)(object)btnChangeStatus).Visible = ((dtBankOut.Rows[0]["CheckStatus"] != DBNull.Value) ? true : false) && !bool.Parse(dtBankOut.Rows[0]["Collected"].ToString()) && !bool.Parse(dtBankOut.Rows[0]["Returned"].ToString());
			}
			ClosedPeriod = FiscalYear.ChkForClosingFsicalPeriod(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID, IsFromServer: false);
			dtDetails = PSInvoicesPaymentsDetails.SelectByPSInvoicePaymentID(drMaster["PSInvoicePaymentID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
	...
	public override void SetControls(bool NavMode)
	{
		...
		((EditorButtonControlBase)cboSuppliers).ReadOnly = !Adding;
		rbIsCheck.Enabled = Adding;
		rbPayable.Enabled = Adding;
		...
		((EditorButtonControlBase)txtCreditBalance).ReadOnly = NavMode;
		((EditorButtonControlBase)txtDebitBalance).ReadOnly = true;
		...
		((Control)(object)btnSupplierSearch).Visible = Adding;
	...
	public override void ClearControls()
	{
		...
		((EditorButtonControlBase)cboSuppliers).ReadOnly = false;
		((Control)(object)btnSupplierSearch).Visible = true;
		cboCurrency.SelectedIndex = -1;
		((Control)(object)txtCode).Text = (Adding ? PSInvoicesPayments.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID) : "");
	...
	public override void InitGrid()
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Override.AllowAddNew = (AllowAddNew)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSInvoiceNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSInvoiceDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSInvoiceNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الفاتورة" : "Invoice No");
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSInvoiceDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الفاتورة" : "Invoice Date");
	...
	public override bool ValidateData()
	{
		...
		if (Updating && DateTime.Parse(drMaster["PSInvoicePaymentDate"].ToString()).Year != dtpDate.DateTime.Year)
		{
		...
		if (Main.CheckForValueByBranchIDAndFiscalYearID("PS_PSInvoicesPayments", "PSInvoicePaymentNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["PSInvoicePaymentNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = PSInvoicesPayments.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		...
		else
		{
			...
			{
				GlobalVariables.InformationMB.Show("برجاء إختيار المورد", "Please select Supplier");
				((TextEditorControlBase)cboSuppliers).Focus();
				return false;
			}
			...
			if (decimal.Parse(((Control)(object)txtTotal).Text) > 0m && decimal.Parse(((Control)(object)txtTotal).Text) - decimal.Parse(((Control)(object)txtDebitBalance).Text) - decimal.Parse(((Control)(object)txtCreditBalance).Text) > decimal.Parse(((Control)(object)txtPaidAmount).Text))
			{
	...
	public override void AddData()
	{
		...
		try
		{
			string text = ((((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count == 0) ? "" : (GlobalVariables.IsArabic ? "  سداد فاتورة مشتريات رقم  " : " Purchases Invoice Payment No "));
			int num = PSInvoicesPayments.Insert_Update("-1", ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSuppliers).Value.ToString(), rbIsCheck.Checked ? "1" : "0", rbPayable.Checked ? "1" : "0", rbIsSafe.Checked ? "1" : "0", (cboSafe.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, rbIsCheck.Checked ? ((Control)(object)txtCheckNo).Text : "Null", rbIsCheck.Checked ? dtpCheckDate.DateTime.ToString(GlobalVariables.DateLongFormate) : "Null", (!rbIsCheck.Checked && !rbPayable.Checked) ? "Null" : ((cboBank.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboBank).Value.ToString()), ((Control)(object)txtChargedPerson).Text, ((Control)(object)txtDebitBalance).Text, ((Control)(object)txtCreditBalance).Text, ((Control)(object)txtPaidAmount).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, ((Control)(object)txtPayingBank).Text, (cboBinderSafe.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboBinderSafe).Value.ToString(), "Null", "Null", "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				PSInvoicesPaymentsDetails.Insert_Update("-1", num.ToString(), ((UltraGridBase)ULGData).Rows[i].Cells["PSInvoiceID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[i].Cells["PaidAmount"].Value.ToString(), ((UltraGridBase)ULGData).Rows[i].Cells["Notes"].Value.ToString(), "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				text = ((i != ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count - 1) ? (text + ((UltraGridBase)ULGData).Rows[i].Cells["PSInvoiceNo"].Value.ToString() + ",") : (text + ((UltraGridBase)ULGData).Rows[i].Cells["PSInvoiceNo"].Value.ToString()));
			}
			if (((Control)(object)txtPaidAmount).Text != "" && decimal.Parse(((Control)(object)txtPaidAmount).Text) > 0m)
			{
				if (rbIsSafe.Checked)
				{
					string codeByBranchID = SafeOut.GetCodeByBranchID((cboSafe.SelectedIndex == -1) ? "" : ((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
					int num2 = SafeOut.Insert_Update("-1", codeByBranchID, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, "0", "0", ((Control)(object)txtChargedPerson).Text, (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, ((Control)(object)txtPaidAmount).Text, "Null", "0", ((Control)(object)txtPaidAmount).Text, "Null", "0", GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					SafeOutDetails.Insert_Update("-1", num2.ToString(), ((Control)(object)txtPaidAmount).Text, dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"].ToString(), dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"].ToString(), "Null", "Null", "0", text, GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					dtJvDetails = JVDetails.Select("0", "-1", GlobalVariables.IsArabic ? "1" : "0");
					DataRow dataRow = dtJvDetails.NewRow();
					dataRow["JVDetailID"] = "-1";
					dataRow["AccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"];
					dataRow["SubAccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"];
					...
					dataRow["BranchID"] = GlobalVariables.CurrentBranchID;
					dtJvDetails.Rows.Add(dataRow);
					string code = JV.GetCode(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID, "2");
					int num3 = JV.GenerateJV_Insert(code, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "2", num2.ToString(), codeByBranchID, " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
					((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + code + ")قيد رقم ") : ("JV No ( " + code + " )"));
					Main.ExecuteNonQuery(" Update SB_SafeOut  set JvID= " + num3 + " Where SafeOutID=" + num2.ToString());
					Main.ExecuteNonQuery(" Update PS_PSInvoicesPayments  set SafeOutID= " + num2 + " Where PSInvoicePaymentID=" + num);
				}
				else
				{
					string codeByBranchID2 = BankOut.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
					int num4 = BankOut.Insert_Update("-1", codeByBranchID2, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, "0", rbIsCheck.Checked ? "1" : "0", ((Control)(object)txtCheckNo).Text, rbIsCheck.Checked ? dtpCheckDate.DateTime.ToString(GlobalVariables.DateLongFormate) : "Null", ((TextEditorControlBase)cboBank).Value.ToString(), ((Control)(object)txtChargedPerson).Text, (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, ((Control)(object)txtPaidAmount).Text, ((Control)(object)txtPayingBank).Text, (cboBinderSafe.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboBinderSafe).Value.ToString(), "Null", "0", ((Control)(object)txtPaidAmount).Text, (dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPAccID"].ToString() == dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPUnderCollectionAccID"].ToString()) ? "1" : "0", "Null", "0", "0", "Null", "Null", "Null", "Null", "Null", "0", GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					BankOutDetails.Insert_Update("-1", num4.ToString(), ((Control)(object)txtPaidAmount).Text, dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"].ToString(), dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"].ToString(), "Null", "Null", "0", text, GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					dtJvDetails = JVDetails.Select("0", "-1", GlobalVariables.IsArabic ? "1" : "0");
					DataRow dataRow2 = dtJvDetails.NewRow();
					dataRow2["JVDetailID"] = "-1";
					dataRow2["AccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"];
					dataRow2["SubAccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"];
					...
					dataRow2["JVDetailID"] = "-1";
					dataRow2["AccountID"] = (rbIsCheck.Checked ? dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPAccID"].ToString() : dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["AccountID"].ToString());
					...
					dataRow2["BranchID"] = GlobalVariables.CurrentBranchID;
					dtJvDetails.Rows.Add(dataRow2);
					string code2 = JV.GetCode(dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), GlobalVariables.CurrentBranchID, "4");
					int num5 = JV.GenerateJV_Insert(code2, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "4", num4.ToString(), codeByBranchID2, " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
					((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + code2 + ")قيد رقم ") : ("JV No ( " + code2 + " )"));
					Main.ExecuteNonQuery(" Update SB_BankOut  set JvID= " + num5 + " Where BankOutID=" + num4.ToString());
					Main.ExecuteNonQuery(" Update PS_PSInvoicesPayments  set BankOutID= " + num4 + " Where PSInvoicePaymentID=" + num);
				}
			}
	...
	public override void UpdateData()
	{
		...
		{
			string text = ((((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count == 0) ? "" : (GlobalVariables.IsArabic ? "  سداد فاتورة مشتريات رقم  " : " Purchases Invoice Payment No "));
			int num = PSInvoicesPayments.Insert_Update(drMaster["PSInvoicePaymentID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSuppliers).Value.ToString(), rbIsCheck.Checked ? "1" : "0", rbPayable.Checked ? "1" : "0", rbIsSafe.Checked ? "1" : "0", (cboSafe.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, rbIsCheck.Checked ? ((Control)(object)txtCheckNo).Text : "Null", rbIsCheck.Checked ? dtpCheckDate.DateTime.ToString(GlobalVariables.DateLongFormate) : "Null", (!rbIsCheck.Checked && !rbPayable.Checked) ? "Null" : ((cboBank.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboBank).Value.ToString()), ((Control)(object)txtChargedPerson).Text, ((Control)(object)txtDebitBalance).Text, ((Control)(object)txtCreditBalance).Text, ((Control)(object)txtPaidAmount).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, ((Control)(object)txtPayingBank).Text, (cboBinderSafe.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboBinderSafe).Value.ToString(), (drMaster["BankOutID"] == DBNull.Value) ? "Null" : drMaster["BankOutID"].ToString(), (drMaster["SafeOutID"] == DBNull.Value) ? "Null" : drMaster["SafeOutID"].ToString(), "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			string text2 = ",";
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				text2 = text2 + ((UltraGridBase)ULGData).Rows[i].Cells["PSInvoicePaymentDetailID"].Value.ToString() + ",";
			}
			Main.DeleteForUpdate("PS_PSInvoicesPaymentsDetails", "PSInvoicePaymentID", drMaster["PSInvoicePaymentID"].ToString(), "PSInvoicePaymentDetailID", text2);
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				PSInvoicesPaymentsDetails.Insert_Update(((UltraGridBase)ULGData).Rows[j].Cells["PSInvoicePaymentDetailID"].Value.ToString(), num.ToString(), ((UltraGridBase)ULGData).Rows[j].Cells["PSInvoiceID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[j].Cells["PaidAmount"].Value.ToString(), ((UltraGridBase)ULGData).Rows[j].Cells["Notes"].Value.ToString(), "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				text = ((j != ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count - 1) ? (text + ((UltraGridBase)ULGData).Rows[j].Cells["PSInvoiceNo"].Value.ToString() + ",") : (text + ((UltraGridBase)ULGData).Rows[j].Cells["PSInvoiceNo"].Value.ToString()));
			}
			...
			else if (rbIsSafe.Checked && drMaster != null && bool.Parse(drMaster["IsSafe"].ToString()))
			{
				if (((Control)(object)txtPaidAmount).Text != "" && decimal.Parse(((Control)(object)txtPaidAmount).Text) == 0m)
				{
					if (drMaster["SafeOutID"] != DBNull.Value)
					{
						SafeOutDetails.DeleteVirtualBySafeOutID(drMaster["SafeOutID"].ToString(), GlobalVariables.UserID);
						SafeOut.DeleteVirtual(drMaster["SafeOutID"].ToString(), GlobalVariables.UserID);
						if (dtSafeOut != null && dtSafeOut.Rows.Count > 0)
						{
							JVDetails.DeleteByJVID(dtSafeOut.Rows[0]["JVID"].ToString(), GlobalVariables.UserID);
							JV.DeleteVirtual(dtSafeOut.Rows[0]["JVID"].ToString(), GlobalVariables.UserID, IsFromServer: false);
						}
						Main.ExecuteNonQuery(" Update PS_PSInvoicesPayments  set SafeOutID=Null  Where PSInvoicePaymentID=" + num);
					}
				}
				else
				{
					int num6 = SafeOut.Insert_Update(dtSafeOut.Rows[0]["SafeOutID"].ToString(), dtSafeOut.Rows[0]["SafeOutNo"].ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, "0", "0", ((Control)(object)txtChargedPerson).Text, (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, ((Control)(object)txtPaidAmount).Text, "Null", "0", ((Control)(object)txtPaidAmount).Text, dtSafeOut.Rows[0]["JVID"].ToString(), bool.Parse(dtSafeOut.Rows[0]["Approved"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					SafeOutDetails.DeleteBySafeOutID(num6.ToString(), GlobalVariables.UserID);
					SafeOutDetails.Insert_Update("-1", num6.ToString(), ((Control)(object)txtPaidAmount).Text, dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"].ToString(), dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"].ToString(), "Null", "Null", "0", text, GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					JVDetails.DeleteByJVID(dtSafeOut.Rows[0]["JVID"].ToString(), GlobalVariables.UserID);
					dtJvDetails = JVDetails.Select("0", "-1", GlobalVariables.IsArabic ? "1" : "0");
					DataRow dataRow3 = dtJvDetails.NewRow();
					dataRow3["JVDetailID"] = "-1";
					dataRow3["AccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"];
					dataRow3["SubAccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"];
					...
					dataRow3["BranchID"] = GlobalVariables.CurrentBranchID;
					dtJvDetails.Rows.Add(dataRow3);
					string code3 = JV.GetCode(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID, "2");
					int num7 = JV.GenerateJV_Insert(code3, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "2", num6.ToString(), dtSafeOut.Rows[0]["SafeOutNo"].ToString(), " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
					((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + code3 + ")قيد رقم ") : ("JV No ( " + code3 + " )"));
					Main.ExecuteNonQuery(" Update SB_SafeOut  set JvID= " + num7 + " Where SafeOutID=" + num6.ToString());
					Main.ExecuteNonQuery(" Update PS_PSInvoicesPayments  set SafeOutID= " + num6 + " Where PSInvoicePaymentID=" + num);
				}
			}
			else if (!rbIsSafe.Checked && drMaster != null && !bool.Parse(drMaster["IsSafe"].ToString()))
			{
				if (((Control)(object)txtPaidAmount).Text != "" && decimal.Parse(((Control)(object)txtPaidAmount).Text) == 0m)
				{
					if (drMaster["BankOutID"] != DBNull.Value)
					{
						...
						if (dtBankOut != null && dtBankOut.Rows.Count > 0)
						{
							if (dtBankOut.Rows[0]["JVID2"] != DBNull.Value)
							{
								JV.DeleteVirtual(dtBankOut.Rows[0]["JVID2"].ToString(), GlobalVariables.UserID, IsFromServer: false);
							}
							if (dtBankOut.Rows[0]["JVID3"] != DBNull.Value)
							{
								JV.DeleteVirtual(dtBankOut.Rows[0]["JVID3"].ToString(), GlobalVariables.UserID, IsFromServer: false);
							}
							JV.DeleteVirtual(dtBankOut.Rows[0]["JVID"].ToString(), GlobalVariables.UserID, IsFromServer: false);
							JVDetails.DeleteByJVID(dtBankOut.Rows[0]["JVID"].ToString(), GlobalVariables.UserID);
						}
						Main.ExecuteNonQuery(" Update PS_PSInvoicesPayments  set BankOutID=Null  Where PSInvoicePaymentID=" + num);
					}
				}
				else
				{
					int num8 = BankOut.Insert_Update(dtBankOut.Rows[0]["BankOutID"].ToString(), dtBankOut.Rows[0]["BankOutNo"].ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, "0", rbIsCheck.Checked ? "1" : "0", ((Control)(object)txtCheckNo).Text, rbIsCheck.Checked ? dtpCheckDate.DateTime.ToString(GlobalVariables.DateLongFormate) : "Null", ((TextEditorControlBase)cboBank).Value.ToString(), ((Control)(object)txtChargedPerson).Text, (GlobalVariables.IsArabic ? " إيصال رقم " : " Receipt No ") + ((Control)(object)txtCode).Text + "  " + ((Control)(object)txtNotes).Text, ((Control)(object)txtPaidAmount).Text, ((Control)(object)txtPayingBank).Text, (cboBinderSafe.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboBinderSafe).Value.ToString(), "Null", "0", ((Control)(object)txtPaidAmount).Text, (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked && dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPAccID"].ToString() != dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPUnderCollectionAccID"].ToString()) ? "0" : (dtBankOut.Rows[0]["UnderCollection"].Equals(true) ? "1" : "0"), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "Null" : ((dtBankOut.Rows[0]["UnderCollectionDate"] == DBNull.Value) ? "Null" : DateTime.Parse(dtBankOut.Rows[0]["UnderCollectionDate"].ToString()).ToString(GlobalVariables.DateLongFormate)), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "0" : (dtBankOut.Rows[0]["Collected"].Equals(true) ? "1" : "0"), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "0" : (dtBankOut.Rows[0]["Returned"].Equals(true) ? "1" : "0"), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "Null" : ((dtBankOut.Rows[0]["CollectionDate"] == DBNull.Value) ? "Null" : DateTime.Parse(dtBankOut.Rows[0]["CollectionDate"].ToString()).ToString(GlobalVariables.DateLongFormate)), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "Null" : ((dtBankOut.Rows[0]["CollectionExpense"] == DBNull.Value) ? "Null" : dtBankOut.Rows[0]["CollectionExpense"].ToString()), (dtBankOut.Rows[0]["JVID"] == DBNull.Value) ? "Null" : dtBankOut.Rows[0]["JVID"].ToString(), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "Null" : ((dtBankOut.Rows[0]["JVID2"] == DBNull.Value) ? "Null" : dtBankOut.Rows[0]["JVID2"].ToString()), (bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) != rbIsCheck.Checked) ? "Null" : ((dtBankOut.Rows[0]["JVID3"] == DBNull.Value) ? "Null" : dtBankOut.Rows[0]["JVID3"].ToString()), bool.Parse(dtBankOut.Rows[0]["Approved"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, bool.Parse(dtBankOut.Rows[0]["Deleted"].ToString()) ? "1" : "0", GlobalVariables.UserID);
					BankOutDetails.DeleteByBankOutID(dtBankOut.Rows[0]["BankOutID"].ToString(), GlobalVariables.UserID);
					BankOutDetails.Insert_Update("-1", num8.ToString(), ((Control)(object)txtPaidAmount).Text, dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"].ToString(), dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"].ToString(), "Null", "Null", "0", text, GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
					...
					else
					{
						...
						if (dtBankOut.Rows[0]["JVID2"] != DBNull.Value)
						{
							JVDetails.DeleteByJVID(dtBankOut.Rows[0]["JVID2"].ToString(), GlobalVariables.UserID);
						}
						if (dtBankOut.Rows[0]["JVID3"] != DBNull.Value)
						{
							JVDetails.DeleteByJVID(dtBankOut.Rows[0]["JVID3"].ToString(), GlobalVariables.UserID);
						}
					}
					dtJvDetails = JVDetails.Select("0", "-1", GlobalVariables.IsArabic ? "1" : "0");
					DataRow dataRow4 = dtJvDetails.NewRow();
					dataRow4["JVDetailID"] = "-1";
					dataRow4["AccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SupplierAccountID"];
					dataRow4["SubAccountID"] = dtSuppliers.Select(" SubAccountID = " + ((TextEditorControlBase)cboSuppliers).Value.ToString())[0]["SubAccountID"];
					...
					dataRow4["JVDetailID"] = "-1";
					dataRow4["AccountID"] = (rbIsCheck.Checked ? dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPAccID"].ToString() : dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["AccountID"].ToString());
					...
					dataRow4["BranchID"] = GlobalVariables.CurrentBranchID;
					dtJvDetails.Rows.Add(dataRow4);
					JV.GenerateJV_Update(JVNo: (dtBankOut.Rows[0]["JVDate"] == DBNull.Value || (DateTime.Parse(dtBankOut.Rows[0]["JVDate"].ToString()).Month == dtpDate.DateTime.Month && DateTime.Parse(dtBankOut.Rows[0]["JVDate"].ToString()).Year == dtpDate.DateTime.Year)) ? dtBankOut.Rows[0]["JVNo"].ToString() : JV.GetCode(dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), GlobalVariables.CurrentBranchID, "4"), JVID: dtBankOut.Rows[0]["JVID"].ToString(), JVDate: dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), TransTypeID: "4", VoucherID: num8.ToString(), ReceiptNo: dtBankOut.Rows[0]["BankOutNo"].ToString(), HNotes: " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + ((Control)(object)txtNotes).Text, IsOpenningJv: "0", Approved: "1", dtJvDetails: dtJvDetails, Deleted: "0", BranchID: GlobalVariables.CurrentBranchID, IsInternalJV: "1", UserID: GlobalVariables.UserID);
					dtJvDetails.Clear();
					dataRow4.Delete();
					if (dtBankOut.Rows[0]["JVID2"] != DBNull.Value && bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) == rbIsCheck.Checked)
					{
						dataRow4 = dtJvDetails.NewRow();
						dataRow4["JVDetailID"] = "-1";
						dataRow4["AccountID"] = dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPAccID"].ToString();
						dataRow4["Debit"] = ((Control)(object)txtPaidAmount).Text;
						dataRow4["Credit"] = "0";
						dataRow4["CurrencyID"] = ((TextEditorControlBase)cboCurrency).Value.ToString();
						dataRow4["ExchangeRate"] = ((Control)(object)txtExchangeRate).Text;
						...
						dataRow4 = dtJvDetails.NewRow();
						dataRow4["JVDetailID"] = "-1";
						dataRow4["AccountID"] = dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPUnderCollectionAccID"].ToString();
						dataRow4["Debit"] = "0";
						dataRow4["Credit"] = ((Control)(object)txtPaidAmount).Text;
						dataRow4["CurrencyID"] = ((TextEditorControlBase)cboCurrency).Value.ToString();
						dataRow4["ExchangeRate"] = ((Control)(object)txtExchangeRate).Text;
						...
						dataRow4["BranchID"] = GlobalVariables.CurrentBranchID;
						dtJvDetails.Rows.Add(dataRow4);
						JV.GenerateJV_Update(dtBankOut.Rows[0]["JVID2"].ToString(), dtBankOut.Rows[0]["JVNo2"].ToString(), DateTime.Parse(dtBankOut.Rows[0]["UnderCollectionDate"].ToString()).ToString(GlobalVariables.DateLongFormate), "18", num8.ToString(), dtBankOut.Rows[0]["BankOutNo"].ToString(), " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
						dtJvDetails.Clear();
						dataRow4.Delete();
					}
					if (dtBankOut.Rows[0]["JVID3"] != DBNull.Value && bool.Parse(dtBankOut.Rows[0]["IsCheck"].ToString()) == rbIsCheck.Checked)
					{
						dataRow4 = dtJvDetails.NewRow();
						dataRow4["JVDetailID"] = "-1";
						dataRow4["AccountID"] = dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["NPUnderCollectionAccID"].ToString();
						...
						dataRow4["BranchID"] = GlobalVariables.CurrentBranchID;
						dtJvDetails.Rows.Add(dataRow4);
						if (dtBankOut.Rows[0]["CollectionExpense"] != DBNull.Value && decimal.Parse(dtBankOut.Rows[0]["CollectionExpense"].ToString()) > 0m)
						{
							dataRow4 = dtJvDetails.NewRow();
							dataRow4["JVDetailID"] = "-1";
							dataRow4["AccountID"] = dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["CollectionExpensesAccID"];
							dataRow4["Debit"] = ((dtBankOut.Rows[0]["CollectionExpense"] == DBNull.Value) ? 0m : decimal.Parse(dtBankOut.Rows[0]["CollectionExpense"].ToString()));
							dataRow4["Credit"] = "0";
							dataRow4["CurrencyID"] = ((TextEditorControlBase)cboCurrency).Value.ToString();
							dataRow4["ExchangeRate"] = ((Control)(object)txtExchangeRate).Text;
							dataRow4["LocalDebit"] = decimal.Parse(dataRow4["Debit"].ToString()) * decimal.Parse(((Control)(object)txtExchangeRate).Text);
						...
						if (dtBankOut.Rows[0]["Returned"].Equals(true))
						{
						...
						else
						{
							dataRow4 = dtJvDetails.NewRow();
							dataRow4["JVDetailID"] = "-1";
							dataRow4["AccountID"] = dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["AccountID"];
							dataRow4["SubAccountID"] = dtBanks.Select(" BankID= " + ((TextEditorControlBase)cboBank).Value.ToString())[0]["SubAccountID"];
							dataRow4["Debit"] = "0";
							dataRow4["Credit"] = decimal.Parse(((Control)(object)txtPaidAmount).Text) + ((dtBankOut.Rows[0]["CollectionExpense"] == DBNull.Value) ? 0m : decimal.Parse(dtBankOut.Rows[0]["CollectionExpense"].ToString()));
							dataRow4["CurrencyID"] = ((TextEditorControlBase)cboCurrency).Value.ToString();
							dataRow4["ExchangeRate"] = ((Control)(object)txtExchangeRate).Text;
							dataRow4["LocalDebit"] = "0";
							dataRow4["LocalCredit"] = decimal.Parse(dataRow4["Credit"].ToString()) * decimal.Parse(((Control)(object)txtExchangeRate).Text);
						...
						JV.GenerateJV_Update(dtBankOut.Rows[0]["JVID3"].ToString(), dtBankOut.Rows[0]["JVNo3"].ToString(), DateTime.Parse(dtBankOut.Rows[0]["CollectionDate"].ToString()).ToString(GlobalVariables.DateLongFormate), "19", num8.ToString(), dtBankOut.Rows[0]["BankOutNo"].ToString(), " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
						dtJvDetails.Clear();
						dataRow4.Delete();
					}
				}
	...
	public override void DeleteData()
	{
		...
		try
		{
			...
			PSInvoicesPaymentsDetails.DeleteVirtualByPSInvoicePaymentID(drMaster["PSInvoicePaymentID"].ToString(), GlobalVariables.UserID);
			PSInvoicesPayments.DeleteVirtual(drMaster["PSInvoicePaymentID"].ToString(), GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
		}
		catch
		{
	...
	public override void btnPrintClick()
	{
		if (RowID != "" && drMaster != null)
		{
			...
			if (drMaster["BankOutID"] != DBNull.Value)
			{
				...
				GlobalVariables.ReportDocument.SetParameterValue("@BankOutIDs", "," + drMaster["BankOutID"].ToString() + ",");
				GlobalVariables.ReportDocument.SetParameterValue("@IsoCode", val);
				GlobalVariables.ReportDocument.SetParameterValue("@IsArabic", GlobalVariables.IsArabic ? "1" : "0");
				frmReporViwer2.ShowDialog();
				frmReporViwer2 = null;
			}
			else if (drMaster["SafeOutID"] != DBNull.Value)
			{
				GlobalVariables.ReportDocument.Load(GlobalVariables.ReportsPath + (GlobalVariables.IsArabic ? "Rep_SB_SafeOut_A.rpt" : "Rep_SB_SafeOut_E.rpt"));
				frmReporViwer frmReporViwer3 = new frmReporViwer();
				GlobalVariables.ReportDocument.SetParameterValue("@SafeOutIDs", "," + drMaster["SafeOutID"].ToString() + ",");
				GlobalVariables.ReportDocument.SetParameterValue("@IsoCode", val);
				GlobalVariables.ReportDocument.SetParameterValue("@IsArabic", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		dtSearchResult = SearchFunctions.PSInvoicesPaymentsSearchReport();
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["PSInvoicePaymentID"].ToString();
			FillData();
		}
	}
	...
	public override void SelectFullRow(object sender, EventArgs e)
	{
		...
		if (ULGData.ActiveCell != null && (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "PSInvoiceNo" || ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "PSInvoiceDate" || ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "NetPrice"))
		{
	...
	private void dtpJVDate_ValueChanged(object sender, EventArgs e)
	{
		...
		if (Adding)
		{
			((Control)(object)txtCode).Text = PSInvoicesPayments.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		}
	}
	...
	private void CalculateSupplierBalance()
	{
		if (cboSuppliers.SelectedIndex > -1 && cboCurrency.SelectedIndex > -1 && Adding)
		{
			decimal num = default(decimal);
			...
			num = SubAccounts.Balance(((TextEditorControlBase)cboSuppliers).Value.ToString(), "-1", "," + GlobalVariables.CurrentBranchID + ",", ((TextEditorControlBase)cboCurrency).Value.ToString(), IsFromServer: false, 0);
			num2 = PSInvoicesPayments.GetInvoicesBalancesBySubAccountID(((TextEditorControlBase)cboSuppliers).Value.ToString(), GlobalVariables.CurrentBranchID, ((TextEditorControlBase)cboCurrency).Value.ToString());
			if (num + num2 < 0m)
			{
	...
	public override void btnRefreshDataClick()
	{
		...
		object value3 = ((TextEditorControlBase)cboSuppliers).Value;
		...
		((TextEditorControlBase)cboSuppliers).Value = value3;
	...
	{
		if (rbIsSafe.Checked && dtSafeOut != null && dtSafeOut.Rows.Count > 0 && !Adding && !Updating && dtSafeOut.Rows[0]["JVID"] != DBNull.Value)
		{
			frmJV frmJV2 = new frmJV(int.Parse(dtSafeOut.Rows[0]["JVID"].ToString()));
		...
		else if (!rbIsSafe.Checked && dtBankOut != null && dtBankOut.Rows.Count > 0 && !Adding && !Updating && dtBankOut.Rows[0]["JVID"] != DBNull.Value)
		{
			frmJV frmJV3 = new frmJV(int.Parse(dtBankOut.Rows[0]["JVID"].ToString()));
	...
	{
		if (!rbIsSafe.Checked && dtBankOut != null && dtBankOut.Rows.Count > 0 && !Adding && !Updating && dtBankOut.Rows[0]["JVID2"] != DBNull.Value)
		{
			frmJV frmJV2 = new frmJV(int.Parse(dtBankOut.Rows[0]["JVID2"].ToString()));
			frmJV2.Size = new Size(base.Width, base.Height);
			frmJV2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmJV2.lblTitle).Text = (GlobalVariables.IsArabic ? "قيود اليومية" : "JV");
			frmJV2.ShowDialog();
	...
	{
		if (!rbIsSafe.Checked && dtBankOut != null && dtBankOut.Rows.Count > 0 && !Adding && !Updating && dtBankOut.Rows[0]["JVID3"] != DBNull.Value)
		{
			frmJV frmJV2 = new frmJV(int.Parse(dtBankOut.Rows[0]["JVID3"].ToString()));
			frmJV2.Size = new Size(base.Width, base.Height);
			frmJV2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmJV2.lblTitle).Text = (GlobalVariables.IsArabic ? "قيود اليومية" : "JV");
			frmJV2.ShowDialog();
	...
	private void btnChangeStatus_Click(object sender, EventArgs e)
	{
		if (dtBankOut != null && dtBankOut.Rows.Count > 0)
		{
			if (rbIsCheck.Checked && !bool.Parse(dtBankOut.Rows[0]["UnderCollection"].ToString()))
			{
				frmNPAndNRApproval frmNPAndNRApproval2 = new frmNPAndNRApproval(int.Parse(dtBankOut.Rows[0]["BankOutID"].ToString()), IsBankIn: false);
			...
			else if (rbIsCheck.Checked && !bool.Parse(dtBankOut.Rows[0]["Collected"].ToString()) && !bool.Parse(dtBankOut.Rows[0]["Returned"].ToString()))
			{
				frmNotesUnderCollection frmNotesUnderCollection2 = new frmNotesUnderCollection(int.Parse(dtBankOut.Rows[0]["BankOutID"].ToString()), IsBankIn: false);
	...
	private void RadioButtons_CheckedChanged(object sender, EventArgs e)
	{
		...
		((Control)(object)lblPayingBank).Visible = !rbIsSafe.Checked;
		((Control)(object)txtPayingBank).Visible = !rbIsSafe.Checked;
		...
		((Control)(object)btnChangeStatus).Visible = !Adding && !Updating && dtBankOut != null && dtBankOut.Rows.Count > 0 && rbIsCheck.Checked && !bool.Parse(dtBankOut.Rows[0]["Collected"].ToString()) && !bool.Parse(dtBankOut.Rows[0]["Returned"].ToString());
		((Control)(object)lblCheckStatus).Visible = !Adding && !Updating && dtBankOut != null && dtBankOut.Rows.Count > 0 && rbIsCheck.Checked;
	...
	private void btnSupplierSearch_Click(object sender, EventArgs e)
	{
		...
		if (num != 0)
		{
			((TextEditorControlBase)cboSuppliers).Value = num;
		}
	}

	private void btnSelectInvoice_Click(object sender, EventArgs e)
	{
		if (cboSuppliers.SelectedIndex != -1 && cboCurrency.SelectedIndex != -1)
		{
			dtSearchResult = SearchFunctions.PSInvoicesForInvoicesPaymentsReport(int.Parse(((TextEditorControlBase)cboSuppliers).Value.ToString()), "," + GlobalVariables.CurrentBranchID + ",", int.Parse(((TextEditorControlBase)cboCurrency).Value.ToString()), -1);
			for (int i = 0; i < dtSearchResult.Rows.Count; i++)
			{
				if (dtDetails.Select(" PSInvoiceID= " + dtSearchResult.Rows[i]["PSInvoiceID"].ToString()).Length == 0)
				{
					DataRow dataRow = dtDetails.NewRow();
					dataRow["PSInvoiceID"] = dtSearchResult.Rows[i]["PSInvoiceID"];
					dataRow["PSInvoiceNo"] = dtSearchResult.Rows[i]["PSInvoiceNo"];
					dataRow["PSInvoiceDate"] = dtSearchResult.Rows[i]["PSInvoiceDate"];
			...
			if (dtDetails.Rows.Count > 0)
			{
				InitGrid();
				CalculateTotals();
				((EditorButtonControlBase)cboSuppliers).ReadOnly = true;
				((Control)(object)btnSupplierSearch).Visible = false;
			}
		}
		dtSearchResult = null;
	}

	private void cboSuppliers_ValueChanged(object sender, EventArgs e)
	{
		CalculateSupplierBalance();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Purchasing.Transactions.frmPSInvoicesPayments));
		...
		this.rbPayable.BackColor = System.Drawing.Color.Transparent;
		this.rbPayable.Name = "rbPayable";
		this.rbPayable.TabStop = true;
		this.rbPayable.UseVisualStyleBackColor = false;
		this.rbPayable.CheckedChanged += new System.EventHandler(RadioButtons_CheckedChanged);
		...
		((ControlBase)this.btnSupplierSearch).Appearance = (AppearanceBase)(object)val12;
		...
		((TextEditorControlBase)this.cboSuppliers).AlwaysInEditMode = true;
		...
		((TextEditorControlBase)this.cboSuppliers).ValueChanged += new System.EventHandler(cboSuppliers_ValueChanged);
		...
		((System.Windows.Forms.Control)(object)this.txtCreditBalance).KeyPress += new System.Windows.Forms.KeyPressEventHandler(txtCreditBalance_KeyPress);
		...
		base.Name = "frmPSInvoicesPayments";
