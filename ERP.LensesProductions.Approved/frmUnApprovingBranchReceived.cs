...
public class frmUnApprovingBranchReceived : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingBranchReceived()
	{
		InitializeComponent();
		fromServer = true;
	...
	public override void FillGrid()
	{
		...
		dtsource = BlanksInvoices.SelectByBranchReceived("," + GlobalVariables.CurrentBranchID + ",", "1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsBranchReceive"].Header).Caption = "";
	...
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IsBranchReceive")
		{
	...
	public bool ValidateData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsBranchReceive"].Value.Equals(true) && Trans_Log.HasModificationCheckByDate(Convert.ToDateTime(DisplayDataDate).ToString(GlobalVariables.DateLongFormateMS), ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString(), "Lns_BlanksInvoices", IsFromServer: true))
			{
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsBranchReceive"].Value.Equals(true))
			{
		...
		if (text != ",")
		{
			BlanksInvoices.SetBranchReceived(text, "0", "Null", IsFromServer: true);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.LensesProductions.Approved.frmUnApprovingBranchReceived));
		...
		base.Name = "frmUnApprovingBranchReceived";
