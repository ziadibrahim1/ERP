...
public class frmSendToManufacturing : frmPosted
{
	private IContainer components = null;

	public frmSendToManufacturing()
	{
		InitializeComponent();
		fromServer = true;
	...
	public override void FillGrid()
	{
		...
		dtsource = BlanksInvoices.SelectByManufacturingSent("," + GlobalVariables.CurrentBranchID + ",", "0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["LineName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
	...
	{
		if (((GridItemBase)((UltraGridBase)ULGData).ActiveRow).Band.Index == 0 && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IsSent")
		{
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " Update Lns_BlanksInvoices Set IsSent = 1 , SentDate = '" + DateTime.Parse(((UltraGridBase)ULGData).Rows[i].Cells["SentDate"].Value.ToString()).ToString(GlobalVariables.DateLongFormate) + "' Where BlankInvoiceID  =" + ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString() + "; exec SP_Trans_Log " + GlobalVariables.UserID + " ,'Lns_BlanksInvoices' ," + ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString() + " ,'U';";
			}
		}
		if (text != "")
		{
	...
	public bool ValidateData()
	{
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsSent"].Value.ToString()) && Trans_Log.HasModificationCheckByDate(Convert.ToDateTime(DisplayDataDate).ToString(GlobalVariables.DateLongFormateMS), ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString(), "Lns_BlanksInvoices", IsFromServer: true))
			{
			...
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsSent"].Value.ToString()) && ((UltraGridBase)ULGData).Rows[i].Cells["SentDate"].Value == DBNull.Value)
			{
				GlobalVariables.InformationMB.Show(" لابد من إختيار تاريخ الارسال ", "Please Select Sent Date");
				ULGData.ActiveCell = ((UltraGridBase)ULGData).Rows[i].Cells["SentDate"];
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.LensesProductions.Approved.frmSendToManufacturing));
		...
		base.Name = "frmSendToManufacturing";
