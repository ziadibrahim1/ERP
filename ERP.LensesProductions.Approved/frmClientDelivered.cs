...
public class frmClientDelivered : frmPosted
{
	private IContainer components = null;

	public frmClientDelivered()
	{
		InitializeComponent();
		fromServer = false;
	...
	public override void FillGrid()
	{
		...
		dtsource = BlanksInvoices.SelectByClientDeliverd("," + GlobalVariables.CurrentBranchID + ",", "0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceiveDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ استلام الفرع" : "Branch Received Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsDeliverd"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsDeliverd"].Header).Caption = (GlobalVariables.IsArabic ? "استلام" : "Delivered");
	...
	{
		if (((GridItemBase)((UltraGridBase)ULGData).ActiveRow).Band.Index == 0 && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IsDeliverd")
		{
	...
	public override void SaveData()
	{
		...
		{
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsDeliverd"].Value.ToString()))
			{
				text = text + " Update Lns_BlanksInvoices Set IsDeliverd = 1 , DeliverdDate = '" + DateTime.Parse(((UltraGridBase)ULGData).Rows[i].Cells["DeliverdDate"].Value.ToString()).ToString(GlobalVariables.DateLongFormate) + "' Where IsBranchReceive = 1 And  BlankInvoiceID  =" + ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString() + "; exec SP_Trans_Log " + GlobalVariables.UserID + " ,'Lns_BlanksInvoices' ," + ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString() + " ,'U';";
			}
		}
		if (text != "")
		{
	...
	public bool ValidateData()
	{
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsDeliverd"].Value.ToString()) && Trans_Log.HasModificationCheckByDate(Convert.ToDateTime(DisplayDataDate).ToString(GlobalVariables.DateLongFormateMS), ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString(), "Lns_BlanksInvoices"))
			{
			...
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsDeliverd"].Value.ToString()) && ((UltraGridBase)ULGData).Rows[i].Cells["DeliverdDate"].Value == DBNull.Value)
			{
			...
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsDeliverd"].Value.ToString()) && Convert.ToDateTime(((UltraGridBase)ULGData).Rows[i].Cells["DeliverdDate"].Value) < Convert.ToDateTime(((UltraGridBase)ULGData).Rows[i].Cells["BranchReceiveDate"].Value))
			{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.LensesProductions.Approved.frmClientDelivered));
