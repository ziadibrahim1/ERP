...
public class frmUnApprovingSendToManufacturing : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingSendToManufacturing()
	{
		InitializeComponent();
		fromServer = true;
		NoCol = "BlankInvoiceNo";
		((Control)(object)btnPost).Text = (GlobalVariables.IsArabic ? "إعادة" : "UnSend");
	}

	public override void FillGrid()
	{
		DisplayDataDate = GlobalFunctions.GetServerDateTimeNow(IsFromServer: true);
		dtsource = BlanksInvoices.SelectByManufacturingSent("," + GlobalVariables.CurrentBranchID + ",", "1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsSent"].Header).Caption = "";
	...
	public bool ValidateData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsSent"].Value.Equals(true) && Trans_Log.HasModificationCheckByDate(Convert.ToDateTime(DisplayDataDate).ToString(GlobalVariables.DateLongFormateMS), ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString(), "Lns_BlanksInvoices", IsFromServer: true))
			{
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsSent"].Value.Equals(true))
			{
		...
		if (text != ",")
		{
			BlanksInvoices.SetSentToManufacturing(text, "0", "Null", IsFromServer: true);
			Main.SyncExecuteNonQuery(text2);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد فواتير تصنيع مرسله لإعادتها" : "There are No Sent Blank Invoices To Be Un Sent");
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.LensesProductions.Approved.frmUnApprovingSendToManufacturing));
		...
		base.Name = "frmUnApprovingSendToManufacturing";
