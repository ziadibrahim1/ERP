...
public class frmUnApprovingFactoryReceived : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingFactoryReceived()
	{
		InitializeComponent();
		fromServer = true;
		NoCol = "BlankInvoiceNo";
		((Control)(object)btnPost).Text = (GlobalVariables.IsArabic ? "عدم استلام" : "Un Receive");
	}

	public override void FillGrid()
	{
		DisplayDataDate = GlobalFunctions.GetServerDateTimeNow(IsFromServer: true);
		dtsource = BlanksInvoices.SelectByFactoryReceived("-1", "1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["FactoryReceiveDate"].Header).Caption = (GlobalVariables.IsArabic ? "التاريخ" : "Date");
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactoryReceive"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactoryReceive"].Header).Caption = "";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactoryReceive"].Width = (int)((double)((Control)(object)ULGData).Width * 0.05);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactoryReceive"].AllowRowFiltering = (DefaultableBoolean)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactoryReceive"].Header.CheckBoxVisibility = (HeaderCheckBoxVisibility)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactoryReceive"].Header.CheckBoxAlignment = (HeaderCheckBoxAlignment)2;
	}

	public override void SelectFullRow()
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IsFactoryReceive")
		{
	...
	public bool ValidateData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsFactoryReceive"].Value.Equals(true) && Trans_Log.HasModificationCheckByDate(Convert.ToDateTime(DisplayDataDate).ToString(GlobalVariables.DateLongFormateMS), ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString(), "Lns_BlanksInvoices", IsFromServer: true))
			{
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsFactoryReceive"].Value.Equals(true))
			{
		...
		if (text != ",")
		{
			BlanksInvoices.SetFactoryReceived(text, "0", "Null", IsFromServer: true);
			Main.SyncExecuteNonQuery(text2);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد فواتير تصنيع مستلمه لإعادتها" : "There are No Sent Blank Invoices To Be Un Received");
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.LensesProductions.Approved.frmUnApprovingFactoryReceived));
		...
		base.Name = "frmUnApprovingFactoryReceived";
