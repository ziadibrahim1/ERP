...
public class frmUnApprovingFactorySent : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingFactorySent()
	{
		InitializeComponent();
		fromServer = true;
		NoCol = "BlankInvoiceNo";
		((Control)(object)btnPost).Text = (GlobalVariables.IsArabic ? "عدم ارسال" : "Un Send");
	}

	public override void FillGrid()
	{
		DisplayDataDate = GlobalFunctions.GetServerDateTimeNow(IsFromServer: true);
		dtsource = BlanksInvoices.SelectByFactorySent("-1", "1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["FinishedDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الانتهاء" : "Finished Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactorySent"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsFactorySent"].Header).Caption = "";
	...
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IsFactorySent")
		{
	...
	public bool ValidateData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsFactorySent"].Value.Equals(true) && Trans_Log.HasModificationCheckByDate(Convert.ToDateTime(DisplayDataDate).ToString(GlobalVariables.DateLongFormateMS), ((UltraGridBase)ULGData).Rows[i].Cells["BlankInvoiceID"].Value.ToString(), "Lns_BlanksInvoices", IsFromServer: true))
			{
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["IsFactorySent"].Value.Equals(true))
			{
		...
		if (text != ",")
		{
			BlanksInvoices.SetFactorySent(text, "0", "Null", IsFromServer: true);
			Main.SyncExecuteNonQuery(text2);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد فواتير تصنيع لإعادتها" : "There are No Sent Blank Invoices To Be Un  Sent");
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.LensesProductions.Approved.frmUnApprovingFactorySent));
		...
		base.Name = "frmUnApprovingFactorySent";
