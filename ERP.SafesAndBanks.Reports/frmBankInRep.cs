...
public class frmBankInRep : frmReportTree2010
{
	...
	public frmBankInRep()
	{
		...
		TreeItems2IDCol = "BankInID";
		TreeItems2NameCol = "BankInNo";
	...
	{
		GetBranches();
		dtItems2 = BankIn.FillRepTree(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"));
		if (dtItems2 != null)
		{
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_BankIn_A.rpt" : "Rep_SB_BankIn_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_BankIn_A_nologo.rpt" : "Rep_SB_BankIn_E_nologo.rpt");
		}
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "2")
		{
			if (((UltraToggleEditorBase)chkWithLogo).Checked)
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_BankIn_summary_A.rpt" : "Rep_SB_BankIn_summary_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_BankIn_summary_A_nologo.rpt" : "Rep_SB_BankIn_summary_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		GlobalVariables.ReportDocument.SetParameterValue("@BankInIDs", Items2);
	...
	public override void ReportDoubleClick(string GroupNamePath, frmReporViwer frmViewer)
	{
		if (GroupNamePath != "")
		{
			string text = GroupNamePath.Substring(0, GroupNamePath.IndexOf("BankInID") + 8);
			if (text.IndexOf("BankInID") >= 0)
			{
				string s = GroupNamePath.Replace(text, "").Replace(",", "").Replace("[", "")
					.Replace("]", "");
				frmBankIn frmBankIn2 = new frmBankIn(int.Parse(s));
	...
	{
		dtSearchResult = SearchFunctions.BankInReportByDate(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"), -1, -1, -1, -1, -1, 0);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Reports.frmBankInRep));
