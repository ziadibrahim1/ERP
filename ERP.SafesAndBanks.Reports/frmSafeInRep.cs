...
public class frmSafeInRep : frmReportTree2010
{
	...
	public frmSafeInRep()
	{
		...
		TreeItems2IDCol = "SafeInID";
		TreeItems2NameCol = "SafeInNo";
		...
		cboReportType.Items.Add((object)2, GlobalVariables.IsArabic ? "طباعة الأذون إجمالى" : "Print Vouchers Totals");
	...
	{
		GetBranches();
		dtItems2 = SafeIn.FillRepTree(GlobalVariables.SafeIDs, Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"));
		if (dtItems2 != null)
		{
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_SafeIn_A.rpt" : "Rep_SB_SafeIn_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_SafeIn_A_nologo.rpt" : "Rep_SB_SafeIn_E_nologo.rpt");
		}
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "2")
		{
			if (((UltraToggleEditorBase)chkWithLogo).Checked)
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_SafeIn_summary_A.rpt" : "Rep_SB_SafeIn_summary_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_SafeIn_summary_A_nologo.rpt" : "Rep_SB_SafeIn_summary_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		GlobalVariables.ReportDocument.SetParameterValue("@SafeInIDs", Items2);
	...
	public override void ReportDoubleClick(string GroupNamePath, frmReporViwer frmViewer)
	{
		if (GroupNamePath != "")
		{
			string text = GroupNamePath.Substring(0, GroupNamePath.IndexOf("SafeInID") + 8);
			if (text.IndexOf("SafeInID") >= 0)
			{
				string s = GroupNamePath.Replace(text, "").Replace(",", "").Replace("[", "")
					.Replace("]", "");
				frmSafeIn frmSafeIn2 = new frmSafeIn(int.Parse(s));
				frmSafeIn2.StartPosition = FormStartPosition.CenterParent;
				((Control)(object)frmSafeIn2.lblTitle).Text = (GlobalVariables.IsArabic ? "وارد خزينه" : "Safe In");
	...
	{
		dtSearchResult = SearchFunctions.SafeInReportByDate(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"), -1, 0);
		for (int i = 0; i < dtSearchResult.Rows.Count; i++)
		{
			TreeItems2.GetNodeByKey(dtSearchResult.Rows[i]["SafeInID"].ToString()).CheckedState = CheckState.Checked;
		}
	}

	private void TreeItems2_AfterSelect(object sender, SelectEventArgs e)
	{
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Reports.frmSafeInRep));
