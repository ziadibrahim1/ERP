...
public class frmCustodyRep : frmReportTree2010
{
	private IContainer components = null;

	public frmCustodyRep()
	{
		TreeItems2ParentIDCol = "ParentID";
		TreeItems2IDCol = "CustodyID";
		TreeItems2NameCol = "CustodyNo";
		TreeItems2IsMainCol = "IsMain";
		InitializeComponent();
		cboReportType.Items.Clear();
		cboReportType.Items.Add((object)1, GlobalVariables.IsArabic ? "طباعه كل تفاصيل تسوية العهدة" : "Print Adustment Custody With All Details");
		cboReportType.Items.Add((object)2, GlobalVariables.IsArabic ? "طباعة تسوية العهد إجمالى" : "Print Adjustment Custody Totals");
	...
	{
		GetBranches();
		dtItems2 = Custody.FillRepTree(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"));
		if (dtItems2 != null)
		{
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_Custody_A.rpt" : "Rep_SB_Custody_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_Custody_A_nologo.rpt" : "Rep_SB_Custody_E_nologo.rpt");
		}
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "2")
		{
			if (((UltraToggleEditorBase)chkWithLogo).Checked)
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_Custody_summary_A.rpt" : "Rep_SB_Custody_summary_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SB_Custody_summary_A_nologo.rpt" : "Rep_SB_Custody_summary_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		if (Items2.Equals(","))
		{
			GlobalVariables.InformationMB.Show("لم تقم باختيار اى إذن تسوية عهدة ليتم عرضه ", "There is no choosen Adjustment Custody to be shown in the report, please check Adjustment Custody to be shown in report");
			return;
		}
		frmReporViwer frmReporViwer2 = new frmReporViwer();
		GlobalVariables.ReportDocument.SetParameterValue("@CustodyIDs", Items2);
	...
	public override void ReportDoubleClick(string GroupNamePath, frmReporViwer frmViewer)
	{
		if (GroupNamePath != "")
		{
			string text = GroupNamePath.Substring(0, GroupNamePath.IndexOf("CustodyID") + 9);
			if (text.IndexOf("CustodyID") >= 0)
			{
				string s = GroupNamePath.Replace(text, "").Replace(",", "").Replace("[", "")
					.Replace("]", "");
				frmCustody frmCustody2 = new frmCustody(int.Parse(s));
				frmCustody2.StartPosition = FormStartPosition.CenterParent;
				((Control)(object)frmCustody2.lblTitle).Text = (GlobalVariables.IsArabic ? "تسوية عهدة" : "Custody");
				frmCustody2.ShowDialog();
	...
	{
		dtSearchResult = SearchFunctions.CustodyReportByDate(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"), -1, 0);
		for (int i = 0; i < dtSearchResult.Rows.Count; i++)
		{
			TreeItems2.GetNodeByKey(dtSearchResult.Rows[i]["CustodyID"].ToString()).CheckedState = CheckState.Checked;
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Reports.frmCustodyRep));
