...
public class frmBlanksTypesDiscountSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmBlanksTypesDiscountSearchReport()
	{
		InitializeComponent();
	}

	public frmBlanksTypesDiscountSearchReport(DataTable dt, string idColumnName)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BlankTypeDiscountNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BlankTypeDiscountNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن" : "NO");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BlankTypeDiscountNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.13) - GlobalVariables.ScrollWidth;
