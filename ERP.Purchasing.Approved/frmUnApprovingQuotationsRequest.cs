...
public class frmUnApprovingQuotationsRequest : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingQuotationsRequest()
	{
		InitializeComponent();
	...
	{
		dtsource = QuotationsRequest.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSRequestNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الطلب " : "Request No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSRequestNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationRequestDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationRequestNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationRequestNo"].Header).Caption = (GlobalVariables.IsArabic ? "طلب عرض السعر" : "Quotation Request No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationRequestNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["QuotationRequestID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			QuotationsRequest.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عروض أسعار لفك إعتمادها " : "There are No Quotation Request to UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmQuotationsRequest frmQuotationsRequest2 = new frmQuotationsRequest(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["QuotationRequestID"].Value.ToString()));
			frmQuotationsRequest2.Size = new Size(base.Width, base.Height);
			frmQuotationsRequest2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmQuotationsRequest2.lblTitle).Text = (GlobalVariables.IsArabic ? "طلب عرض  سعر" : "Quotations Request");
			frmQuotationsRequest2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.QuotationsRequestReport(1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["QuotationRequestID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["QuotationRequestID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Purchasing.Approved.frmUnApprovingQuotationsRequest));
		...
		base.Name = "frmUnApprovingQuotationsRequest";
