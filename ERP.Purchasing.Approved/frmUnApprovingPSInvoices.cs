...
public class frmUnApprovingPSInvoices : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingPSInvoices()
	{
		InitializeComponent();
		NoCol = "PSInvoiceNo";
	...
	{
		dtsource = PSInvoices.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSOrderNo"].Header).Caption = (GlobalVariables.IsArabic ? "امر الشراء" : "PS Order No");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["PSInvoiceID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			PSInvoices.SetApprove("0", text);
			FillGrid();
		}
		else
		{
	...
	public override void ClickCellButton()
	{
		...
		{
			frmPSInvoices frmPSInvoices4 = new frmPSInvoices(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["PSInvoiceID"].Value.ToString()));
			frmPSInvoices4.Size = new Size(base.Width, base.Height);
			frmPSInvoices4.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmPSInvoices4.lblTitle).Text = (GlobalVariables.IsArabic ? "فاتورة المشتريات" : "Purchasing Invoice");
			frmPSInvoices4.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.PSInvoicesReport(1, 0, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["PSInvoiceID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["PSInvoiceID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Purchasing.Approved.frmUnApprovingPSInvoices));
		...
		base.Name = "frmUnApprovingPSInvoices";
