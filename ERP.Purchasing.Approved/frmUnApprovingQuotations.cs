...
public class frmUnApprovingQuotations : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingQuotations()
	{
		InitializeComponent();
		NoCol = "QuotationNo";
	...
	{
		dtsource = Quotations.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Quotation No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.125) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.075);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotationRequestNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["QuotationID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			Quotations.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عروض أسعار لفك اعتمادها " : "There are No Quotations to UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmQuotations frmQuotations2 = new frmQuotations(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["QuotationID"].Value.ToString()));
			frmQuotations2.Size = new Size(base.Width, base.Height);
			frmQuotations2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmQuotations2.lblTitle).Text = (GlobalVariables.IsArabic ? "عــــرض  ســـعـر" : "Quotations");
			frmQuotations2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.PSQuotationstReport(1, 0);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Purchasing.Approved.frmUnApprovingQuotations));
		...
		base.Name = "frmUnApprovingQuotations";
