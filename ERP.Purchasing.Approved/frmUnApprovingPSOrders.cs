...
public class frmUnApprovingPSOrders : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingPSOrders()
	{
		InitializeComponent();
		NoCol = "PSOrderNo";
	...
	{
		dtsource = PSOrders.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSOrderNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "PSOrder No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSOrderNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSOrderDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSOrderDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSOrderDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.075);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSRequestNo"].Header).Caption = (GlobalVariables.IsArabic ? "طلب الشراء" : "PSRequest No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PSRequestNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["PSOrderID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			PSOrders.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد أوامر شراء لفك إعتمادها " : "There are No Orders to  UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmPSOrders frmPSOrders2 = new frmPSOrders(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["PSOrderID"].Value.ToString()));
			frmPSOrders2.Size = new Size(base.Width, base.Height);
			frmPSOrders2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmPSOrders2.lblTitle).Text = (GlobalVariables.IsArabic ? "اوامــر  الــشــراء" : "Purchase Orders");
			frmPSOrders2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.PSOrdersReport(1, 0, -1, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["PSOrderID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["PSOrderID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Purchasing.Approved.frmUnApprovingPSOrders));
		...
		base.Name = "frmUnApprovingPSOrders";
