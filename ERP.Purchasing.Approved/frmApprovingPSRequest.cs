...
public class frmApprovingPSRequest : frmPosted
{
	private IContainer components = null;

	public frmApprovingPSRequest()
	{
		InitializeComponent();
		NoCol = "PSRequestNo";
	}
	...
	{
		dtsource = PSRequest.SelectByApproved(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " EXEC PS_PSRequest_SetApprove  1," + (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsRefused"].Value.ToString()) ? "1" : "0") + "," + ((UltraGridBase)ULGData).Rows[i].Cells["PSRequestID"].Value.ToString() + " ; ";
			}
		}
		if (text != "")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات شراء لإعتمادها " : "There are No Purchase Request to Approve");
		}
	}
	...
	{
		DataTable dataTable = SearchFunctions.PSRequestReport(0, -1, 0, 0);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Purchasing.Approved.frmApprovingPSRequest));
		...
		base.Name = "frmApprovingPSRequest";
