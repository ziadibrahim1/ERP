...
public class frmCaptainsTree : frmTree2
{
	...
	public override void PrepareData()
	{
		if (TableName != "" && LevelsTable != "")
		{
			...
			dtChart = SubAccounts.FillTreeBySubAccountTypeIDs(GlobalVariables.CaptainSubAccountTypeIDs, "-1", IsFromServer: true);
		...
		dtSubAccountType = SuAccountsTypes.SelectBySuAccountsTypeIDs(GlobalVariables.CaptainSubAccountTypeIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void ClearControls()
	{
		...
		if (Adding)
		{
			...
			((Control)(object)txtNo).Text = Captains.GetCode(IsFromServer: true);
			((TextEditorControlBase)cboType).Value = 11;
	...
	public void DisplayCaptainData()
	{
		CaptainID = "-1";
		DataTable dataTable = Captains.SelectBySubAccountID(((KeyedSubObjectBase)SelectedNode).Key, GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		if (dataTable.Rows.Count > 0)
		{
			DataRow dataRow = dataTable.Rows[0];
			CaptainID = dataRow["CaptainID"].ToString();
			((TextEditorControlBase)cboTitle).Value = dataRow["TitleID"];
			dtpBirthDate.Value = dataRow["BirthDate"];
			((Control)(object)txtNo).Text = dataRow["CaptainNo"].ToString();
	...
	public override void TreeDeleteData()
	{
		...
		try
		{
			Captains.DeleteBySubAccountID(((KeyedSubObjectBase)SelectedNode).Key, GlobalVariables.UserID, IsFromServer: true);
			SubAccounts_Details.DeleteBySubAccountID(((KeyedSubObjectBase)SelectedNode).Key, GlobalVariables.UserID, IsFromServer: true);
	...
	public override bool ValidateData()
	{
		...
		{
			GlobalVariables.InformationMB.Show("من فضلك ادخل الحساب الإفتراضي للقبطان", "Enter Default Captain Account");
			((UltraTabControlBase)tabItemType).SelectedTab = ((UltraTabControlBase)tabItemType).Tabs["Captain"];
	...
	{
		int num = SearchFunctions.CaptainsSearch("-1", IsFromServer: true);
		if (num != 0)
		{
	...
	{
		GlobalVariables.ReportDocument.Load(GlobalVariables.ReportsPath + (GlobalVariables.IsArabic ? "Rep_MS_Captains_A.rpt" : "Rep_MS_Captains_E.rpt"));
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.MarineService.MasterData.frmCaptainsTree));
		...
		((ControlBase)this.lblCDCExpireDate).Appearance = (AppearanceBase)(object)val3;
		...
		((ControlBase)this.lblIDIssueDate).Appearance = (AppearanceBase)(object)val4;
		...
		((ControlBase)this.lblCDCIssueDate).Appearance = (AppearanceBase)(object)val5;
		...
		((ControlBase)this.lblIDNo).Appearance = (AppearanceBase)(object)val6;
		...
		((KeyedSubObjectBase)val30).Key = "Captain";
		val30.TabPage = this.tabItem;
		resources.ApplyResources(val30, "ultraTab1");
