...
public class frmSLInvoicesEInvoiceSend : frmBase
{
	...
	public frmSLInvoicesEInvoiceSend()
	{
	...
	public void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SLInvoiceNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.05) - GlobalVariables.ScrollWidth;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SLInvoiceNo"].Header).Caption = (GlobalVariables.IsArabic ? "الرقم" : "No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SLInvoiceDate"].Hidden = false;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SLInvoiceDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.06);
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SLInvoiceDate"].Header).Caption = (GlobalVariables.IsArabic ? "التاريخ" : "Date ");
	...
	private void ULGData_ClickCellButton(object sender, CellEventArgs e)
	{
		...
		{
			frmSLInvoicesEInvoice frmSLInvoicesEInvoice2 = new frmSLInvoicesEInvoice(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["SLInvoiceID"].Value.ToString()));
			frmSLInvoicesEInvoice2.Size = new Size(base.Width, base.Height);
			frmSLInvoicesEInvoice2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmSLInvoicesEInvoice2.lblTitle).Text = (GlobalVariables.IsArabic ? "فاتورة المبيعات الالكترونية" : "E-Invoices");
			frmSLInvoicesEInvoice2.ShowDialog();
		}
	}
	...
	private async void btnSendInvoices_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				strIDs = strIDs + ((UltraGridBase)ULGData).Rows[i].Cells["SLInvoiceID"].Value.ToString() + ",";
			}
		}
		if (strIDs == ",")
		{
		...
		for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
		{
			...
			{
				int State = await EINV.GetDocumentState(GlobalFunctions.GetDefault("EINVClientID"), GlobalFunctions.GetDefault("EINVClientSecret1"), UseEInvoiceProductionEnvirnoment, ((UltraGridBase)ULGData).Rows[j].Cells["EInvoiceUUID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[j].Cells["SLInvoiceID"].Value.ToString());
				if (State > -1)
				{
		...
		for (int k = 0; k < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; k++)
		{
			...
			{
				strIDs = strIDs + ((UltraGridBase)ULGData).Rows[k].Cells["SLInvoiceID"].Value.ToString() + ",";
				lstSLInvoiceID.Add(int.Parse(((UltraGridBase)ULGData).Rows[k].Cells["SLInvoiceID"].Value.ToString()));
			}
		}
		if (strIDs != ",")
		{
			string Message = EINV.ValidationBySLInvoiceIDs(strIDs, GlobalVariables.IsArabic);
			if (Message != "")
			{
			...
			await EINV.DocumentSubmission(GlobalFunctions.GetDefault("EINVClientID"), GlobalFunctions.GetDefault("EINVClientSecret1"), UseEInvoiceProductionEnvirnoment, strIDs, GlobalVariables.UserID, GlobalVariables.IsArabic);
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد فواتير لإرسالها " : "There are No Invoice To Send");
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["SendInvoice"].Value.ToString()) && ((UltraGridBase)ULGData).Rows[i].Cells["EInvoiceUUID"].Value.ToString() != "")
			{
				lstInvoices.Add(new List<string>
				{
					...
					((UltraGridBase)ULGData).Rows[i].Cells["EInvoiceInternalCode"].Value.ToString().Replace("/", "-") + "-" + ((UltraGridBase)ULGData).Rows[i].Cells["SLInvoiceNo"].Value.ToString()
				});
			}
		}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.EInvoices.Transactions.frmSLInvoicesEInvoiceSend));
		...
		((AppearanceBase)val32).Image = resources.GetObject("appearance27.Image");
		resources.ApplyResources(val32, "appearance27");
		((ControlBase)this.lblCancelledDoc).Appearance = (AppearanceBase)(object)val32;
		...
		((AppearanceBase)val33).BackColor = System.Drawing.Color.FromArgb(255, 128, 0);
		resources.ApplyResources(val33, "appearance28");
		((ControlBase)this.lblCancelledDocColor).Appearance = (AppearanceBase)(object)val33;
		...
		base.Name = "frmSLInvoicesEInvoiceSend";
