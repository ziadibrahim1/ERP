...
public class frmLnsInvoicesReturnsByInvoiceEInvoiceSend : frmBase
{
	...
	public frmLnsInvoicesReturnsByInvoiceEInvoiceSend()
	{
	...
	{
		dtDetails = InvoicesReturns.SelectForEInvoice((cboBranches.SelectedIndex == -1) ? GlobalVariables.BranchIDs : ("," + ((TextEditorControlBase)cboBranches).Value.ToString() + ","), (dtpFromDate.Value == null) ? "Null" : dtpFromDate.DateTime.ToString(GlobalVariables.DateLongFormate), (dtpToDate.Value == null) ? "Null" : dtpToDate.DateTime.ToString(GlobalVariables.DateLongFormate), (cboState.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboState).Value.ToString(), GlobalVariables.IsArabic ? "1" : "0");
	...
	private void ULGData_ClickCellButton(object sender, CellEventArgs e)
	{
		...
		{
			frmLnsInvoicesReturnsByInvoiceEInvoice frmLnsInvoicesReturnsByInvoiceEInvoice2 = new frmLnsInvoicesReturnsByInvoiceEInvoice(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ReturnID"].Value.ToString()));
			frmLnsInvoicesReturnsByInvoiceEInvoice2.Size = new Size(base.Width, base.Height);
			frmLnsInvoicesReturnsByInvoiceEInvoice2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmLnsInvoicesReturnsByInvoiceEInvoice2.lblTitle).Text = (GlobalVariables.IsArabic ? "مرتجع بالفاتورة البيع المباشر الالكترونية" : "E-Invoices Returns By Invoice ");
			frmLnsInvoicesReturnsByInvoiceEInvoice2.ShowDialog();
		}
	}
	...
	private async void btnSendInvoices_Click(object sender, EventArgs e)
	{
		...
		for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
		{
			...
			{
				int State = await EINV.GetDocumentStateRLns(GlobalFunctions.GetDefault("EINVClientID"), GlobalFunctions.GetDefault("EINVClientSecret1"), UseEInvoiceProductionEnvirnoment, ((UltraGridBase)ULGData).Rows[j].Cells["EInvoiceUUID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[j].Cells["ReturnID"].Value.ToString());
				if (State > -1)
				{
		...
		if (strIDs != ",")
		{
			string Message = EINV.ValidationByRLnsInvoiceIDs(strIDs, GlobalVariables.IsArabic);
			if (Message != "")
			{
			...
			await EINV.DocumentSubmissionRLns(GlobalFunctions.GetDefault("EINVClientID"), GlobalFunctions.GetDefault("EINVClientSecret1"), UseEInvoiceProductionEnvirnoment, strIDs, GlobalVariables.UserID, GlobalVariables.IsArabic);
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد فواتير لإرسالها " : "There are No Invoice To Send");
	...
	private async void btnCancelled_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			if (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["SendInvoice"].Value.ToString()) && ((UltraGridBase)ULGData).Rows[i].Cells["EInvoiceUUID"].Value.ToString() != "")
			{
				...
				{
					((UltraGridBase)ULGData).Rows[i].Cells["EInvoiceUUID"].Value.ToString(),
					((UltraGridBase)ULGData).Rows[i].Cells["ReturnNo"].Value.ToString()
				});
			}
		}
		if (lstInvoices.Count == 0)
		...
		EINV.Set_Connection(GlobalVariables.Server, GlobalVariables.DatabaseName, GlobalVariables.dbUserID, GlobalVariables.dbPassword);
		await EINV.DocumentCancellationRLns(GlobalFunctions.GetDefault("EINVClientID"), GlobalFunctions.GetDefault("EINVClientSecret1"), UseEInvoiceProductionEnvirnoment, lstInvoices, GlobalVariables.UserID);
		FillGrid();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.EInvoices.Transactions.frmLnsInvoicesReturnsByInvoiceEInvoiceSend));
		...
		ValueListItem val36 = new ValueListItem();
		...
		val18.DataValue = "1";
		resources.ApplyResources(val18, "valueListItem1");
		((SubObjectBase)val18).ForceApplyResources = "";
		val19.DataValue = "2";
		resources.ApplyResources(val19, "valueListItem2");
		((SubObjectBase)val19).ForceApplyResources = "";
		val20.DataValue = "3";
		resources.ApplyResources(val20, "valueListItem3");
		((SubObjectBase)val20).ForceApplyResources = "";
		val21.DataValue = "4";
		resources.ApplyResources(val21, "valueListItem4");
		((SubObjectBase)val21).ForceApplyResources = "";
		val22.DataValue = "5";
		resources.ApplyResources(val22, "valueListItem5");
		...
		((ControlBase)this.btnCancelled).Appearance = (AppearanceBase)(object)val30;
		...
		((ControlBase)this.lblBranch).Appearance = (AppearanceBase)(object)val31;
		...
		val35.DataValue = "4";
		resources.ApplyResources(val35, "valueListItem9");
		((SubObjectBase)val35).ForceApplyResources = "";
		val36.DataValue = "5";
		resources.ApplyResources(val36, "valueListItem10");
		((SubObjectBase)val36).ForceApplyResources = "";
		this.cboBranches.Items.AddRange((ValueListItem[])(object)new ValueListItem[5] { val32, val33, val34, val35, val36 });
		...
		((AppearanceBase)val37).Image = resources.GetObject("appearance27.Image");
		resources.ApplyResources(val37, "appearance27");
		((ControlBase)this.lblSubmittedDoc).Appearance = (AppearanceBase)(object)val37;
		...
		((AppearanceBase)val38).BackColor = System.Drawing.Color.DodgerBlue;
		resources.ApplyResources(val38, "appearance28");
		((ControlBase)this.lblSubmittedDocColor).Appearance = (AppearanceBase)(object)val38;
		...
		((AppearanceBase)val40).BackColor = System.Drawing.Color.FromArgb(255, 128, 0);
		resources.ApplyResources(val40, "appearance30");
		((ControlBase)this.lblCancelledDocColor).Appearance = (AppearanceBase)(object)val40;
		...
		base.Name = "frmLnsInvoicesReturnsByInvoiceEInvoiceSend";
