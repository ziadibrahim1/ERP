...
public class frmContractsDuesSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmContractsDuesSearchReport()
	{
		InitializeComponent();
	}

	public frmContractsDuesSearchReport(DataTable dt, string idColumnName, int ApprovedValue, int DeletedValue)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueCode"].Header).Caption = (GlobalVariables.IsArabic ? "رقم المستخلص " : "Contract ]عثس No");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueDate"].Header).Caption = (GlobalVariables.IsArabic ? "التاريخ " : "Date");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractCode"].Header).Caption = (GlobalVariables.IsArabic ? "رقم المقايسه " : "Contract No");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ProjectName"].Header).Caption = (GlobalVariables.IsArabic ? "المشروع" : "Project Name");
