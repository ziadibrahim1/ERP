...
public class frmReservationsSearch : frmSearch
{
	private IContainer components = null;

	public frmReservationsSearch()
	{
		InitializeComponent();
	}

	public frmReservationsSearch(DataTable dt, string idColumnName)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReservationNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReservationDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReservationDate"].Header).Caption = (GlobalVariables.IsArabic ? "التاريخ" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReservationDate"].MaskInput = "dd/mm/yyyy hh:mm:ss tt";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReservationDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PaidAmount"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["NetPrice"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["NetPrice"].Header).Caption = (GlobalVariables.IsArabic ? "صافى الحجز" : "Net Price");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsDeliverd"].Header).Caption = (GlobalVariables.IsArabic ? "تم التسليم" : "Delivered");
