...
using ERP.Classes;
using ERP.CRM.Transactions;
using Infragistics.Shared;
using Infragistics.Win;
using Infragistics.Win.UltraWinEditors;
using Infragistics.Win.UltraWinGrid;

namespace ERP.CRM.Approved;

public class frmUnApprovingCustomersReservations : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingCustomersReservations()
	{
		InitializeComponent();
	...
	{
		dtsource = CustomersReservations.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Reservation No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.11) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.075);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ClientName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.17);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CurrencyName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.07);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["EmployeeName"].Header).Caption = (GlobalVariables.IsArabic ? "مندوب البيع" : "Employee Name");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["EmployeeName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsCash"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsCash"].Header).Caption = (GlobalVariables.IsArabic ? "نقدي" : "Cash");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsCash"].Width = (int)((double)((Control)(object)ULGData).Width * 0.05);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["CustomerReservationID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			CustomersReservations.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا يوجد أي حجز لإعتماده " : "There are No Reservation to Approve");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmCustomersReservations frmCustomersReservations2 = new frmCustomersReservations(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["CustomerReservationID"].Value.ToString()));
			frmCustomersReservations2.Size = new Size(base.Width, base.Height);
			frmCustomersReservations2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmCustomersReservations2.lblTitle).Text = (GlobalVariables.IsArabic ? "حجز العميل" : "Customer Reservation");
			frmCustomersReservations2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.CustomersReservationsReport(1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["CustomerReservationID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["CustomerReservationID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.CRM.Approved.frmUnApprovingCustomersReservations));
		...
		base.Name = "frmUnApprovingCustomersReservations";
