...
public class frmVisaTypesReturns : frmHeaderDetails
{
	...
	public frmVisaTypesReturns(int _VisaTypeReturnID)
		: this()
	{
		VisaTypeReturnID = _VisaTypeReturnID;
	}
	...
	{
		if (VisaTypeReturnID != 0)
		{
			DataTable dataTable = Expenses.Select(VisaTypeReturnID.ToString(), GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
		...
		{
			DataTable dataTable2 = VisaTypesReturns.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = VisaTypesReturns.Insert_Update(drMaster["VisaTypeReturnID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), (cboVisaType.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboVisaType).Value.ToString(), (((Control)(object)txtVisaNo).Text == "") ? "Null" : ((Control)(object)txtVisaNo).Text, drMaster["ShiftDetailID"].ToString(), drMaster["ShiftDetailUserID"].ToString(), "Null", (cboAccountName.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboAccountName).Value.ToString(), (cboSubAccountName.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboSubAccountName).Value.ToString(), dtPOSDefaultData.Rows[0]["CurrencyID"].ToString(), dtCurrency.Select(" CurrencyID=  " + dtPOSDefaultData.Rows[0]["CurrencyID"].ToString())[0]["ExchangeRate"].ToString(), ((Control)(object)txtNotes).Text, (((Control)(object)txtTotal).Text == "") ? "0" : ((Control)(object)txtTotal).Text, dtSubAccounts.Select(" SubAccountID= " + ((TextEditorControlBase)cboSubAccountName).Value.ToString())[0]["BranchID"].ToString(), (drMaster["TransferJVID"] == DBNull.Value) ? "Null" : drMaster["TransferJVID"].ToString(), (drMaster["TransferJVID2"] == DBNull.Value) ? "Null" : drMaster["TransferJVID2"].ToString(), "1", "Null", "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
	...
	public override void btnRefreshDataClick()
	{
		...
		object value2 = ((TextEditorControlBase)cboAccountName).Value;
		object value3 = ((TextEditorControlBase)cboSubAccountName).Value;
		object value4 = ((TextEditorControlBase)cboClientCode).Value;
		object value5 = ((TextEditorControlBase)cboBranches).Value;
		dtPOSDefaultData = BusinessLayer.POS.Settings.SelectByBranchIDWithoutImage(GlobalVariables.CurrentBranchID, GlobalVariables.IsArabic ? "1" : "0");
		dtVisaType = VisaTypes.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		if (Adding)
		{
		...
		dataView2.RowFilter = "ForAllBranches = 1 or  BranchID= " + ((TextEditorControlBase)cboBranches).Value.ToString();
		GlobalFunctions.FillCombo(cboSubAccountName, dataView2.ToTable(), "SubAccountID", "SubAccountName");
		GlobalFunctions.FillCombo(cboClientCode, dataView2.ToTable(), "SubAccountID", "ClientSupplierNo");
		dtBranches = Branches.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		GlobalFunctions.FillCombo(cboBranches, dtBranches, "BranchID", "BranchName");
		((TextEditorControlBase)cboVisaType).Value = value;
		((TextEditorControlBase)cboAccountName).Value = value2;
		((TextEditorControlBase)cboSubAccountName).Value = value3;
		((TextEditorControlBase)cboClientCode).Value = value4;
		((TextEditorControlBase)cboBranches).Value = value5;
	}
	...
	private void cboBranches_ValueChanged(object sender, EventArgs e)
	{
		if (cboBranches.SelectedIndex > -1)
		{
			...
			dataView.RowFilter = "ForAllBranches = 1 or  BranchID= " + ((TextEditorControlBase)cboBranches).Value.ToString();
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Lenses.Transactions.frmVisaTypesReturns));
