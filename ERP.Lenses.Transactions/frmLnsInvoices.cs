...
public class frmLnsInvoices : frmHeaderDetails
{
	private InputLanguage Language;

	private DataTable dtClientsContacts;
	...
	private UltraButton btnImportData;
	...
	private UltraComboEditor cboContacts;

	private UltraLabel lblContact;

	public frmLnsInvoices()
	{
	...
	public override void PrepareData()
	{
		...
		dtClientsContacts = SubAccountsClientSupplierContacts.FillCombo(IsFromServer: false);
		GlobalFunctions.FillCombo(cboContacts, dtClientsContacts, "ClientSupplierContactID", "ContactName");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			IsDisplayData = false;
			((TextEditorControlBase)cboClient).ValueChanged += cboClient_ValueChanged;
			((TextEditorControlBase)cboContacts).Value = drMaster["ClientSupplierContactID"];
	...
	public override void SetControls(bool NavMode)
	{
		...
		((EditorButtonControlBase)cboContacts).ReadOnly = NavMode;
		...
		((Control)(object)btnImportData).Visible = !NavMode;
		NewPriceUserID = 0;
		DiscountUserID = 0;
	...
	public override void btnPrintClick()
	{
		...
		if (dtReports.Rows.Count > 0 && (dtReports.Rows[0]["ProcedureName"].ToString().Trim() == "QRBarcode" || (dtReports.Rows.Count > 0 && dtReports.Rows[0]["ProcedureName"].ToString().Trim() == "QRBarcode2")))
		{
			...
			if (dtReports.Rows[0]["ProcedureName"].ToString().Trim() == "QRBarcode2")
			{
	...
	{
		object value = ((TextEditorControlBase)cboPaymentMethod).Value;
		UsingColors = GlobalFunctions.GetOption("UsingItemsColors");
		UsingSizes = GlobalFunctions.GetOption("UsingItemsSizes");
		if (UsingColors)
		{
		...
		{
			DataView dataView = new DataView(dtClientsContacts);
			dataView.RowFilter = "SubAccountID = " + ((TextEditorControlBase)cboClient).Value.ToString();
			GlobalFunctions.FillCombo(cboContacts, dataView.ToTable(), "ClientSupplierContactID", "ContactName");
		}
		...
		dtPriceType = PricesTypes.FillCombo("-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		GlobalFunctions.FillCombo(cboPriceType, dtPriceType, "PriceTypeID", "PriceName");
		((TextEditorControlBase)cboPaymentMethod).Value = value;
	}
	...
	public override bool ValidateData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			if (((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value != DBNull.Value && decimal.Parse(((Control)(object)txtRestAmount).Text) > 0m && dtItems.Select("ItemID = " + ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value.ToString())[0]["ItemThirdClassificationID"].ToString() == "1" && dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["SubAccountClassificationID"].ToString() != "100")
			{
			...
			if (dataTable.Select("StoreID = " + ((UltraGridBase)ULGData).Rows[i].Cells["StoreID"].Value.ToString()).Length == 0)
			{
				((Control)(object)ULGData).Enter -= ULGData_Enter;
				GlobalVariables.InformationMB.Show("برجاء إختيار المخزن  ", "Please Select Store Name");
				ULGData.ActiveCell = ((UltraGridBase)ULGData).Rows[i].Cells["StoreID"];
	...
	public void AddItemInGid()
	{
		...
		if (dtItems.Select(" ItemBarCode = '" + text + "'").Length == 0)
		{
			...
			if (dtItems.Select(" ItemBarCode = '" + text + "'").Length == 0)
			{
				if (dtItems.Select(" Name = '" + text + "'").Length == 0)
				{
					((TextEditorControlBase)txtBarCode).Clear();
				...
				text = dtItems.Select(" Name = '" + text + "'")[0]["ItemBarCode"].ToString();
			}
		}
		...
		{
			((UltraGridBase)ULGData).ActiveRow.Cells["TaxID"].Value = ((dtPOSDefaultData.Rows[0]["TaxID"] == DBNull.Value) ? dtItems.Select(" ItemBarCode = '" + text + "'")[0]["TaxID"] : dtPOSDefaultData.Rows[0]["TaxID"]);
		}
		else
		{
	...
	private void ULGData_KeyDown(object sender, KeyEventArgs e)
	{
		...
		{
			ULGData.Selected.Rows.AddRange((UltraGridRow[])((SubObjectsCollectionBase)((UltraGridBase)ULGData).Rows).All);
			ULGData.PerformAction((UltraGridAction)48);
			e.Handled = true;
		}
		...
		else if (e.KeyValue == 40 && ULGData.ActiveCell != null && (ULGData.ActiveCell.ValueList != null || ULGData.ActiveCell.Column.ValueList != null))
		{
	...
	private void ULGData_AfterCellUpdate(object sender, CellEventArgs e)
	{
		...
		{
			decimal num = decimal.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["StockBalance"].Value.ToString());
			if (decimal.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["Qty"].Value.ToString()) > num)
			{
				GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? ("  الحد الاقصى للكمية  " + Math.Round(num, 3)) : ("Maximum Allowed Qty " + num));
				((UltraGridBase)ULGData).ActiveRow.Cells["Qty"].Value = num;
				((UltraGridBase)ULGData).ActiveRow.Cells["TotalPrice"].Value = decimal.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["UnitPrice"].Value.ToString()) * decimal.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["Qty"].Value.ToString());
				CalculateGoss();
			}
		}
	...
	private void cboClient_ValueChanged(object sender, EventArgs e)
	{
		...
		((TextEditorControlBase)cboClientCode).ValueChanged += cboClientCode_ValueChanged;
		cboContacts.SelectedIndex = -1;
		((Control)(object)cboContacts).Text = "";
		if (cboClient.SelectedIndex <= -1)
		{
			return;
		}
		...
		object value = ((TextEditorControlBase)cboContacts).Value;
		DataView dataView = new DataView(dtClientsContacts);
		dataView.RowFilter = "SubAccountID = " + ((TextEditorControlBase)cboClient).Value.ToString();
		GlobalFunctions.FillCombo(cboContacts, dataView.ToTable(), "ClientSupplierContactID", "ContactName");
		((TextEditorControlBase)cboContacts).Value = value;
		...
		if (dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["PriceTypeID"] != DBNull.Value)
		{
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				if (((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value != DBNull.Value)
				{
					DataRow[] array = dtItemPrices.Select(" ItemID= " + ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value.ToString());
					if (array.Length != 0)
					{
						decimal num = decimal.Parse(array[0]["Price"].ToString()) - decimal.Parse(array[0]["Price"].ToString()) * Math.Round(decimal.Parse(array[0]["DiscountPercentage"].ToString()), 3) / 100m;
						((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value = num;
						((UltraGridBase)ULGData).Rows[i].Cells["TotalPrice"].Value = num * decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value.ToString());
						CalculateRow(((UltraGridBase)ULGData).Rows[i]);
					}
				}
			}
		...
		if (Adding || Updating)
		{
			...
			{
				DataView dataView2 = new DataView(dtSalesMan);
				dataView2.RowFilter = " SubAccountID=" + dtClients.Select("SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["EmployeeID"].ToString();
				GlobalFunctions.FillCombo(cboSalesMan2, dataView2.ToTable(), "SubAccountID", "SubAccountName");
	...
	private void cboStore_ValueChanged(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				((UltraGridBase)ULGData).UpdateData();
				GetItemStockBlance(((UltraGridBase)ULGData).Rows[i]);
			}
		}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Lenses.Transactions.frmLnsInvoices));
		...
		this.cboContacts = new UltraComboEditor();
		this.lblContact = new UltraLabel();
		...
		((System.ComponentModel.ISupportInitialize)this.cboContacts).BeginInit();
		base.SuspendLayout();
		...
		((System.Windows.Forms.Control)(object)this.btnImportData).Click += new System.EventHandler(btnStoreTransfer_Click);
		...
		((ControlBase)this.btnPaymentMethodSearch).Appearance = (AppearanceBase)(object)val17;
		...
		resources.ApplyResources(this.cboContacts, "cboContacts");
		((TextEditorControlBase)this.cboContacts).AlwaysInEditMode = true;
		this.cboContacts.AutoCompleteMode = (AutoCompleteMode)4;
		((System.Windows.Forms.Control)(object)this.cboContacts).Name = "cboContacts";
		((System.Windows.Forms.Control)(object)this.cboContacts).KeyDown += new System.Windows.Forms.KeyEventHandler(cboPaymentMethod_KeyDown);
		resources.ApplyResources(this.lblContact, "lblContact");
		this.lblContact.AutoEllipsis = false;
		((System.Windows.Forms.Control)(object)this.lblContact).Name = "lblContact";
		((ControlBase)this.lblContact).WrapText = false;
		base.AcceptButton = null;
		resources.ApplyResources(this, "$this");
		base.Controls.Add((System.Windows.Forms.Control)(object)this.btnPaymentMethodSearch);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.lblContact);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.lblPaymentMethod);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.cboContacts);
		...
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.cboContacts, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.lblPaymentMethod, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.lblContact, 0);
		...
		((System.ComponentModel.ISupportInitialize)this.cboContacts).EndInit();
