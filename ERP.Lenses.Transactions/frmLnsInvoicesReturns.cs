...
public class frmLnsInvoicesReturns : frmHeaderDetails
{
	...
	private DataTable dtMaxReturnedQtyOrignal;

	private DataTable dtMaxReturnedQty;
	...
	public UltraButton btnSelectItems;
	...
	public frmLnsInvoicesReturns(int ID)
		: this()
	{
		RowID = ID.ToString();
	}
	...
	public override void AddData()
	{
		...
		try
		{
			num = InvoicesReturns.Insert_Update("-1", ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "Null", ShiftDetailID.ToString(), ShiftDetailUserID.ToString(), GlobalVariables.UserID, (cboClient.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboClient).Value.ToString(), "Null", ((Control)(object)txtNotes).Text, (((Control)(object)txtTotal).Text == "") ? "0" : ((Control)(object)txtTotal).Text, (((Control)(object)txtPaidAmount).Text == "") ? "0" : ((Control)(object)txtPaidAmount).Text, (((Control)(object)txtRestAmount).Text == "") ? "0" : ((Control)(object)txtRestAmount).Text, dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value.ToString())[0]["BranchID"].ToString(), "Null", "Null", "Null", "Null", "0", "Null", "Null", "1", "Null", "Null", "Null", "1", "Null", "Null", "0", "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
	...
	public void FastPrint()
	{
		...
		instance.AddTextCell("الرصيدالحالى بالفرع", font4, 28f / instance.OverallWidth, 4f, StringAlignment.Center, darkBlue, DrawRectangle: true, black2, lineWidth);
		instance.AcceptChanges();
	...
	private void CalculateTotals()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			num += decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value.ToString()) * (decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value.ToString()) - decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["TotalDiscount"].Value.ToString()) + decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["TaxValue"].Value.ToString()));
		}
	...
	private void cboClient_ValueChanged(object sender, EventArgs e)
	{
		...
		if (dtMaxReturnedQtyOrignal != null && dtMaxReturnedQtyOrignal.Rows.Count > 0)
		{
			dtMaxReturnedQtyOrignal.Rows.Clear();
		}
	}
	...
	private void btnSelectItems_Click(object sender, EventArgs e)
	{
		...
		if (Adding && (dtMaxReturnedQtyOrignal == null || dtMaxReturnedQtyOrignal.Rows.Count == 0))
		{
			dtMaxReturnedQtyOrignal = InvoicesReturnsDetails.SelectMaxReturnedQtyByClient("-1", ((TextEditorControlBase)cboClient).Value.ToString(), (GlobalFunctions.GetDefault("ClientReturnPeriod") == "") ? "0" : GlobalFunctions.GetDefault("ClientReturnPeriod"), GlobalVariables.CurrentBranchID, GlobalVariables.IsArabic ? "1" : "0");
			dtMaxReturnedQty = dtMaxReturnedQtyOrignal.Copy();
			dtMaxReturnedQty.Rows.Clear();
		}
		else if (Updating && (dtMaxReturnedQtyOrignal == null || dtMaxReturnedQtyOrignal.Rows.Count == 0))
		{
			dtMaxReturnedQtyOrignal = InvoicesReturnsDetails.SelectMaxReturnedQtyByClient(RowID, ((TextEditorControlBase)cboClient).Value.ToString(), (GlobalFunctions.GetDefault("ClientReturnPeriod") == "") ? "0" : GlobalFunctions.GetDefault("ClientReturnPeriod"), GlobalVariables.CurrentBranchID, GlobalVariables.IsArabic ? "1" : "0");
			dtMaxReturnedQty = dtMaxReturnedQtyOrignal.Copy();
			dtMaxReturnedQty.Rows.Clear();
		}
		if (dtMaxReturnedQtyOrignal == null || dtMaxReturnedQtyOrignal.Rows.Count <= 0)
		...
		frmLnsInvoicesReturnsDetails frmLnsInvoicesReturnsDetails2 = new frmLnsInvoicesReturnsDetails(dtMaxReturnedQtyOrignal, dtMaxReturnedQty);
		frmLnsInvoicesReturnsDetails2.Size = new Size(Screen.PrimaryScreen.WorkingArea.Width, Screen.PrimaryScreen.WorkingArea.Height);
		frmLnsInvoicesReturnsDetails2.Location = new Point(0, 0);
		((Control)(object)frmLnsInvoicesReturnsDetails2.lblTitle).Text = (GlobalVariables.IsArabic ? "الكمية المرتجعة" : "Returned Qty");
		frmLnsInvoicesReturnsDetails2.ShowDialog();
		StringWriter stringWriter = new StringWriter();
		DataView dataView = new DataView(frmLnsInvoicesReturnsDetails2.DtMaxReturnedQty);
		dataView.RowFilter = "Qty > 0 And ItemID is not null";
		dataView.ToTable("Table", false, "OriginalItemID", "ColorID", "ItemSizeID", "BatchID").WriteXml((TextWriter)stringWriter);
		((DataTable)((UltraGridBase)ULGData).DataSource).Rows.Clear();
		if (dataView.Count > 0)
		{
			((UltraGridBase)ULGData).DataSource = InvoicesReturnsDetails.SelectByItemIDs(stringWriter.ToString(), Adding ? "-1" : RowID, ((TextEditorControlBase)cboClient).Value.ToString(), (GlobalFunctions.GetDefault("ClientReturnPeriod") == "") ? "0" : GlobalFunctions.GetDefault("ClientReturnPeriod"), GlobalVariables.CurrentBranchID, GlobalVariables.IsArabic ? "1" : "0");
			InitGrid();
			for (int i = 0; i < dataView.Count; i++)
			{
				decimal num = decimal.Parse(dataView[i]["Qty"].ToString());
				DataRow[] array = ((DataTable)((UltraGridBase)ULGData).DataSource).Select(" ItemID = " + dataView[i]["OriginalItemID"].ToString() + " And ColorID = " + dataView[i]["ColorID"].ToString() + " And ItemSizeID = " + dataView[i]["ItemSizeID"].ToString() + ((dataView[i]["BatchID"] == DBNull.Value) ? "" : (" And BatchID = " + dataView[i]["BatchID"].ToString())));
				for (int j = 0; j < array.Length; j++)
				{
					if (decimal.Parse(array[j]["MaxAllowedQty"].ToString()) >= num)
					{
						array[j]["Qty"] = num;
					}
					else if (num >= decimal.Parse(array[j]["MaxAllowedQty"].ToString()))
					{
						array[j]["Qty"] = decimal.Parse(array[j]["MaxAllowedQty"].ToString());
					}
					num -= decimal.Parse(array[j]["Qty"].ToString());
					if (num == 0m)
					{
						break;
					}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Lenses.Transactions.frmLnsInvoicesReturns));
		...
		((ControlBase)this.btnBranchesSearch).Appearance = (AppearanceBase)(object)val9;
		...
		base.Name = "frmLnsInvoicesReturns";
