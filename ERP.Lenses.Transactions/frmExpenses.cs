...
public class frmExpenses : frmHeaderDetails
{
	...
	private UltraTextEditor txtNotes2;

	private UltraLabel lblNotes2;
	...
	public frmExpenses(int _ExpenseID)
		: this()
	{
	...
	public override void FillData()
	{
		if (ExpenseID != 0)
		{
			DataTable dataTable = Expenses.Select(ExpenseID.ToString(), GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
		...
		{
			DataTable dataTable2 = Expenses.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((Control)(object)txtNotes2).Text = drMaster["Notes2"].ToString();
	...
	public override void SetControls(bool NavMode)
	{
		...
		((EditorButtonControlBase)txtNotes2).ReadOnly = NavMode || (!Main.IsSynchronization && drMaster != null && drMaster["Approved"].Equals(true)) || (dtConn != null && !dtConn.Rows[0]["SyncBranchID"].Equals(DBNull.Value));
	}
	...
	public override void ClearControls()
	{
		...
		((TextEditorControlBase)txtNotes2).Clear();
	...
	public override bool ValidateData()
	{
		...
		if (GlobalFunctions.GetOption("EnforceSubAccountsUse") && cboAccountName.SelectedIndex > -1)
		{
		...
		else if (Main.CheckForValueByBranchIDAndFiscalYearID("Lns_Expenses", "ExpenseNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["ExpenseNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
	...
	public override void AddData()
	{
		...
		try
		{
			...
			{
				if (dtReports.Rows[0]["Rep" + (GlobalVariables.IsArabic ? "_A" : "_E")].ToString().Trim() == "Rep_Lns_ExpensesFastPrint")
				{
					RowID = num.ToString();
					FastPrint();
					return;
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = Expenses.Insert_Update(drMaster["ExpenseID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), drMaster["ShiftDetailID"].ToString(), drMaster["ShiftDetailUserID"].ToString(), (drMaster["User_ID"] == DBNull.Value) ? "Null" : drMaster["User_ID"].ToString(), "Null", "Null", (cboAccountName.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboAccountName).Value.ToString(), (cboSubAccountName.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboSubAccountName).Value.ToString(), drMaster["CurrencyID"].ToString(), drMaster["ExchangeRate"].ToString(), ((Control)(object)txtNotes).Text, ((Control)(object)txtNotes2).Text, (((Control)(object)txtTotal).Text == "") ? "0" : ((Control)(object)txtTotal).Text, (cboSubAccountName.SelectedIndex > -1 && cboAccountName.SelectedIndex > -1 && dtSubAccounts.Select(" (SubAccountTypeID=4 or SubAccountTypeID=5 or SubAccountTypeID=7)And DefaultClientAccountID=" + ((TextEditorControlBase)cboAccountName).Value.ToString() + "  and SubAccountID= " + ((TextEditorControlBase)cboSubAccountName).Value.ToString()).Length != 0) ? dtSubAccounts.Select(" (SubAccountTypeID=4 or SubAccountTypeID=5 or SubAccountTypeID=7)And DefaultClientAccountID=" + ((TextEditorControlBase)cboAccountName).Value.ToString() + " and SubAccountID= " + ((TextEditorControlBase)cboSubAccountName).Value.ToString())[0]["BranchID"].ToString() : "Null", (drMaster["TransferJVID"] == DBNull.Value) ? "Null" : drMaster["TransferJVID"].ToString(), (drMaster["TransferJVID2"] == DBNull.Value) ? "Null" : drMaster["TransferJVID2"].ToString(), "1", "Null", bool.Parse(drMaster["Approved"].ToString()) ? "1" : "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			...
			{
				if (bool.Parse(GlobalVariables.dtSystemModules.Select(" ModuleEnName= 'POS'")[0]["Installed"].ToString()) && GlobalFunctions.GetOption("POSSalesAutoGenerateJVs"))
				{
					ShiftsDetails.SalesJVDineInRegenerate(drMaster["ShiftDetailID"].ToString(), GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
					ShiftsDetails.ReturnJVDineInRegenerate(drMaster["ShiftDetailID"].ToString(), GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				...
				if (bool.Parse(GlobalVariables.dtSystemModules.Select(" ModuleEnName= 'Lenses'")[0]["Installed"].ToString()))
				{
	...
	{
		DataTable dataTable = Main.ExecuteQuery_DataTable(" Rep_Lns_Expenses '," + RowID + ",'," + (GlobalVariables.IsArabic ? "1" : "0"));
		...
		instance.AcceptChanges();
		instance.AddTextCell(dataRow["ExpenseNo"].ToString(), font5, 0.7f, 4f, StringAlignment.Far, black, DrawRectangle: true, black2, lineWidth);
		instance.AddTextCell("اذن مصروفات برقم", font5, 0.3f, 4f, StringAlignment.Near, darkBlue, DrawRectangle: true, black2, lineWidth);
		instance.AcceptChanges();
		instance.AddTextCell(((DateTime)dataRow["ExpenseDate"]).ToString("dd/MM/yyyy hh:mm:ss tt"), font, 0.7f, 4f, StringAlignment.Far, black, DrawRectangle: true, black2, lineWidth);
	...
	public override void btnPrintClick()
	{
		...
		try
		{
			...
			GlobalFunctions.ConfigureReport(reportDocument);
			reportDocument.SetParameterValue("@ExpenseIDs", "," + RowID + ",");
			reportDocument.SetParameterValue("@BranchID", GlobalVariables.CurrentBranchID);
			reportDocument.SetParameterValue("@IsArabic", GlobalVariables.IsArabic ? "1" : "0");
			reportDocument.PrintOptions.PrinterName = GlobalVariables.POSPrinter;
			try
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Lenses.Transactions.frmExpenses));
		...
		this.txtNotes2 = new UltraTextEditor();
		this.lblNotes2 = new UltraLabel();
		...
		resources.ApplyResources(this.txtNotes2, "txtNotes2");
		((System.Windows.Forms.Control)(object)this.txtNotes2).Name = "txtNotes2";
		this.lblNotes2.AutoEllipsis = false;
		resources.ApplyResources(this.lblNotes2, "lblNotes2");
		((System.Windows.Forms.Control)(object)this.lblNotes2).Name = "lblNotes2";
		((ControlBase)this.lblNotes2).WrapText = false;
		...
		base.Controls.Add((System.Windows.Forms.Control)(object)this.lblNotes2);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.lblNotes);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.txtNotes2);
		...
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.txtNotes2, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.lblNotes, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.lblNotes2, 0);
