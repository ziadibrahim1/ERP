...
public class frmSafeIn : frmHeaderDetails
{
	...
	public override void PrepareData()
	{
		...
		dtDetails = SafeInDetails.SelectBySafeInID("0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = SafeIn.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((Control)(object)txtCode).Text = drMaster["SafeInNo"].ToString();
			dtpDate.Value = (DateTime)drMaster["SafeInDate"];
			...
			dtDetails = SafeInDetails.SelectBySafeInID(drMaster["SafeInID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
	...
	public override void ClearControls()
	{
		...
		((Control)(object)txtCode).Text = (Adding ? SafeIn.GetCodeByBranchID((cboSafe.SelectedIndex == -1) ? "" : ((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID) : "");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInDetailsID"].DefaultCellValue = -1;
	...
	public override bool ValidateData()
	{
		...
		if (Main.CheckForValueByBranchIDAndFiscalYearID("SB_SafeIn", "SafeInNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["SafeInNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = SafeIn.GetCodeByBranchID(((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
	...
	public override void AddData()
	{
		...
		try
		{
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				...
				((UltraGridBase)ULGData).Rows[i].Cells["SafeInDetailsID"].Value = -1;
				((UltraGridBase)ULGData).Rows[i].Cells["SafeInID"].Value = num;
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
				ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			}
			SafeInDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
			string code = JV.GetCode(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID, "1");
			int num2 = JV.GenerateJV_Insert(code, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "1", num.ToString(), ((Control)(object)txtCode).Text, " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
			((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + code + ")قيد رقم ") : ("JV No ( " + code + " )"));
			Main.ExecuteNonQuery(" Update SB_SafeIn  set JvID= " + num2 + " Where SafeInID=" + num.ToString());
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = SafeIn.Insert_Update(drMaster["SafeInID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, ((Control)(object)txtChargedPerson).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, "Null", drMaster["JVID"].ToString(), bool.Parse(drMaster["Approved"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, bool.Parse(drMaster["Deleted"].ToString()) ? "1" : "0", GlobalVariables.UserID);
			JVDetails.DeleteByJVID(drMaster["JVID"].ToString(), GlobalVariables.UserID);
			dtJvDetails = JVDetails.Select("0", "-1", GlobalVariables.IsArabic ? "1" : "0");
			string text = ",";
			DataRow dataRow = dtJvDetails.NewRow();
			dataRow["JVDetailID"] = "-1";
			dataRow["AccountID"] = dtsafes.Select(" SafeID= " + ((TextEditorControlBase)cboSafe).Value.ToString())[0]["AccountID"].ToString();
			dataRow["SubAccountID"] = dtsafes.Select(" SafeID= " + ((TextEditorControlBase)cboSafe).Value.ToString())[0]["SubAccountID"];
			dataRow["Debit"] = ((Control)(object)txtTotal).Text;
			dataRow["Credit"] = "0";
			dataRow["CurrencyID"] = ((TextEditorControlBase)cboCurrency).Value.ToString();
	...
	public override void btnPrintClick()
	{
		...
		if (RowID != "")
		{
			...
			GlobalVariables.ReportDocument.SetParameterValue("@SafeInIDs", "," + RowID + ",");
	...
	{
		dtSearchResult = SearchFunctions.SafeInReport(-1, 0);
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["SafeInID"].ToString();
			FillData();
		}
	}
	...
	{
		if ((Adding || Updating) && cboSafe.SelectedIndex > -1)
		{
			((Control)(object)txtCode).Text = SafeIn.GetCodeByBranchID((cboSafe.SelectedIndex == -1) ? "" : ((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		}
	}
	...
	private void btnSelectClients_Click(object sender, EventArgs e)
	{
		...
		for (int i = 0; i < dtSearchResult.Rows.Count; i++)
		{
			...
			dataRow["SafeInDetailsID"] = -1;
			if (drMaster != null)
			{
				dataRow["SafeInID"] = drMaster["SafeInID"];
			}
			dataRow["Value"] = 0;
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.SafeTransactions.frmSafeIn));
