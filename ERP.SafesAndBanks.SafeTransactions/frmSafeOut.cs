...
public class frmSafeOut : frmHeaderDetails
{
	...
	private UltraCheckEditor chkIsCustody;
	...
	public override void PrepareData()
	{
		...
		dtDetails = SafeOutDetails.SelectBySafeOutID("0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = SafeOut.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((Control)(object)txtCode).Text = drMaster["SafeOutNo"].ToString();
			dtpDate.Value = (DateTime)drMaster["SafeOutDate"];
			...
			((UltraToggleEditorBase)chkIsCustody).Checked = drMaster["IsCustody"].Equals(true);
			rbIsRequest.Checked = drMaster["IsRequest"].Equals(true);
			...
			dtDetails = SafeOutDetails.SelectBySafeOutID(drMaster["SafeOutID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
			...
			if (rbIsRequest.Checked)
			{
				...
				dtRequests = Main.ExecuteQuery_DataTable(" Select SafeOutRequestID,SafeOutRequestNo from  SB_SafeOutRequests Where SafeOutID =" + drMaster["SafeOutID"].ToString());
				Main.Fillclb(clbRequestNo, dtRequests, "SafeOutRequestID", "SafeOutRequestNo");
				for (int i = 0; i < clbRequestNo.Items.Count; i++)
				{
					clbRequestNo.SetItemChecked(i, value: true);
				}
	...
	public override void SetControls(bool NavMode)
	{
		...
		((Control)(object)chkIsCustody).Enabled = !NavMode;
		rbIsDirect.Enabled = Adding;
		rbIsRequest.Enabled = Adding;
	...
	public override void ClearControls()
	{
		...
		((Control)(object)txtCode).Text = (Adding ? SafeOut.GetCodeByBranchID((cboSafe.SelectedIndex == -1) ? "" : ((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID) : "");
		...
		((UltraToggleEditorBase)chkIsCustody).Checked = false;
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Override.AllowDelete = (DefaultableBoolean)(((Adding || Updating) && rbIsDirect.Checked) ? 1 : 2);
		((UltraGridBase)ULGData).DisplayLayout.Override.AllowAddNew = (AllowAddNew)(((Adding || Updating) && rbIsDirect.Checked) ? 6 : 2);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeOutDetailsID"].DefaultCellValue = -1;
	...
	public override bool ValidateData()
	{
		...
		if (cboSafe.SelectedIndex < 0)
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "برجاء إختيار الخزنة" : "Please Select the safe ");
			((TextEditorControlBase)cboSafe).Focus();
			return false;
		}
		if (Main.CheckForValueByBranchIDAndFiscalYearID("SB_SafeOut", "SafeOutNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["SafeOutNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = SafeOut.GetCodeByBranchID(((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
	...
	public override void AddData()
	{
		...
		try
		{
			int num = SafeOut.Insert_Update("-1", ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, ((UltraToggleEditorBase)chkIsCustody).Checked ? "1" : "0", rbIsRequest.Checked ? "1" : "0", ((Control)(object)txtChargedPerson).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, (cboTax.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboTax).Value.ToString(), (((Control)(object)txtTaxValue).Text == "") ? "0" : ((Control)(object)txtTaxValue).Text, ((Control)(object)txtTotalAfterTax).Text, "Null", "0", GlobalVariables.CurrentBranchID, "0", GlobalVariables.UserID);
			SafeOutRequests.UpdateSafeOutID(GetRequestsIDs(), num.ToString(), GlobalVariables.UserID);
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				...
				((UltraGridBase)ULGData).Rows[i].Cells["SafeOutDetailsID"].Value = -1;
				((UltraGridBase)ULGData).Rows[i].Cells["SafeOutID"].Value = num;
			...
			dataRow["Debit"] = "0";
			dataRow["Credit"] = decimal.Parse(((Control)(object)txtTotalAfterTax).Text);
			dataRow["CurrencyID"] = ((TextEditorControlBase)cboCurrency).Value.ToString();
			dataRow["ExchangeRate"] = ((Control)(object)txtExchangeRate).Text;
			dataRow["LocalDebit"] = "0";
			dataRow["LocalCredit"] = decimal.Parse(((Control)(object)txtTotalAfterTax).Text) * decimal.Parse(((Control)(object)txtExchangeRate).Text);
			dataRow["Deleted"] = false;
			dataRow["BranchID"] = GlobalVariables.CurrentBranchID;
			dtJvDetails.Rows.Add(dataRow);
			...
			SafeOutDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
			string code = JV.GetCode(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID, "2");
			int num2 = JV.GenerateJV_Insert(code, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "2", num.ToString(), ((Control)(object)txtCode).Text, " ( " + ((Control)(object)txtChargedPerson).Text + " ) " + ((Control)(object)txtNotes).Text, "0", "1", dtJvDetails, "0", GlobalVariables.CurrentBranchID, "1", GlobalVariables.UserID);
			((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + code + ")قيد رقم ") : ("JV No ( " + code + " )"));
			Main.ExecuteNonQuery(" Update SB_SafeOut  set JvID= " + num2 + " Where SafeOutID=" + num.ToString());
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = SafeOut.Insert_Update(drMaster["SafeOutID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSafe).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, ((UltraToggleEditorBase)chkIsCustody).Checked ? "1" : "0", rbIsRequest.Checked ? "1" : "0", ((Control)(object)txtChargedPerson).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, (cboTax.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboTax).Value.ToString(), (((Control)(object)txtTaxValue).Text == "") ? "0" : ((Control)(object)txtTaxValue).Text, ((Control)(object)txtTotalAfterTax).Text, drMaster["JVID"].ToString(), bool.Parse(drMaster["Approved"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, bool.Parse(drMaster["Deleted"].ToString()) ? "1" : "0", GlobalVariables.UserID);
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				...
				((UltraGridBase)ULGData).Rows[i].Cells["SafeOutID"].Value = num;
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["SafeOutDetailsID"].Value.ToString() + ",";
				ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			}
			dataRow = dtJvDetails.NewRow();
			dataRow["JVDetailID"] = "-1";
	...
	public override void DeleteData()
	{
		...
		try
		{
			...
			SafeOutRequests.UpdateSafeOutID(GetRequestsIDs(), "Null", GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
		}
		catch
		{
	...
	public override void btnPrintClick()
	{
		...
		if (RowID != "")
		{
			...
			GlobalVariables.ReportDocument.SetParameterValue("@SafeOutIDs", "," + RowID + ",");
	...
	{
		dtSearchResult = SearchFunctions.SafeOutReport(-1, 0);
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["SafeOutID"].ToString();
			FillData();
		}
	}
	...
	{
		if (Adding && cboSafe.SelectedIndex > -1)
		{
			((Control)(object)txtCode).Text = SafeOut.GetCodeByBranchID((cboSafe.SelectedIndex == -1) ? "" : ((TextEditorControlBase)cboSafe).Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		}
	}
	...
	private void cboCurrency_ValueChanged(object sender, EventArgs e)
	{
		if (cboCurrency.SelectedIndex != -1)
		{
			...
			{
				dtRequests = Main.ExecuteQuery_DataTable(" Select SafeOutRequestID,SafeOutRequestNo from  SB_SafeOutRequests Where Deleted=0 And BranchID=" + GlobalVariables.CurrentBranchID + "And CurrencyID=" + ((TextEditorControlBase)cboCurrency).Value.ToString() + " And Approved=1 And SafeOutID is null ");
				Main.Fillclb(clbRequestNo, dtRequests, "SafeOutRequestID", "SafeOutRequestNo");
			}
		}
		else
	...
	private void CalculateTotals()
	{
		...
		{
			((Control)(object)txtTaxValue).Text = decimal.Parse(Math.Round(num * (decimal.Parse(dtTaxs.Select(" TaxID= " + ((TextEditorControlBase)cboTax).Value.ToString())[0]["TaxPercent"].ToString()) / 100m), 8).ToString()).ToString(GlobalVariables.txtDecimalFormate);
		}
	...
	public override void btnRefreshDataClick()
	{
		...
		object value2 = ((TextEditorControlBase)cboSafe).Value;
		object value3 = ((TextEditorControlBase)cboTax).Value;
		...
		((TextEditorControlBase)cboSafe).Value = value2;
		((TextEditorControlBase)cboTax).Value = value3;
	...
	private void rbIsDirect_CheckedChanged(object sender, EventArgs e)
	{
		if (Adding)
		{
			...
			if (rbIsRequest.Checked && cboCurrency.SelectedIndex != -1 && Adding)
			{
				dtRequests = Main.ExecuteQuery_DataTable(" Select SafeOutRequestID,SafeOutRequestNo from  SB_SafeOutRequests Where Deleted=0 And BranchID=" + GlobalVariables.CurrentBranchID + "And CurrencyID=" + ((TextEditorControlBase)cboCurrency).Value.ToString() + " And Approved=1 And SafeOutID is null ");
				Main.Fillclb(clbRequestNo, dtRequests, "SafeOutRequestID", "SafeOutRequestNo");
			}
		...
		bool visible = (clbRequestNo.Visible = rbIsRequest.Checked);
	...
	private void chkAll_CheckedChanged(object sender, EventArgs e)
	{
		...
		if (requestsIDs != ",")
		{
			dtDetails = SafeOutDetails.FillBySafeOutRequestIDs(requestsIDs, GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
			CalculateTotals();
		}
	...
	public string GetRequestsIDs()
	{
		...
		for (int i = 0; i < clbRequestNo.Items.Count; i++)
		{
			...
			{
				text = text + dtRequests.Rows[i]["SafeOutRequestID"].ToString() + ",";
			}
		}
		return text;
	}
	...
	public override void txtCode_KeyUp(object sender, KeyEventArgs e)
	{
		...
		DataTable comboData = Main.GetComboData(TableName, IDCol, NoCol + "=''" + ((Control)(object)txtCode).Text.Trim() + "'' And Deleted=0 And  (charindex('',''+rtrim(ltrim(str(SafeID)))+'','', ''" + GlobalVariables.SafeIDs + "'' ) > 0 )");
	...
	public override void PriveousData()
	{
		...
		{
			DataTable dataTable = Main.SelectNext(GlobalVariables.BranchIDs, TableName, IDCol, NoCol, DateCol, RowID, " And  (charindex('',''+rtrim(ltrim(str(SafeID)))+'','', ''" + GlobalVariables.SafeIDs + "'' ) > 0) ", "0");
			if (dataTable.Rows.Count > 0)
			{
	...
	public override void NextData()
	{
		...
		{
			DataTable dataTable = Main.SelectNext(GlobalVariables.BranchIDs, TableName, IDCol, NoCol, DateCol, RowID, " And  (charindex('',''+rtrim(ltrim(str(SafeID)))+'','', ''" + GlobalVariables.SafeIDs + "'' ) > 0) ", "1");
			if (dataTable.Rows.Count > 0)
			{
				RowID = dataTable.Rows[0][IDCol].ToString();
			}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.SafeTransactions.frmSafeOut));
		...
		this.chkIsCustody = new UltraCheckEditor();
		...
		((System.ComponentModel.ISupportInitialize)this.chkIsCustody).BeginInit();
		...
		resources.ApplyResources(this.chkIsCustody, "chkIsCustody");
		((System.Windows.Forms.Control)(object)this.chkIsCustody).Name = "chkIsCustody";
		...
		base.Controls.Add((System.Windows.Forms.Control)(object)this.chkIsCustody);
		...
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.chkIsCustody, 0);
		...
		((System.ComponentModel.ISupportInitialize)this.chkIsCustody).EndInit();
