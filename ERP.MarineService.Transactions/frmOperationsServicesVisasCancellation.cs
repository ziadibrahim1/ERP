...
public class frmOperationsServicesVisasCancellation : frmDetails
{
	...
	public frmOperationsServicesVisasCancellation()
	{
	...
	public frmOperationsServicesVisasCancellation(DataTable DTServices, string OPERATIONSERVICEID, string OPERATIONID, string SERVICEID, string VESSELID, bool READONLY)
		: this()
	{
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["OperationServiceVisaCancellationID"].DefaultCellValue = -1;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManRankID"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ValidFromDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
	...
	public override void DisplayData()
	{
		...
		{
			dtDetails = OperationsServicesVisasCancellation.SelectByOperationServiceID(OperationServiceID.ToString(), GlobalVariables.IsArabic ? "1" : "0");
			InitGrid();
		}
	}
	...
	public override void SaveData()
	{
		if (NoInvoice)
		{
			...
			try
			{
				...
				for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
				{
					text = text + ((UltraGridBase)ULGData).Rows[i].Cells["OperationServiceVisaCancellationID"].Value.ToString() + ",";
				}
				Main.DeleteForUpdate("MS_OperationsServicesVisasCancellation", "OperationServiceID", OperationServiceID, "OperationServiceVisaCancellationID", text);
				if (((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count > 0)
				{
					OperationsServicesVisasCancellation.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
				}
	...
	private void btnSelectVisas_Click(object sender, EventArgs e)
	{
		if (NoInvoice)
		{
			dtSearchResult = SearchFunctions.CancelledVisasReport(OperationID, VesselID, "1", IsFromServer: false);
			...
			foreach (DataRow row2 in dataTable.Rows)
			{
				...
				dataRow3["OperationServiceVisaCancellationID"] = -1;
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.MarineService.Transactions.frmOperationsServicesVisasCancellation));
		...
		base.Name = "frmOperationsServicesVisasCancellation";
