...
public class frmUnFinished : frmPosted
{
	private IContainer components = null;

	public frmUnFinished()
	{
		InitializeComponent();
		NoCol = "OperationNo";
	}
	...
	{
		dtsource = Operations.SelectByFinished(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["OperationDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BrokerFinishedDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BrokerFinishedDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الانهاء " : "Broker Finished Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BrokerFinishedDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomsBrokerName"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomsBrokerName"].Header).Caption = (GlobalVariables.IsArabic ? "اسم المستخلص" : "CustomsBroker Name");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomsBrokerName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CertificateNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Finished"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Finished"].Header).Caption = "";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Finished"].Width = (int)((double)((Control)(object)ULGData).Width * 0.05);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Finished"].AllowRowFiltering = (DefaultableBoolean)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Finished"].Header.CheckBoxVisibility = (HeaderCheckBoxVisibility)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Finished"].Header.CheckBoxAlignment = (HeaderCheckBoxAlignment)2;
	}

	public override void SelectFullRow()
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "Finished")
		{
	...
	public override void SaveData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["Finished"].Value.Equals(true))
			{
		...
		if (text != ",")
		{
			Operations.SetFinish(text, "0", GlobalVariables.UserID);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عمليات لفك الانهاء " : "There are No Operations to UnFinished");
		}
	}
	...
	public override void Search()
	{
		...
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				...
				{
					((UltraGridBase)ULGData).Rows[j].Cells["Finished"].Value = true;
				}
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.CustomsClearence.Approved.frmApprovingOperations));
