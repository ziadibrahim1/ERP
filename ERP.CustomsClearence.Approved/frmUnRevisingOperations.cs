...
public class frmUnRevisingOperations : frmPosted
{
	private IContainer components = null;

	public frmUnRevisingOperations()
	{
		InitializeComponent();
		NoCol = "OperationNo";
	}
	...
	{
		dtsource = Operations.SelectByRevised(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["RevisedDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["RevisedDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ المراجعة" : "Revise Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["RevisedDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		if (text != ",")
		{
			Operations.SetRevise("0", text, GlobalVariables.UserID);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عمليات لفك المراجعة " : "There are No Operations to UnRevised");
		}
	}
