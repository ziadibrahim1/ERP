...
public class frmSeaMenSearch : frmSearch
{
	private IContainer components = null;

	public frmSeaMenSearch()
	{
		InitializeComponent();
	}

	public frmSeaMenSearch(DataTable dt, string idColumnName)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم البحار " : "SeaMan No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManName"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManName"].Header).Caption = (GlobalVariables.IsArabic ? "اسم البحار" : "SeaMan");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManRank"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManRank"].Header).Caption = (GlobalVariables.IsArabic ? "الوظيفة" : "Rank");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SeaManRank"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
