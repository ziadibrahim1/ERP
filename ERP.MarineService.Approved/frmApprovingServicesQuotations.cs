...
public class frmApprovingServicesQuotations : frmPosted
{
	private IContainer components = null;

	public frmApprovingServicesQuotations()
	{
		InitializeComponent();
	...
	{
		dtsource = ServicesQuotations.SelectByApproved(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SubAccountType"].Header).Caption = (GlobalVariables.IsArabic ? "نوع الحساب" : "SubAccount Type");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SubAccountType"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["ServiceQuotationID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			ServicesQuotations.SetApprove("1", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عروض أسعار خدمات لإعتمادها " : "There are No Services Quotations to be Approved");
		}
	}
	...
	public override void ClickCellButton()
	{
		if (((UltraGridBase)ULGData).ActiveRow != null)
		{
			...
			((Control)(object)frmServicesQuotations2.lblTitle).Text = (GlobalVariables.IsArabic ? "عــرض ســعـر للخدمات" : "Services Quotations");
	...
	{
		DataTable dataTable = SearchFunctions.MSServicesQuotationsReport(DateTime.Now.AddYears(-10).ToString(GlobalVariables.DateShortFormate), "-1", 0, -1, 0);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.MarineService.Approved.frmApprovingServicesQuotations));
		...
		base.Name = "frmApprovingServicesQuotations";
