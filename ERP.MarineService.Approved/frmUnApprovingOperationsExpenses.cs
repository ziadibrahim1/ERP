...
public class frmUnApprovingOperationsExpenses : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingOperationsExpenses()
	{
		InitializeComponent();
		NoCol = "ExpenseNo";
	...
	{
		dtsource = OperationsExpenses.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpenseNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PaidDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PaidDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.075);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CompanyName"].Header).Caption = (GlobalVariables.IsArabic ? "الشركه" : "Company");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["OperationNo"].Header).Caption = (GlobalVariables.IsArabic ? "العملية" : "Operation No");
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ActuaUnitlPrice"].Header).Caption = (GlobalVariables.IsArabic ? "القيمة" : "Amount");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ActuaUnitlPrice"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["OperationExpenseID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			OperationsExpenses.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد مصروفات لفك إعتمادها " : "There are No Expenses to be UnApproved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmOperationExpenses frmOperationExpenses2 = new frmOperationExpenses(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["OperationExpenseID"].Value.ToString()));
			frmOperationExpenses2.Size = new Size(base.Width, base.Height);
			frmOperationExpenses2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmOperationExpenses2.lblTitle).Text = (GlobalVariables.IsArabic ? "مصروفات العملية" : "Operation Expenses");
	...
	{
		DataTable dataTable = SearchFunctions.OperationsExpensesReport(IsFromServer: false);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["OperationExpenseID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["OperationExpenseID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.MarineService.Approved.frmUnApprovingOperationsExpenses));
		...
		base.Name = "frmUnApprovingOperationsExpenses";
