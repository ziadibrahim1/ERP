...
public class frmUnApprovingItemsQuotations : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingItemsQuotations()
	{
		InitializeComponent();
	...
	{
		dtsource = ItemsQuotations.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemQuotationNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemQuotationNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Quotation No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemQuotationNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.125) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemQuotationDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemQuotationDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemQuotationDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.075);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["VesselName"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["VesselName"].Header).Caption = (GlobalVariables.IsArabic ? "اسم الباخرة" : "Vessel");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["VesselName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["ItemQuotationID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			ItemsQuotations.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عروض أسعار للأصناف لفك إعتمادها " : "There are No Items Quotations to be UnApproved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmItemsQuotations frmItemsQuotations2 = new frmItemsQuotations(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ItemQuotationID"].Value.ToString()));
			frmItemsQuotations2.Size = new Size(base.Width, base.Height);
			frmItemsQuotations2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmItemsQuotations2.lblTitle).Text = (GlobalVariables.IsArabic ? "عــرض ســعـر للأصناف" : "Items Quotations");
			frmItemsQuotations2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.MSItemsQuotationsSearchReport(DateTime.Now.AddYears(-10).ToString(GlobalVariables.DateShortFormate), "-1", 1, -1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["ItemQuotationID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["ItemQuotationID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.MarineService.Approved.frmUnApprovingItemsQuotations));
		...
		base.Name = "frmUnApprovingItemsQuotations";
