...
public class frmGeneralExpenses : frmHeaderDetails
{
	...
	public frmGeneralExpenses()
	{
		...
		InitializeComponent();
		TableName = "A_GeneralExpenses";
		IDCol = "GeneralExpenseID";
		NoCol = "GeneralExpenseNo";
		DateCol = "GeneralExpenseDate";
		((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? "قيد" : "No");
	}

	public frmGeneralExpenses(int ID)
		: this()
	{
		RowID = ID.ToString();
	}
	...
	public override void PrepareData()
	{
		...
		dtDetails = GeneralExpensesDetails.SelectByGeneralExpenseID("0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = GeneralExpenses.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((Control)(object)txtCode).Text = drMaster["GeneralExpenseNo"].ToString();
			dtpDate.Value = (DateTime)drMaster["GeneralExpenseDate"];
			((TextEditorControlBase)cboAccount).Value = drMaster["AccountID"];
			...
			((Control)(object)btnJV).Text = (GlobalVariables.IsArabic ? ("(" + drMaster["JvID"].ToString() + ")قيد") : ("No( " + drMaster["JvID"].ToString() + " )"));
			((Control)(object)btnJV).Visible = ((drMaster["JvID"] != DBNull.Value) ? true : false) && CanViewJV;
			ClosedPeriod = FiscalYear.ChkForClosingFsicalPeriod(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID, IsFromServer: false);
			dtDetails = GeneralExpensesDetails.SelectByGeneralExpenseID(drMaster["GeneralExpenseID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
	...
	{
		base.ClearControls();
		cboAccount.SelectedIndex = ((((DisposableObjectCollectionBase)cboAccount.Items).Count <= 0) ? (-1) : 0);
		cboSubAccount.SelectedIndex = -1;
		cboCostCenter.SelectedIndex = -1;
		((Control)(object)txtCode).Text = (Adding ? GeneralExpenses.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID) : "");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["GeneralExpenseDetailID"].DefaultCellValue = -1;
	...
	public override bool ValidateData()
	{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "برجاء إختيار الحساب" : "Please Select the safe ");
			((TextEditorControlBase)cboAccount).Focus();
			return false;
		}
		if (Main.CheckForValueByBranchIDAndFiscalYearID("A_GeneralExpenses", "GeneralExpenseNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["GeneralExpenseNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = GeneralExpenses.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		...
		if (UseSubAccounts && GlobalFunctions.GetOption("EnforceSubAccountsUse") && ((DisposableObjectCollectionBase)cboSubAccount.Items).Count > 0 && cboSubAccount.SelectedIndex == -1)
		{
			GlobalVariables.InformationMB.Show("برجاء اختيار حساب تحليلي", "Please choose Sub-Account");
			((Control)(object)cboSubAccount).Select();
			return false;
		}
		if (UseCostCenters && GlobalFunctions.GetOption("EnforceCostCentersUse") && (dtAccounts.Select("AccountID = " + ((TextEditorControlBase)cboAccount).Value.ToString())[0]["AccountTypeID"].ToString() == "3" || dtAccounts.Select("AccountID = " + ((TextEditorControlBase)cboAccount).Value.ToString())[0]["AccountTypeID"].ToString() == "4") && cboCostCenter.SelectedIndex == -1)
		{
			GlobalVariables.InformationMB.Show("برجاء اختيار مركز تكلفة", "Please choose Cost-Center");
			((Control)(object)cboCostCenter).Select();
			return false;
		}
	...
	public override void AddData()
	{
		...
		try
		{
			int num = GeneralExpenses.Insert_Update("-1", ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), (cboAccount.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboAccount).Value.ToString(), (cboSubAccount.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboSubAccount).Value.ToString(), (cboCostCenter.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboCostCenter).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, "Null", "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			ULGData.AfterCellUpdate -= new CellEventHandler(ULGData_AfterCellUpdate);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				((UltraGridBase)ULGData).Rows[i].Cells["GeneralExpenseDetailID"].Value = -1;
				((UltraGridBase)ULGData).Rows[i].Cells["GeneralExpenseID"].Value = num;
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
			}
			ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			GeneralExpensesDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
			GeneralExpenses.GenerateJvs(num.ToString(), GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = GeneralExpenses.Insert_Update(drMaster["GeneralExpenseID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboAccount).Value.ToString(), (cboSubAccount.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboSubAccount).Value.ToString(), (cboCostCenter.SelectedIndex == -1) ? "Null" : ((TextEditorControlBase)cboCostCenter).Value.ToString(), ((TextEditorControlBase)cboCurrency).Value.ToString(), ((Control)(object)txtExchangeRate).Text, ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, (drMaster["JVID"] == DBNull.Value) ? "Null" : drMaster["JVID"].ToString(), bool.Parse(drMaster["Approved"].ToString()) ? "1" : "0", bool.Parse(drMaster["Deleted"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			string text = ",";
			ULGData.AfterCellUpdate -= new CellEventHandler(ULGData_AfterCellUpdate);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				((UltraGridBase)ULGData).Rows[i].Cells["GeneralExpenseID"].Value = num;
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["GeneralExpenseDetailID"].Value.ToString() + ",";
			}
			ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			Main.DeleteForUpdate("A_GeneralExpensesDetails", "GeneralExpenseID", drMaster["GeneralExpenseID"].ToString(), "GeneralExpenseDetailID", text);
			GeneralExpensesDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
			GeneralExpenses.GenerateJvs(num.ToString(), GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
		}
	...
	public override void DeleteData()
	{
		...
		try
		{
			...
			GeneralExpenses.DeleteVirtual(drMaster["GeneralExpenseID"].ToString(), GlobalVariables.UserID);
			GeneralExpensesDetails.DeleteVirtualByGeneralExpenseID(drMaster["GeneralExpenseID"].ToString(), GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
		}
		catch
		{
	...
	public override void btnPrintClick()
	{
		if (RowID != "")
		{
			...
			{
				GlobalVariables.ReportDocument.Load(GlobalVariables.ReportsPath + (GlobalVariables.IsArabic ? "Rep_A_GeneralExpenses_A.rpt" : "Rep_A_GeneralExpenses_E.rpt"));
			}
			frmReporViwer frmReporViwer2 = new frmReporViwer();
			GlobalVariables.ReportDocument.SetParameterValue("@GeneralExpenseIDs", "," + RowID + ",");
	...
	{
		dtSearchResult = SearchFunctions.GeneralExpensesSearchReport(-1, 0);
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["GeneralExpenseID"].ToString();
			FillData();
		}
	}
	...
	private void dtpJVDate_ValueChanged(object sender, EventArgs e)
	{
		...
		if (Adding)
		{
			((Control)(object)txtCode).Text = GeneralExpenses.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Accounting.Transactions.frmGeneralExpenses));
