...
public class frmApprovingBankOutRequest : frmPosted
{
	private IContainer components = null;

	public frmApprovingBankOutRequest()
	{
		InitializeComponent();
	...
	{
		dtsource = BankOutRequests.SelectByApproved(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BankOutRequestNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BankOutRequestNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BankOutRequestNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BankOutRequestDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Approved"].Header).Caption = (GlobalVariables.IsArabic ? "إعتماد" : "Approved");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " EXEC SB_BankOutRequests_SetApprove  1," + (bool.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IsRefused"].Value.ToString()) ? "1" : "0") + "," + ((UltraGridBase)ULGData).Rows[i].Cells["BankOutRequestID"].Value.ToString() + " ; ";
			}
		}
		if (text != "")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات لإعتمادها " : "There are No Requests to be Approved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmBankOutRequests frmBankOutRequests2 = new frmBankOutRequests(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["BankOutRequestID"].Value.ToString()));
			frmBankOutRequests2.Size = new Size(base.Width, base.Height);
			frmBankOutRequests2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmBankOutRequests2.lblTitle).Text = (GlobalVariables.IsArabic ? "طلـب صـرف بنك" : "Bank Out Request");
			frmBankOutRequests2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.BanKOutRequestReport(0, -1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["BankOutRequestID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["BankOutRequestID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Approved.frmApprovingBankOutRequest));
		...
		base.Name = "frmApprovingBankOutRequest";
