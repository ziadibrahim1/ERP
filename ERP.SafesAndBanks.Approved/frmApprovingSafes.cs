...
public class frmApprovingSafes : frmPosted
{
	...
	public override void FillGrid()
	{
		if (rbSafeIn.Checked)
		{
			dtsource = SafeIn.SelectByApproved(GlobalVariables.BranchIDs, GlobalVariables.SafeIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
		}
		else
		{
			dtsource = SafeOut.SelectByApproved(GlobalVariables.BranchIDs, GlobalVariables.SafeIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
		}
	...
	public override void SaveData()
	{
		...
		if (text != ",")
		{
			if (rbSafeIn.Checked)
			{
				SafeIn.SetApprove("1", text);
			}
			else
			{
				SafeOut.SetApprove("1", text);
			}
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد قيود لإعتمادها " : "There are No JV's to Approve");
		}
	}
	...
	public override void Search()
	{
		if (rbSafeIn.Checked)
		{
			DataTable dataTable = SearchFunctions.SafeInReport(0, 0);
		...
		DataTable dataTable2 = SearchFunctions.SafeOutReport(0, 0);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Approved.frmApprovingSafes));
