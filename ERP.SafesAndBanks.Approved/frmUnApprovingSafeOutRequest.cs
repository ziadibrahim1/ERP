...
public class frmUnApprovingSafeOutRequest : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingSafeOutRequest()
	{
		InitializeComponent();
	...
	{
		dtsource = SafeOutRequests.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeOutRequestNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeOutRequestNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeOutRequestNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeOutRequestDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + " EXEC SB_SafeOutRequests_SetApprove  0,0," + ((UltraGridBase)ULGData).Rows[i].Cells["SafeOutRequestID"].Value.ToString() + " ; ";
			}
		}
		if (text != "")
		{
	...
	public override void ClickCellButton()
	{
		...
		{
			frmSafeOutRequest frmSafeOutRequest2 = new frmSafeOutRequest(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["SafeOutRequestID"].Value.ToString()));
			frmSafeOutRequest2.Size = new Size(base.Width, base.Height);
			frmSafeOutRequest2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmSafeOutRequest2.lblTitle).Text = (GlobalVariables.IsArabic ? "طلـب صـرف خزينة" : "SafeOut Request");
	...
	{
		DataTable dataTable = SearchFunctions.SafeOutRequestsReport(0, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["SafeOutRequestID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["SafeOutRequestID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Approved.frmUnApprovingSafeOutRequest));
		...
		base.Name = "frmUnApprovingSafeOutRequest";
