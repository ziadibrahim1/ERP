...
public class frmUnApprovingBanks : frmPosted
{
	...
	public override void FillGrid()
	{
		if (rbBankIn.Checked)
		{
			dtsource = BankIn.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
		}
		else
		{
			dtsource = BankOut.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
		}
	...
	public override void SaveData()
	{
		...
		if (text != ",")
		{
			if (rbBankIn.Checked)
			{
				BankIn.SetApprove("0", text);
			}
			else
			{
				BankOut.SetApprove("0", text);
			}
			FillGrid();
		}
		else
	...
	public override void Search()
	{
		if (rbBankIn.Checked)
		{
			DataTable dataTable = SearchFunctions.BankInReport(1, -1, -1, -1, -1, 0);
		...
		DataTable dataTable2 = SearchFunctions.BanKOutReport(1, -1, -1, -1, -1, 0);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Approved.frmUnApprovingBanks));
