...
public class frmUnApprovingCustodys : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingCustodys()
	{
		InitializeComponent();
		NoCol = "CustodyNo";
	...
	{
		dtsource = Custody.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SubAccountName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.3);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyDate"].Header).Caption = (GlobalVariables.IsArabic ? "التاريخ" : "Date");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["CustodyID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			Custody.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عهد لفك إعتمادها " : "There are No Custody To UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmCustody frmCustody2 = new frmCustody(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["CustodyID"].Value.ToString()));
			frmCustody2.Size = new Size(base.Width, base.Height);
	...
	{
		DataTable dataTable = SearchFunctions.CustodyReport(1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["CustodyID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["CustodyID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.SafesAndBanks.Approved.frmUnApprovingCustodys));
		...
		base.Name = "frmUnApprovingCustodys";
