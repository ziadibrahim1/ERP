...
public class frmSLInvoices : frmHeaderManyDetails
{
	...
	public UltraButton btnSLOrderSearch;

	private UltraLabel lblSLOrder;

	private UltraComboEditor cboSLOrder;

	public UltraTree TreeMaterialIssueVoucherNo;

	private UltraCheckEditor chkWithoutMIV;

	public frmSLInvoices()
	{
	...
	public override void PrepareData()
	{
		...
		GlobalFunctions.FillCombo(cboSLOrder, dtSLOrders, "SLOrderID", "SLOrderNo");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((TextEditorControlBase)cboQuotation).ValueChanged += cboQuotation_ValueChanged;
			((TextEditorControlBase)cboSLOrder).ValueChanged -= cboSLOrder_ValueChanged;
			((TextEditorControlBase)cboSLOrder).Value = drMaster["SLOrderID"];
			((TextEditorControlBase)cboSLOrder).ValueChanged += cboSLOrder_ValueChanged;
			...
			if (rbIsMaterialIssueVoucher.Checked)
			{
				...
				TreeFunctions.FillTreeOneLevel(TreeMaterialIssueVoucherNo, dtMaterialIssueVouchers, "MaterialIssueVoucherID", "MaterialIssueVoucherNo");
				((UltraToggleEditorBase)chkAll).Checked = true;
				TreeMaterialIssueVoucherNo.AfterCheck -= new AfterNodeChangedEventHandler(TreeMaterialIssueVoucherNo_AfterCheck);
				TreeFunctions.SetAllTreeNodesCheckState(((UltraToggleEditorBase)chkAll).Checked, TreeMaterialIssueVoucherNo);
				TreeMaterialIssueVoucherNo.AfterCheck += new AfterNodeChangedEventHandler(TreeMaterialIssueVoucherNo_AfterCheck);
				((UltraToggleEditorBase)chkAll).CheckedChanged += chkAll_CheckedChanged;
			}
			((UltraGridBase)ULGData).DataSource = dtDetails;
	...
	public override void SetControls(bool NavMode)
	{
		...
		((EditorButtonControlBase)cboSLOrder).ReadOnly = NavMode || Updating;
		...
		((Control)(object)btnSLOrderSearch).Visible = Adding && rbIsSLOrder.Checked;
		((Control)(object)btnJV).Visible = NavMode && drMaster != null && CanViewJV;
		((Control)(object)btnStockBalance).Visible = Adding || Updating;
		((Control)(object)btnClientSearch).Visible = (rbIsDirectInvoice.Checked ? (!NavMode) : (Adding && (rbIsMaterialIssueVoucher.Checked || rbIsQuotation.Checked)));
		...
		((Control)(object)TreeMaterialIssueVoucherNo).Enabled = Adding;
		...
		if (Adding)
		{
			...
			DataView dataView4 = new DataView(dtSLOrders);
			dataView4.RowFilter = " Approved=1 And HasSLInvoice = 0 and BranchID= " + GlobalVariables.CurrentBranchID;
			GlobalFunctions.FillCombo(cboSLOrder, dataView4.ToTable(), "SLOrderID", "SLOrderNo");
		}
		else
		{
			GlobalFunctions.FillCombo(cboQuotation, dtSLQuotation, "QuotationID", "QuotationNo");
	...
	public override void ClearControls()
	{
		...
		rbIsQuotation.CheckedChanged += RadioButtons_CheckedChanged;
		rbIsSLOrder.CheckedChanged -= RadioButtons_CheckedChanged;
		rbIsSLOrder.Checked = false;
		rbIsSLOrder.CheckedChanged += RadioButtons_CheckedChanged;
	...
	public override bool ValidateData()
	{
		...
		if (rbIsMaterialIssueVoucher.Checked && TreeFunctions.GetTreeOneLevelCheckedItemCount(TreeMaterialIssueVoucherNo) == 0)
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "برجاء إختيار إذن الصرف" : "Please Select Material Issue Voucher");
			return false;
		}
		...
		if (rbIsSLOrder.Checked && cboSLOrder.SelectedIndex == -1)
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "برجاء إختيار أمر البيع" : "Please Select Sales order No");
			((TextEditorControlBase)cboSLOrder).Focus();
			cboSLOrder.DropDown();
			return false;
		}
	...
	public override void AddData()
	{
		...
		try
		{
			...
			string text2 = "";
			if (GlobalFunctions.GetOption("CreateMaterialIssueVoucherFromSalesInvoiceEnforceCreation") && !((UltraToggleEditorBase)chkWithoutMIV).Checked && ((!rbIsMaterialIssueVoucher.Checked && !rbIsSLOrder.Checked) || (rbIsSLOrder.Checked = cboSLOrder.SelectedIndex > -1 && !bool.Parse(dtSLOrders.Select(" SLOrderID= " + ((TextEditorControlBase)cboSLOrder).Value.ToString())[0]["IsMIVFirst"].ToString()))))
			{
	...
	public override void DeleteData()
	{
		...
		try
		{
			...
			{
				Main.ExecuteNonQuery(" Update SC_MaterialIssueVouchers set SLInvoiceID=null Where SLOrderID =" + ((TextEditorControlBase)cboSLOrder).Value.ToString());
				Main.ExecuteNonQuery(" Update SL_SLOrders Set HasSLInvoice=0 where SLOrderID= " + ((TextEditorControlBase)cboSLOrder).Value.ToString());
			}
			if (rbIsMaterialIssueVoucher.Checked)
			{
	...
	public override void btnRefreshDataClick()
	{
		...
		object value11 = ((TextEditorControlBase)cboSLOrder).Value;
		((TextEditorControlBase)cboSLOrder).ValueChanged -= cboSLOrder_ValueChanged;
		UsingColors = GlobalFunctions.GetOption("UsingItemsColors");
		UsingSizes = GlobalFunctions.GetOption("UsingItemsSizes");
		if (UsingColors)
		...
		dtItems = Items.FillCombo("-1", "-1", "-1", "1", "-1", "-1", "-1", "-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		dtItemsUnitsBarCode = ItemsUnits.FillCombo("-1", "-1", "-1", "1", "-1", "-1", "-1", "-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		DataView dataView = new DataView(dtSLOrders);
		dataView.RowFilter = " Approved=1 And HasSLInvoice = 0 and BranchID= " + GlobalVariables.CurrentBranchID;
		GlobalFunctions.FillCombo(cboSLOrder, dataView.ToTable(), "SLOrderID", "SLOrderNo");
		dtSLQuotation = BusinessLayer.Sales.Quotations.FillCombo(GlobalVariables.BranchIDs, "-1");
		dtProductionRequest = ProductionRequests.FillCombo(GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
		GlobalFunctions.FillCombo(cboRequestNo, dtProductionRequest, "ProductionRequestID", "ProductionRequestNo");
		dtPaymentMethod = PaymentMethods.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		...
		{
			DataView dataView4 = new DataView(dtClients);
			...
			GlobalFunctions.FillCombo(cboClient, dataView4.ToTable(), "SubAccountID", "SubAccountName");
		}
		else
		{
			GlobalFunctions.FillCombo(cboClient, dtClients, "SubAccountID", "SubAccountName");
		}
		((TextEditorControlBase)cboSLOrder).Value = value11;
	...
	public override void SelectFullRow(object sender, EventArgs e)
	{
		...
		else if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "UnitPrice" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "TaxID" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "TotalPrice" && rbIsMaterialIssueVoucher.Checked)
		{
			((GridItemBase)((UltraGridBase)ULGData).ActiveRow).Selected = true;
		}
		...
		if (cboSLOrder.SelectedIndex > -1 && bool.Parse(dtSLOrders.Select(" SLOrderID= " + ((TextEditorControlBase)cboSLOrder).Value.ToString())[0]["IsMIVFirst"].ToString()) && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "UnitPrice" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "Value" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "TaxID")
		{
			((GridItemBase)((UltraGridBase)ULGData).ActiveRow).Selected = true;
		}
	}
	...
	private void dtpDate_ValueChanged(object sender, EventArgs e)
	{
		...
		if (cboClient.SelectedIndex > -1 && dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["PriceTypeID"] != DBNull.Value)
		{
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				...
				if (array != null && array.Length != 0)
				{
					((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value = decimal.Parse(array[0]["Price"].ToString()) - decimal.Parse(array[0]["Price"].ToString()) * decimal.Parse(array[0]["DiscountPercentage"].ToString()) / 100m;
					((UltraGridBase)ULGData).Rows[i].Cells["TotalPrice"].Value = decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value.ToString()) * decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value.ToString());
				}
				else
				{
	...
	private void cboQuotation_ValueChanged(object sender, EventArgs e)
	{
		...
		if (cboQuotation.SelectedIndex > -1)
		{
			...
			if (((TextEditorControlBase)cboClient).Value != null)
			{
				((Control)(object)txtBranchBalance).Text = decimal.Parse(Math.Round(SubAccounts.Balance(((TextEditorControlBase)cboClient).Value.ToString(), "-1", Adding ? ("," + GlobalVariables.CurrentBranchID + ",") : ("," + drMaster["BranchID"].ToString() + ","), GlobalVariables.LocalCurrencyID.ToString(), IsFromServer: false, 0), 2).ToString()).ToString(GlobalVariables.txtDecimalFormate);
			}
	...
	private void RadioButtons_CheckedChanged(object sender, EventArgs e)
	{
		...
		UltraLabel obj2 = lblSLOrder;
		visible = (((Control)(object)cboSLOrder).Visible = rbIsSLOrder.Checked);
		((Control)(object)obj2).Visible = visible;
		((Control)(object)btnSLOrderSearch).Visible = rbIsSLOrder.Checked && Adding;
		UltraLabel obj3 = lblRequestNo;
		UltraComboEditor obj4 = cboRequestNo;
		bool flag3 = (((Control)(object)btnRequestSearch).Visible = rbIsProductionRequest.Checked);
		visible = (((Control)(object)obj4).Visible = flag3);
		((Control)(object)obj3).Visible = visible;
		UltraCheckEditor obj5 = chkAll;
		visible = (((Control)(object)TreeMaterialIssueVoucherNo).Visible = rbIsMaterialIssueVoucher.Checked);
		((Control)(object)obj5).Visible = visible;
		UltraCheckEditor obj6 = chkStore;
		visible = (((Control)(object)cboStore).Visible = !rbIsMaterialIssueVoucher.Checked);
	...
	private void cboClient_ValueChanged(object sender, EventArgs e)
	{
		...
		if (cboClient.SelectedIndex > -1)
		{
			...
			if (rbIsQuotation.Checked)
			{
				((TextEditorControlBase)cboQuotation).ValueChanged -= cboQuotation_ValueChanged;
				((TextEditorControlBase)cboCurrency).ValueChanged -= cboCurrency_ValueChanged;
				DataView dataView2 = new DataView(dtSLQuotation);
				dataView2.RowFilter = " SubAccountID= " + ((TextEditorControlBase)cboClient).Value.ToString() + " And  Closed= 0 And  BranchID= " + GlobalVariables.CurrentBranchID;
			...
			else if (rbIsProductionRequest.Checked)
			{
				((TextEditorControlBase)cboRequestNo).ValueChanged -= cboRequestNo_ValueChanged;
				((TextEditorControlBase)cboCurrency).ValueChanged -= cboCurrency_ValueChanged;
				DataView dataView3 = new DataView(dtProductionRequest);
				dataView3.RowFilter = " SubAccountID= " + ((TextEditorControlBase)cboClient).Value.ToString() + " And  BranchID= " + GlobalVariables.CurrentBranchID;
				GlobalFunctions.FillCombo(cboRequestNo, dataView3.ToTable(), "ProductionRequestID", "ProductionRequestNo");
				dtDetails.Rows.Clear();
				cboCurrency.SelectedIndex = -1;
				((Control)(object)txtExchangeRate).Text = "";
				((TextEditorControlBase)cboRequestNo).ValueChanged += cboRequestNo_ValueChanged;
			...
			{
				TreeMaterialIssueVoucherNo.DataSource = null;
				((DataTable)((UltraGridBase)ULGData).DataSource).Rows.Clear();
	...
	private void cboSLOrder_ValueChanged(object sender, EventArgs e)
	{
		//IL_0293: Unknown result type (might be due to invalid IL or missing references)
		//IL_029d: Expected O, but got Unknown
		//IL_07d2: Unknown result type (might be due to invalid IL or missing references)
		//IL_07dc: Expected O, but got Unknown
		if (cboSLOrder.SelectedIndex > -1)
		{
			((TextEditorControlBase)cboSLOrder).ValueChanged -= cboSLOrder_ValueChanged;
			((TextEditorControlBase)cboCurrency).ValueChanged -= cboCurrency_ValueChanged;
			((TextEditorControlBase)cboClient).ValueChanged -= cboClient_ValueChanged;
			object obj = dtSLOrders.Select(" SLOrderID= " + ((TextEditorControlBase)cboSLOrder).Value.ToString())[0]["SubAccountID"];
			if (((DataTable)cboClient.DataSource).Select("SubAccountID = " + obj.ToString()).Length != 0)
			{
				((TextEditorControlBase)cboClient).Value = obj;
			}
			else
			{
				((TextEditorControlBase)cboClient).Value = null;
			}
			((TextEditorControlBase)cboCurrency).Value = dtSLOrders.Select(" SLOrderID= " + ((TextEditorControlBase)cboSLOrder).Value.ToString())[0]["CurrencyID"];
			((Control)(object)txtExchangeRate).Text = decimal.Parse(dtSLOrders.Select(" SLOrderID= " + ((TextEditorControlBase)cboSLOrder).Value.ToString())[0]["ExchangeRate"].ToString()).ToString(GlobalVariables.txtDecimalFormate);
			if (((TextEditorControlBase)cboClient).Value != null)
			{
				((TextEditorControlBase)cboPaymentMethod).Value = dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["DefaultPaymentMethodID"];
				((Control)(object)txtDiscBeforeTaxRatio).Text = decimal.Parse(dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["DiscountPercentage"].ToString()).ToString(GlobalVariables.txtDecimalFormate);
			}
			dtSLOrderDetails = SLInvoicesDetails.FillBySLOrderID(((TextEditorControlBase)cboSLOrder).Value.ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DisplayLayout.Override.AllowAddNew = (AllowAddNew)1;
			dtDetails.Rows.Clear();
			for (int i = 0; i < dtSLOrderDetails.Rows.Count; i++)
			{
				((UltraGridBase)ULGData).DisplayLayout.Bands[0].AddNew();
				ULGData.AfterCellUpdate -= new CellEventHandler(ULGData_AfterCellUpdate);
				UltraGridCell obj2 = ((UltraGridBase)ULGData).ActiveRow.Cells["ItemID"];
				object value = (((UltraGridBase)ULGData).ActiveRow.Cells["ItemBarCode"].Value = dtSLOrderDetails.Rows[i]["ItemID"].ToString());
				obj2.Value = value;
				DataRow dataRow = dtItems.Select("ItemID=" + dtSLOrderDetails.Rows[i]["ItemID"].ToString())[0];
				((UltraGridBase)ULGData).ActiveRow.Cells["ColorID"].Value = dtSLOrderDetails.Rows[i]["ColorID"];
				((UltraGridBase)ULGData).ActiveRow.Cells["ItemSizeID"].Value = dtSLOrderDetails.Rows[i]["ItemSizeID"];
				((UltraGridBase)ULGData).ActiveRow.Cells["BatchID"].Value = dtSLOrderDetails.Rows[i]["BatchID"];
				((UltraGridBase)ULGData).ActiveRow.Cells["UnitID"].Value = dtSLOrderDetails.Rows[i]["UnitID"];
				string value2 = (dataRow["DefaultStoreID"].Equals(DBNull.Value) ? "-1" : dataRow["DefaultStoreID"].ToString());
				((UltraGridBase)ULGData).ActiveRow.Cells["StoreID"].Value = value2;
				((UltraGridBase)ULGData).ActiveRow.Cells["Qty"].Value = dtSLOrderDetails.Rows[i]["Qty"].ToString();
				((UltraGridBase)ULGData).ActiveRow.Cells["UnitPrice"].Value = dtSLOrderDetails.Rows[i]["UnitPrice"].ToString();
				((UltraGridBase)ULGData).ActiveRow.Cells["IssuedQty"].Value = dtSLOrderDetails.Rows[i]["IssueQty"].ToString();
				((UltraGridBase)ULGData).ActiveRow.Cells["TotalPrice"].Value = decimal.Parse(dtSLOrderDetails.Rows[i]["UnitPrice"].ToString()) * decimal.Parse(dtSLOrderDetails.Rows[i]["Qty"].ToString());
				((UltraGridBase)ULGData).ActiveRow.Cells["TaxID"].Value = dataRow["TaxID"];
				CalculateRow(((UltraGridBase)ULGData).ActiveRow);
				if (dtSLOrderDetails.Rows[i]["UnitID"] != DBNull.Value)
				{
					ValueList unitsValueList = getUnitsValueList(int.Parse(dtUnits.Select(" UnitID =" + dtSLOrderDetails.Rows[i]["UnitID"].ToString())[0]["UnitTypeID"].ToString()));
					((UltraGridBase)ULGData).ActiveRow.Cells["UnitID"].ValueList = (IValueList)(object)unitsValueList;
				}
				if (UsingBatchNoAndValidityPeriod)
				{
					ValueList batchsValueList = getBatchsValueList(int.Parse(dtSLOrderDetails.Rows[i]["ItemID"].ToString()));
					((UltraGridBase)ULGData).ActiveRow.Cells["BatchID"].ValueList = (IValueList)(object)batchsValueList;
				}
				if (UsingColors && dataRow["ItemColorCategoryID"] != DBNull.Value)
				{
	...
	private void btnSLOrderSearch_Click(object sender, EventArgs e)
	{
		int num = SearchFunctions.SLOrdersSearch("," + GlobalVariables.CurrentBranchID + ",", 1, 0, 0, 0);
		if (num != 0)
		{
			((TextEditorControlBase)cboSLOrder).Value = num;
		}
	}

	private void cboSLOrder_KeyDown(object sender, KeyEventArgs e)
	{
		if ((Adding || Updating) && e.KeyValue == 119)
		{
			int num = SearchFunctions.SLOrdersSearch("," + GlobalVariables.CurrentBranchID + ",", 1, 0, 0, 1);
			if (num != 0)
			{
				((TextEditorControlBase)cboSLOrder).Value = num;
			}
	...
	private void btnGetFromXL_Click(object sender, EventArgs e)
	{
		...
		for (int j = 0; j < dataTable.Rows.Count; j++)
		{
			...
			if (dataTable.Columns.Contains(GlobalVariables.IsArabic ? "المخزن" : "Store") && dtStores.Select("StoreName = '" + dataTable.Rows[j][GlobalVariables.IsArabic ? "المخزن" : "Store"].ToString() + "'").Length != 0)
			{
	...
	private void TreeMaterialIssueVoucherNo_AfterCheck(object sender, NodeEventArgs e)
	{
		...
		((UltraToggleEditorBase)chkAll).CheckedChanged -= chkAll_CheckedChanged;
		((UltraToggleEditorBase)chkAll).Checked = TreeFunctions.GetTreeOneLevelCheckedItemCount(TreeMaterialIssueVoucherNo) == ((DisposableObjectCollectionBase)TreeMaterialIssueVoucherNo.Nodes).Count && ((DisposableObjectCollectionBase)TreeMaterialIssueVoucherNo.Nodes).Count > 0;
		((UltraToggleEditorBase)chkAll).CheckedChanged += chkAll_CheckedChanged;
		...
		if (materialIssueVouchersIds != "")
		{
			...
			if (dtItemPrices != null && dtItemPrices.Rows.Count > 0 && cboClient.SelectedIndex > -1)
			{
				...
				for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
				{
					((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value = decimal.Parse(dtItemPrices.Select(" ItemID= " + ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value.ToString())[0]["Price"].ToString()) - decimal.Parse(dtItemPrices.Select(" ItemID= " + ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value.ToString())[0]["Price"].ToString()) * decimal.Parse(dtItemPrices.Select(" ItemID= " + ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value.ToString())[0]["DiscountPercentage"].ToString()) / 100m;
					((UltraGridBase)ULGData).Rows[i].Cells["TotalPrice"].Value = decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value.ToString()) * decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value.ToString());
					CalculateRow(((UltraGridBase)ULGData).Rows[i]);
				}
				CalculateGoss();
	...
	private string GetMaterialIssueVouchersIds()
	{
		string text = "";
		for (int i = 0; i < ((DisposableObjectCollectionBase)TreeMaterialIssueVoucherNo.Nodes).Count; i++)
		{
			if (TreeMaterialIssueVoucherNo.Nodes[i].CheckedState == CheckState.Checked)
			{
				text = ((!(text == "") || Convert.ToBoolean(((SubObjectBase)TreeMaterialIssueVoucherNo.Nodes[i]).Tag)) ? (text + "," + ((KeyedSubObjectBase)TreeMaterialIssueVoucherNo.Nodes[i]).Key) : (text + ((KeyedSubObjectBase)TreeMaterialIssueVoucherNo.Nodes[i]).Key));
			}
		}
		return text;
	}
	...
	private void cboRequestNo_ValueChanged(object sender, EventArgs e)
	{
		...
		{
			((TextEditorControlBase)cboClient).ValueChanged -= cboClient_ValueChanged;
			((TextEditorControlBase)cboClient).Value = dtProductionRequest.Select(" ProductionRequestID= " + ((TextEditorControlBase)cboRequestNo).Value.ToString())[0]["SubAccountID"];
			if (cboClient.SelectedIndex > -1 && dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["PriceTypeID"] != DBNull.Value)
			{
				dtItemPrices = ItemsPrices.GetPriceWithItemDiscount(dtClients.Select(" SubAccountID= " + ((TextEditorControlBase)cboClient).Value)[0]["PriceTypeID"].ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), GlobalVariables.CurrentBranchID, IsFromServer: false);
			}
	...
	private void InitializeComponent()
	{
		...
		//IL_26fd: Expected O, but got Unknown
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sales.Transactions.frmSLInvoices));
		...
		this.btnSLOrderSearch = new UltraButton();
		this.lblSLOrder = new UltraLabel();
		this.cboSLOrder = new UltraComboEditor();
		this.TreeMaterialIssueVoucherNo = new UltraTree();
		...
		((System.ComponentModel.ISupportInitialize)this.cboSLOrder).BeginInit();
		((System.ComponentModel.ISupportInitialize)this.TreeMaterialIssueVoucherNo).BeginInit();
		...
		this.rbIsProductionRequest.CheckedChanged += new System.EventHandler(RadioButtons_CheckedChanged);
		...
		((ControlBase)this.btnSLOrderSearch).Appearance = (AppearanceBase)(object)val26;
		resources.ApplyResources(this.btnSLOrderSearch, "btnSLOrderSearch");
		((System.Windows.Forms.Control)(object)this.btnSLOrderSearch).Name = "btnSLOrderSearch";
		((System.Windows.Forms.Control)(object)this.btnSLOrderSearch).Click += new System.EventHandler(btnSLOrderSearch_Click);
		this.lblSLOrder.AutoEllipsis = false;
		resources.ApplyResources(this.lblSLOrder, "lblSLOrder");
		((System.Windows.Forms.Control)(object)this.lblSLOrder).Name = "lblSLOrder";
		((ControlBase)this.lblSLOrder).WrapText = false;
		((TextEditorControlBase)this.cboSLOrder).AlwaysInEditMode = true;
		this.cboSLOrder.AutoCompleteMode = (AutoCompleteMode)4;
		resources.ApplyResources(this.cboSLOrder, "cboSLOrder");
		((System.Windows.Forms.Control)(object)this.cboSLOrder).Name = "cboSLOrder";
		((TextEditorControlBase)this.cboSLOrder).ValueChanged += new System.EventHandler(cboSLOrder_ValueChanged);
		((System.Windows.Forms.Control)(object)this.cboSLOrder).KeyDown += new System.Windows.Forms.KeyEventHandler(cboSLOrder_KeyDown);
		((AppearanceBase)val27).FontData.BoldAsString = resources.GetString("resource.BoldAsString2");
		((AppearanceBase)val27).FontData.SizeInPoints = (float)resources.GetObject("resource.SizeInPoints1");
		((AppearanceBase)val27).ForeColor = System.Drawing.Color.Navy;
		this.TreeMaterialIssueVoucherNo.Appearance = (AppearanceBase)(object)val27;
		resources.ApplyResources(this.TreeMaterialIssueVoucherNo, "TreeMaterialIssueVoucherNo");
		this.TreeMaterialIssueVoucherNo.FullRowSelect = true;
		((System.Windows.Forms.Control)(object)this.TreeMaterialIssueVoucherNo).Name = "TreeMaterialIssueVoucherNo";
		val28.NodeStyle = (NodeStyle)1;
		this.TreeMaterialIssueVoucherNo.Override = val28;
		((UltraControlBase)this.TreeMaterialIssueVoucherNo).UseAppStyling = false;
		this.TreeMaterialIssueVoucherNo.AfterCheck += new AfterNodeChangedEventHandler(TreeMaterialIssueVoucherNo_AfterCheck);
		...
		base.Controls.Add((System.Windows.Forms.Control)(object)this.TreeMaterialIssueVoucherNo);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.btnSLOrderSearch);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.lblSLOrder);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.cboSLOrder);
		...
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.cboSLOrder, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.lblSLOrder, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.btnSLOrderSearch, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.TreeMaterialIssueVoucherNo, 0);
		...
		((System.ComponentModel.ISupportInitialize)this.cboSLOrder).EndInit();
		((System.ComponentModel.ISupportInitialize)this.TreeMaterialIssueVoucherNo).EndInit();
