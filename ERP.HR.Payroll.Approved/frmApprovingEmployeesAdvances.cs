...
public class frmApprovingEmployeesAdvances : frmPosted
{
	private IContainer components = null;

	public frmApprovingEmployeesAdvances()
	{
		InitializeComponent();
		fromServer = true;
	...
	{
		dtsource = EmployeesAdvances.SelectByApproved("0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["EmployeeAdvanceID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			EmployeesAdvances.SetApprove("1", text, IsFromServer: true);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات سلفة لإعتمادها " : "There are No Advance Request to Approve");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmEmployeesAdvances frmEmployeesAdvances2 = new frmEmployeesAdvances(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["EmployeeAdvanceID"].Value.ToString()));
			frmEmployeesAdvances2.Size = new Size(base.Width, base.Height);
			frmEmployeesAdvances2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmEmployeesAdvances2.lblTitle).Text = (GlobalVariables.IsArabic ? "سلف الموظفين" : "Employees Advances");
			frmEmployeesAdvances2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.EmployeesAdvancesReport(0, 0, IsFromServer: true);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["EmployeeAdvanceID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["EmployeeAdvanceID"].Value.ToString())
				{
