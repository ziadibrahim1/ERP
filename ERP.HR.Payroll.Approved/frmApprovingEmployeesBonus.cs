...
public class frmApprovingEmployeesBonus : frmPosted
{
	private IContainer components = null;

	public frmApprovingEmployeesBonus()
	{
		InitializeComponent();
		fromServer = true;
	...
	{
		dtsource = EmployeesBonus.SelectByApproved("0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void SaveData()
	{
		...
		if (text != ",")
		{
			EmployeesBonus.SetApprove("1", text, IsFromServer: true);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات مكافأت لإعتمادها " : "There are No Bonus to Approve");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmEmployeesBonus frmEmployeesBonus2 = new frmEmployeesBonus(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["EmployeeBonusID"].Value.ToString()));
			frmEmployeesBonus2.Size = new Size(base.Width, base.Height);
			frmEmployeesBonus2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmEmployeesBonus2.lblTitle).Text = (GlobalVariables.IsArabic ? "مكافأت الموظفين" : "Employees Bonus");
			frmEmployeesBonus2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.EmployeesBonusReport(0, 0, IsFromServer: true);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["EmployeeBonusID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["EmployeeBonusID"].Value.ToString())
				{
