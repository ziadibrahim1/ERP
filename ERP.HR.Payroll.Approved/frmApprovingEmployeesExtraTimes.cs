...
public class frmApprovingEmployeesExtraTimes : frmPosted
{
	private IContainer components = null;

	public frmApprovingEmployeesExtraTimes()
	{
		InitializeComponent();
		fromServer = true;
	...
	{
		dtsource = EmployeesExtraTimes.SelectByApproved("0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["EmployeeExtraTimeID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			EmployeesExtraTimes.SetApprove("1", text, IsFromServer: true);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد طلبات إضافى لإعتمادها " : "There are No Overtime to Approve");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmEmployeesExtraTimes frmEmployeesExtraTimes2 = new frmEmployeesExtraTimes(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["EmployeeExtraTimeID"].Value.ToString()));
			frmEmployeesExtraTimes2.Size = new Size(base.Width, base.Height);
			frmEmployeesExtraTimes2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmEmployeesExtraTimes2.lblTitle).Text = (GlobalVariables.IsArabic ? "اضافى الموظفين" : "Employees Over Times");
			frmEmployeesExtraTimes2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.EmployeesExtraTimesReport(0, 0, IsFromServer: true);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["EmployeeExtraTimeID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["EmployeeExtraTimeID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.HR.Payroll.Approved.frmApprovingEmployeesAdvances));
		...
		base.Name = "frmApprovingEmployeesAdvances";
