...
public class frmRepeatedJVSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmRepeatedJVSearchReport()
	{
		InitializeComponent();
	}

	public frmRepeatedJVSearchReport(DataTable dt, string idColumnName, int DeletedValue)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["JVName"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["JVName"].Header).Caption = (GlobalVariables.IsArabic ? "اسم القيد " : "JV Name");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["JVName"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
