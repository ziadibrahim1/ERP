...
public class frmSpecialOrdersReturns : frmHeaderDetails
{
	...
	private ValueList vlAccessoriesStores = new ValueList();
	...
	private UltraComboEditor cboSpecialOrderNo;

	public frmSpecialOrdersReturns()
	{
		...
		InitializeComponent();
		TableName = "POS_SpecialOrdersReturns";
		IDCol = "ReturnID";
		NoCol = "ReturnNo";
		DateCol = "ReturnDate";
	}

	public frmSpecialOrdersReturns(int ID)
		: this()
	{
		RowID = ID.ToString();
	}
	...
	public override void PrepareData()
	{
		...
		vlAccessoriesStores.ValueListItems.Clear();
		for (int k = 0; k < dtStores.Rows.Count; k++)
		{
			...
			vlAccessoriesStores.ValueListItems.Add(dtStores.Rows[k]["StoreID"], dtStores.Rows[k]["StoreName"].ToString());
		}
		...
		for (int l = 0; l < dtItems.Rows.Count; l++)
		{
			...
			vlAccessoriesItems.ValueListItems.Add(dtItems.Rows[l]["ItemID"], dtItems.Rows[l]["Name"].ToString());
		}
		dtUnits = Units.FillCombo("-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		vlAccessoriesUnits.ValueListItems.Clear();
		for (int m = 0; m < dtUnits.Rows.Count; m++)
		{
			vlAccessoriesUnits.ValueListItems.Add(dtUnits.Rows[m]["UnitID"], dtUnits.Rows[m]["UnitName"].ToString());
		}
		dtSpecialOrders = SpecialOrders.FillCombo(GlobalVariables.BranchIDs, "1", (GlobalFunctions.GetDefault("ClientReturnPeriod") == "") ? "0" : GlobalFunctions.GetDefault("ClientReturnPeriod"));
		GlobalFunctions.FillCombo(cboSpecialOrderNo, dtSpecialOrders, "SpecialOrderID", "SpecialOrderNo");
		...
		dtDetails = SpecialOrdersReturnsDetails.SelectByReturnID("0", GlobalVariables.IsArabic ? "1" : "0");
		dtSpecialOrdersDetailsAccessories = SpecialOrdersReturnsDetails.SelectByReturnID("0", GlobalVariables.IsArabic ? "1" : "0");
		ds = new DataSet();
		ds.Tables.Add(dtDetails);
		ds.Tables.Add(dtSpecialOrdersDetailsAccessories);
		ds.Tables[0].TableName = "dtDetails";
		ds.Tables[1].TableName = "dtSpecialOrdersDetailsAccessories";
		ds.Relations.Add(ds.Tables[0].Columns["ReturnDetailID"], ds.Tables[1].Columns["ReturnDetailID"]);
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemID"].Width = (int)((double)((Control)(object)ULGData).Width * 0.19) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[1].Columns["ItemID"].ValueList = (IValueList)(object)vlAccessoriesItems;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[1].Columns["UnitID"].ValueList = (IValueList)(object)vlAccessoriesUnits;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[1].Columns["StoreID"].Header).Caption = (GlobalVariables.IsArabic ? "المخزن" : "Store");
		((UltraGridBase)ULGData).DisplayLayout.Bands[1].Columns["StoreID"].Hidden = false;
		((UltraGridBase)ULGData).DisplayLayout.Bands[1].Columns["StoreID"].ValueList = (IValueList)(object)vlAccessoriesStores;
	}
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = SpecialOrdersReturns.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((TextEditorControlBase)cboSpecialOrderNo).ValueChanged -= cboSpecialOrderNo_ValueChanged;
			((TextEditorControlBase)cboSpecialOrderNo).Value = drMaster["SpecialOrderID"];
			((TextEditorControlBase)cboSpecialOrderNo).ValueChanged += cboSpecialOrderNo_ValueChanged;
			...
			dtDetails = SpecialOrdersReturnsDetails.SelectByReturnID(drMaster["ReturnID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			dtSpecialOrdersDetailsAccessories = SpecialOrdersReturnsDetailsAccessories.SelectByReturnID(drMaster["ReturnID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			ds = new DataSet();
			ds.Tables.Add(dtDetails);
			ds.Tables.Add(dtSpecialOrdersDetailsAccessories);
			ds.Tables[0].TableName = "dtDetails";
	...
	public override void SetControls(bool NavMode)
	{
		...
		((EditorButtonControlBase)cboSpecialOrderNo).ReadOnly = NavMode || !Adding;
		...
		int num = 0;
		if (cboSpecialOrderNo.SelectedIndex > -1)
		{
			num = int.Parse(((TextEditorControlBase)cboSpecialOrderNo).Value.ToString());
		}
		if (Adding || Updating)
		{
			DataView dataView = new DataView(dtSpecialOrders);
			dataView.RowFilter = " BranchID= " + GlobalVariables.CurrentBranchID;
			GlobalFunctions.FillCombo(cboSpecialOrderNo, dataView.ToTable(), "SpecialOrderID", "SpecialOrderNo");
		...
		if (num > 0)
		{
			((TextEditorControlBase)cboSpecialOrderNo).Value = num;
		}
		((TextEditorControlBase)cboSpecialOrderNo).ValueChanged += cboSpecialOrderNo_ValueChanged;
		if (Adding)
		{
	...
	public override void ClearControls()
	{
		...
		((Control)(object)txtCode).Text = (Adding ? SpecialOrdersReturns.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID) : "");
		...
		((TextEditorControlBase)cboSpecialOrderNo).ValueChanged -= cboSpecialOrderNo_ValueChanged;
		cboSpecialOrderNo.SelectedIndex = -1;
		((TextEditorControlBase)cboSpecialOrderNo).ValueChanged += cboSpecialOrderNo_ValueChanged;
	...
	public override bool ValidateData()
	{
		...
		if (((DataSet)((UltraGridBase)ULGData).DataSource).Tables[0].Select(" Qty =0 ").Length != 0)
		{
		...
		if (cboSpecialOrderNo.SelectedIndex == -1)
		{
			...
			cboSpecialOrderNo.DropDown();
			return false;
		}
		if (dtSpecialOrders.Select(" SpecialOrderID = " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["SubAccountID"] == DBNull.Value && decimal.Parse(((Control)(object)txtRestAmount).Text) > 0m)
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "العميل ليس له حساب تحليلى" : "Client Does not Have a SubAccount");
			return false;
		}
		if (Main.CheckForValueByBranchIDAndFiscalYearID("POS_SpecialOrdersReturns", "ReturnNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["ReturnNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = SpecialOrdersReturns.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			if (((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows).Count > 0 && decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value.ToString()) != decimal.Parse(((UltraGridBase)ULGData).Rows[i].Cells["MaxAllowedQty"].Value.ToString()))
			{
				GlobalVariables.InformationMB.Show("برجاء ادخال الكمية كاملة لأن بها ملحقات", "Please Enter all the Quantity because it has accessories");
				ULGData.ActiveCell = ((UltraGridBase)ULGData).Rows[i].Cells["Qty"];
				return false;
			}
	...
	public override void AddData()
	{
		...
		try
		{
			num = SpecialOrdersReturns.Insert_Update("-1", ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString(), ShiftDetailID.ToString(), ShiftDetailUserID.ToString(), (dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ClientID"] == DBNull.Value) ? "Null" : dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ClientID"].ToString(), (dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["SubAccountID"] == DBNull.Value) ? "Null" : dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["SubAccountID"].ToString(), dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["CurrencyID"].ToString(), dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ExchangeRate"].ToString(), ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, (((Control)(object)txtPaidAmount).Text == "") ? "0" : ((Control)(object)txtPaidAmount).Text, (((Control)(object)txtRestAmount).Text == "") ? "0" : ((Control)(object)txtRestAmount).Text, dtSpecialOrders.Select("SpecialOrderID=" + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["RoomID"].ToString(), GlobalVariables.UserID, "Null", "Null", "0", "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			ULGData.AfterCellUpdate -= new CellEventHandler(ULGData_AfterCellUpdate);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				int num2 = SpecialOrdersReturnsDetails.Insert_Update("-1", num.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["ColorID"].Value == DBNull.Value) ? "1" : ((UltraGridBase)ULGData).Rows[i].Cells["ColorID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["ItemSizeID"].Value == DBNull.Value) ? "1" : ((UltraGridBase)ULGData).Rows[i].Cells["ItemSizeID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value == DBNull.Value) ? "0" : ((UltraGridBase)ULGData).Rows[i].Cells["Qty"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["UnitID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[i].Cells["UnitID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value == DBNull.Value) ? "0" : ((UltraGridBase)ULGData).Rows[i].Cells["UnitPrice"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].Cells["StoreID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[i].Cells["StoreID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[i].Cells["Notes"].Value.ToString(), ((UltraGridBase)ULGData).Rows[i].Cells["SpecialOrderDetailID"].Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows).Count; j++)
				{
					SpecialOrdersReturnsDetailsAccessories.Insert_Update("-1", num2.ToString(), num.ToString(), (((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["ItemID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["ItemID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["Qty"].Value == DBNull.Value) ? "0" : ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["Qty"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["UnitID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["UnitID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["StoreID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["StoreID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["Notes"].Value.ToString(), ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["SpecialOrderDetailAccessoryID"].Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				}
			}
			ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
		...
		try
		{
			reportDocument.Load(GlobalVariables.ReportsPath + (GlobalVariables.IsArabic ? "Rep_POS_SpecialOrdersReturns_A.rpt" : "Rep_POS_SpecialOrdersReturns_E.rpt"));
			GlobalFunctions.ConfigureReport(reportDocument);
			reportDocument.SetParameterValue("@ReturnIDs", "," + num + ",");
			reportDocument.SetParameterValue("@RoomID", dtSpecialOrders.Select("SpecialOrderID=" + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["RoomID"].ToString());
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = SpecialOrdersReturns.Insert_Update(drMaster["ReturnID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString(), ShiftDetailID.ToString(), ShiftDetailUserID.ToString(), (dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ClientID"] == DBNull.Value) ? "Null" : dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ClientID"].ToString(), (dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["SubAccountID"] == DBNull.Value) ? "Null" : dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["SubAccountID"].ToString(), dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["CurrencyID"].ToString(), dtSpecialOrders.Select(" SpecialOrderID= " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ExchangeRate"].ToString(), ((Control)(object)txtNotes).Text, ((Control)(object)txtTotal).Text, (((Control)(object)txtPaidAmount).Text == "") ? "0" : ((Control)(object)txtPaidAmount).Text, (((Control)(object)txtRestAmount).Text == "") ? "0" : ((Control)(object)txtRestAmount).Text, dtSpecialOrders.Select("SpecialOrderID=" + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["RoomID"].ToString(), GlobalVariables.UserID, (drMaster["StockControlJVID"] == DBNull.Value) ? "Null" : drMaster["StockControlJVID"].ToString(), (drMaster["SalesJVID"] == DBNull.Value) ? "Null" : drMaster["SalesJVID"].ToString(), bool.Parse(drMaster["Approved"].ToString()) ? "1" : "0", "1", bool.Parse(drMaster["Deleted"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				...
				for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows).Count; j++)
				{
					text2 = text2 + ((UltraGridBase)ULGData).Rows[i].ChildBands[0].Rows[j].Cells["ReturnDetailAccessoryID"].Value.ToString() + ",";
				}
			}
			ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			Main.DeleteForUpdate("POS_SpecialOrdersReturnsDetailsAccessories", "ReturnID", drMaster["ReturnID"].ToString(), "ReturnDetailAccessoryID", text2);
			Main.DeleteForUpdate("POS_SpecialOrdersReturnsDetails", "ReturnID", drMaster["ReturnID"].ToString(), "ReturnDetailID", text);
			for (int k = 0; k < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; k++)
			{
				int num2 = SpecialOrdersReturnsDetails.Insert_Update(((UltraGridBase)ULGData).Rows[k].Cells["ReturnDetailID"].Value.ToString(), num.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["ItemID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[k].Cells["ItemID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["ColorID"].Value == DBNull.Value) ? "1" : ((UltraGridBase)ULGData).Rows[k].Cells["ColorID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["ItemSizeID"].Value == DBNull.Value) ? "1" : ((UltraGridBase)ULGData).Rows[k].Cells["ItemSizeID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["Qty"].Value == DBNull.Value) ? "0" : ((UltraGridBase)ULGData).Rows[k].Cells["Qty"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["UnitID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[k].Cells["UnitID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["UnitPrice"].Value == DBNull.Value) ? "0" : ((UltraGridBase)ULGData).Rows[k].Cells["UnitPrice"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].Cells["StoreID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[k].Cells["StoreID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[k].Cells["Notes"].Value.ToString(), ((UltraGridBase)ULGData).Rows[k].Cells["SpecialOrderDetailID"].Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				for (int l = 0; l < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows).Count; l++)
				{
					SpecialOrdersReturnsDetailsAccessories.Insert_Update(((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["ReturnDetailAccessoryID"].Value.ToString(), num2.ToString(), num.ToString(), (((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["ItemID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["ItemID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["Qty"].Value == DBNull.Value) ? "0" : ((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["Qty"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["UnitID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["UnitID"].Value.ToString(), (((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["StoreID"].Value == DBNull.Value) ? "Null" : ((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["StoreID"].Value.ToString(), ((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["Notes"].Value.ToString(), ((UltraGridBase)ULGData).Rows[k].ChildBands[0].Rows[l].Cells["SpecialOrderDetailAccessoryID"].Value.ToString(), dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
				}
			}
			ShiftsDetails.ReturnJVDineInRegenerate(ShiftDetailID, GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
		}
		catch
		{
	...
	public override void DeleteData()
	{
		...
		try
		{
			...
			SpecialOrdersReturns.DeleteVirtual(drMaster["ReturnID"].ToString(), GlobalVariables.UserID);
			SpecialOrdersReturnsDetails.DeleteVirtualByReturnID(drMaster["ReturnID"].ToString(), GlobalVariables.UserID);
			SpecialOrdersReturnsDetailsAccessories.DeleteVirtualByReturnID(drMaster["ReturnID"].ToString(), GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
			ItemsTransactions.ManageInThread();
		}
		catch
	...
	{
		dtSearchResult = SearchFunctions.SpecialOrdersReturnsReport(-1, 0);
	...
	public override void btnRefreshDataClick()
	{
		...
		for (int k = 0; k < dataTable.Rows.Count; k++)
		{
			...
			vlAccessoriesStores.ValueListItems.Add(dataTable.Rows[k]["StoreID"], dataTable.Rows[k]["StoreName"].ToString());
		}
		if (Adding)
		{
			...
			for (int l = 0; l < dataTable2.Rows.Count; l++)
			{
				...
				vlAccessoriesItems.ValueListItems.Add(dataTable2.Rows[l]["ItemID"], dataTable2.Rows[l]["Name"].ToString());
			}
		}
		else
		{
			...
			for (int m = 0; m < dtItems.Rows.Count; m++)
			{
				...
				vlAccessoriesItems.ValueListItems.Add(dtItems.Rows[m]["ItemID"], dtItems.Rows[m]["Name"].ToString());
			}
		}
		dtUnits = Units.FillCombo("-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		vlAccessoriesUnits.ValueListItems.Clear();
		...
		{
			vlAccessoriesUnits.ValueListItems.Add(dtUnits.Rows[n]["UnitID"], dtUnits.Rows[n]["UnitName"].ToString());
		}
		dtMinAllowedTransDate = Stores.GetMinAllowedTransDate(IsFromServer: false);
		dtClients = Clients.FillCombo("-1", GlobalVariables.CurrentBranchID, GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		GlobalFunctions.FillCombo(cboClient, dtClients, "ClientID", "ClientName");
	...
	public override void SelectFullRow(object sender, EventArgs e)
	{
		...
		if ((((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "Qty" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "Notes") || ((GridItemBase)ULGData.ActiveCell).Band.Index == 1)
		{
	...
	private void CalculateTotals()
	{
		...
		UltraTextEditor obj = txtTotal;
		string text = (((Control)(object)txtPaidAmount).Text = decimal.Parse(num.ToString()).ToString(GlobalVariables.txtDecimalFormate));
	...
	private void dtpDate_ValueChanged(object sender, EventArgs e)
	{
		if (Adding)
		{
			((Control)(object)txtCode).Text = SpecialOrdersReturns.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		}
	}

	private void cboSpecialOrderNo_ValueChanged(object sender, EventArgs e)
	{
		if (cboSpecialOrderNo.SelectedIndex > -1)
		{
			((DataSet)((UltraGridBase)ULGData).DataSource).Clear();
			((TextEditorControlBase)cboClient).ValueChanged -= cboClient_ValueChanged;
			((TextEditorControlBase)cboClientCode).ValueChanged -= cboClientCode_ValueChanged;
			UltraComboEditor obj = cboClientCode;
			object value = (((TextEditorControlBase)cboClient).Value = dtSpecialOrders.Select("SpecialOrderID = " + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString())[0]["ClientID"]);
			((TextEditorControlBase)obj).Value = value;
			((TextEditorControlBase)cboClient).ValueChanged += cboClient_ValueChanged;
			((TextEditorControlBase)cboClientCode).ValueChanged += cboClientCode_ValueChanged;
			DataTable table = SpecialOrdersReturnsDetails.SelectBySpecialOrderIDs("," + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString() + ",", GlobalVariables.IsArabic ? "1" : "0");
			DataTable table2 = SpecialOrdersReturnsDetailsAccessories.SelectBySpecialOrderIDs("," + ((TextEditorControlBase)cboSpecialOrderNo).Value.ToString() + ",", GlobalVariables.IsArabic ? "1" : "0");
			ds = new DataSet();
			ds.Tables.Add(table);
			ds.Tables.Add(table2);
			ds.Tables[0].TableName = "dtDetails1";
			ds.Tables[1].TableName = "dtSpecialOrdersDetailsAccessories1";
			ds.Relations.Add(ds.Tables[0].Columns["SpecialOrderDetailID"], ds.Tables[1].Columns["SpecialOrderDetailID"]);
			((UltraGridBase)ULGData).DataSource = ds;
			InitGrid();
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.POS.Transactions.frmSpecialOrdersReturns));
		...
		this.cboSpecialOrderNo = new UltraComboEditor();
		...
		((System.ComponentModel.ISupportInitialize)this.cboSpecialOrderNo).BeginInit();
		base.SuspendLayout();
		...
		resources.ApplyResources(this.cboSpecialOrderNo, "cboSpecialOrderNo");
		((TextEditorControlBase)this.cboSpecialOrderNo).AlwaysInEditMode = true;
		this.cboSpecialOrderNo.AutoCompleteMode = (AutoCompleteMode)4;
		((System.Windows.Forms.Control)(object)this.cboSpecialOrderNo).Name = "cboSpecialOrderNo";
		((TextEditorControlBase)this.cboSpecialOrderNo).ValueChanged += new System.EventHandler(cboSpecialOrderNo_ValueChanged);
		...
		base.Controls.Add((System.Windows.Forms.Control)(object)this.cboSpecialOrderNo);
		...
		base.Name = "frmSpecialOrdersReturns";
		...
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.cboSpecialOrderNo, 0);
		...
		((System.ComponentModel.ISupportInitialize)this.cboSpecialOrderNo).EndInit();
