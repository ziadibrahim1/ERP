...
public class frmSelectCheck : frmBase
{
	private int TableID;
	...
	public frmSelectCheck(int tableID, int roomID, string tableNo)
	{
		InitializeComponent();
		TableID = tableID;
		RoomID = roomID;
	...
	{
		dtChecks = Checks.SelectByTableID(TableID.ToString(), RoomID.ToString());
		CreateRooms();
	}
	...
	public void CreateRooms()
	{
		...
		Control[] array = panel1.Controls.Find("btnCheck", searchAllChildren: true);
		...
		int num = 0;
		int num2 = 0;
		for (int j = 0; j < dtChecks.Rows.Count; j++)
		{
			Appearance val = new Appearance();
			UltraButton val2 = new UltraButton();
			((AppearanceBase)val).Image = Resources.MAN;
			...
			((ControlBase)val2).ImageSize = new Size(50, 62);
			...
			((Control)(object)val2).Click += btnCheck_Click;
			((Control)(object)val2).Name = "btnCheck";
			((Control)(object)val2).Tag = dtChecks.Rows[j]["CheckID"];
			((Control)(object)val2).Text = "CheckNo. " + dtChecks.Rows[j]["CheckNo"].ToString() + "\n" + dtChecks.Rows[j]["Notes"].ToString();
			((Control)(object)val2).TabIndex = j + 10;
			((Control)(object)val2).Location = new Point(base.Width * num / 3 + 20, base.Height * num2 / 2 + 20);
			if (Convert.ToBoolean(dtChecks.Rows[j]["IsPrinted"]))
			{
				((ControlBase)val2).Appearance.BackColor = Color.Green;
			}
	...
	{
		frmChecks frmChecks2 = new frmChecks(int.Parse(((Control)sender).Tag.ToString()), TableID, RoomID, TableNo);
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.POS.Transactions.frmSelectCheck));
		Appearance val = new Appearance();
