...
public class frmPOSChecksGiftsoffers : frmBase
{
	...
	private DataTable dtGiftsAndGroups;
	...
	private UltraGroupBox UGBGiftsData;

	private UltraPanel pnlGifts;

	private UltraTabControl UTCGiftsGroups;

	private UltraTabSharedControlsPage ultraTabGifts;

	public frmPOSChecksGiftsoffers(int OFFERID, DataTable DTRoomItems)
	{
	...
	public override void PrepareData()
	{
		...
		for (int k = 0; k < dtUnits.Rows.Count; k++)
		{
			...
			vlUnitsGifts.ValueListItems.Add(dtUnits.Rows[k]["UnitID"], dtUnits.Rows[k]["UnitName"].ToString());
		}
		dtOffersGifts = ChecksDetails.SelectByCheckID("0", GlobalVariables.IsArabic ? "1" : "0");
		...
		if (cboOffer.SelectedIndex > -1)
		{
			...
			((Control)(object)txtItemsQty).Text = dataRow["ItemsQty"].ToString();
			((Control)(object)txtGiftsQty).Text = dataRow["GiftsQty"].ToString();
			...
			for (int m = 0; m < dtGifts.Rows.Count; m++)
			{
				vlGifts.ValueListItems.Add(dtGifts.Rows[m]["ItemID"], dtGifts.Rows[m]["Name"].ToString());
			}
			dtItemsAndGroups = OffersItems.SelectItemsAndGroupsByOfferID(OfferID.ToString(), GlobalVariables.IsArabic ? "1" : "0");
			FillItemsGroups();
			dtGiftsAndGroups = OffersGifts.SelectItemsAndGroupsByOfferID(OfferID.ToString(), GlobalVariables.IsArabic ? "1" : "0");
			FillGiftsGroups();
		}
	...
	private void btnSave_Click(object sender, EventArgs e)
	{
		...
		for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGDataGifts).Rows).Count; j++)
		{
			...
			if (dtRoomItems.Select("IsOffer = 0 and  ItemID =" + ((UltraGridBase)ULGDataGifts).Rows[j].Cells["ItemID"].Value.ToString()).Length == 0)
			{
				GlobalVariables.InformationMB.Show("بعض الهدايا المختارة غير موجوده بهذه الصالة ", "Some of these chosen Gifts not exists in this room ");
				ULGDataGifts.ActiveCell = ((UltraGridBase)ULGDataGifts).Rows[j].Cells["ItemID"];
				((UltraGridBase)ULGDataGifts).Rows[j].Cells["ItemID"].DroppedDown = true;
				return;
			}
		}
	...
	public void AddItemInGid()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGDataItems).Rows).Count; i++)
		{
			if (((UltraGridBase)ULGDataItems).Rows[i].Cells["ItemBarCode"].Text == text && ((UltraGridBase)ULGDataItems).Rows[i].Cells["ColorID"].Value.ToString() == text2 && ((UltraGridBase)ULGDataItems).Rows[i].Cells["ItemSizeID"].Value.ToString() == text3)
			{
	...
	public void AddGiftsInGid()
	{
		...
		{
			if (((UltraGridBase)ULGDataGifts).Rows[i].Cells["ItemBarCode"].Text == text && ((UltraGridBase)ULGDataGifts).Rows[i].Cells["ColorID"].Value.ToString() == text2 && ((UltraGridBase)ULGDataGifts).Rows[i].Cells["ItemSizeID"].Value.ToString() == text3)
			{
	...
	public void FillGiftsGroups()
	{
		...
		int num = 8;
		int num2 = 6;
		DataTable dataTable = dtGiftsAndGroups.DefaultView.ToTable(true, "ItemFirstClassificationName");
		DataView dataView = new DataView(dtGiftsAndGroups);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			UltraPanel val = new UltraPanel();
			dataView.RowFilter = ((dataTable.Rows[i]["ItemFirstClassificationName"] == DBNull.Value) ? "IsMain=1 And ItemFirstClassificationName is null" : ("IsMain=1 And ItemFirstClassificationName='" + dataTable.Rows[i]["ItemFirstClassificationName"].ToString() + "'"));
			...
			if (dataView.Count > 0)
			{
				((UltraTabControlBase)UTCGiftsGroups).Tabs.Add(i.ToString(), (dataTable.Rows[i]["ItemFirstClassificationName"] != DBNull.Value) ? dataTable.Rows[i]["ItemFirstClassificationName"].ToString() : (GlobalVariables.IsArabic ? "مجموعات الاصناف" : "Item Groups"));
				((UltraControlBase)UTCGiftsGroups).UseAppStyling = false;
				((UltraTabControlBase)UTCGiftsGroups).TabSize = new Size(15, 40);
				((UltraTabControlBase)UTCGiftsGroups).Tabs[i.ToString()].FixedWidth = 120;
				((UltraTabControlBase)UTCGiftsGroups).TabHeaderAreaAppearance.FontData.SizeInPoints = 10f;
				((Control)(object)val).Dock = DockStyle.Fill;
				val.AutoScroll = true;
				((Control)(object)((UltraTabControlBase)UTCGiftsGroups).Tabs[i.ToString()].TabPage).Controls.Add((Control)(object)val);
			}
			num = 8;
			num2 = 6;
			for (int j = 0; j < dataView.Count; j++)
			{
				UltraButton val2 = new UltraButton();
				((Control)(object)val2).Click += btnGiftGroub_Click;
				((Control)(object)val2).Tag = dataView[j]["ItemID"].ToString();
				((Control)(object)val2).Text = dataView[j]["ItemName"].ToString();
				((Control)(object)val2).Height += 25;
				((Control)(object)val.ClientArea).Controls.Add((Control)(object)val2);
	...
	public void btnGiftGroub_Click(object sender, EventArgs e)
	{
		//IL_0037: Unknown result type (might be due to invalid IL or missing references)
		//IL_0041: Expected O, but got Unknown
		//IL_005f: Unknown result type (might be due to invalid IL or missing references)
		//IL_0066: Expected O, but got Unknown
		((Control)(object)pnlGifts).Visible = false;
		((Control)(object)pnlGifts.ClientArea).Controls.Clear();
		DataView dataView = new DataView(dtGiftsAndGroups);
		dataView.RowFilter = "IsMain=0 And ParentID=" + ((Control)(UltraButton)sender).Tag;
		dataView.ToTable();
		int num = 8;
		...
		{
			UltraButton val = new UltraButton();
			((Control)(object)val).Click += btnGifts_Click;
			((Control)(object)val).Tag = dataView[i]["ItemID"].ToString();
			((Control)(object)val).Text = dataView[i]["ItemName"].ToString();
			((Control)(object)val).Height += 25;
			((Control)(object)pnlGifts.ClientArea).Controls.Add((Control)(object)val);
			((UltraControlBase)val).Update();
			if (num + ((Control)(object)val).Width > ((Control)(object)pnlGifts).Width)
			{
				num2 += ((Control)(object)val).Height;
				num = 8;
			}
			((Control)(object)val).Left = num;
			((Control)(object)val).Top = num2;
			((Control)(object)val).Width = (((Control)(object)pnlGifts).Width - GlobalVariables.ScrollWidth) / 6;
			num += ((Control)(object)val).Width;
		}
		((Control)(object)pnlGifts).Visible = true;
	}

	public void btnGifts_Click(object sender, EventArgs e)
	{
		//IL_0047: Unknown result type (might be due to invalid IL or missing references)
		//IL_0051: Expected O, but got Unknown
		//IL_02ba: Unknown result type (might be due to invalid IL or missing references)
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGDataGifts).Rows).Count; i++)
		{
			DataRow dataRow = dtGiftsAndGroups.Select("ItemID =" + ((UltraGridBase)ULGDataGifts).Rows[i].Cells["ItemID"].Value)[0];
			if (int.Parse(((Control)(UltraButton)sender).Tag.ToString()) == int.Parse(((UltraGridBase)ULGDataGifts).Rows[i].Cells["ItemID"].Value.ToString()))
			{
				if (bool.Parse(dtGiftsAndGroups.Select("ItemID =" + ((UltraGridBase)ULGDataGifts).Rows[i].Cells["ItemID"].Value)[0]["IsWeight"].ToString()) || bool.Parse(dtGiftsAndGroups.Select("ItemID =" + ((UltraGridBase)ULGDataGifts).Rows[i].Cells["ItemID"].Value)[0]["UsePOSNumPad"].ToString()))
				{
					frmDecimal frmDecimal2 = new frmDecimal(((UltraGridBase)ULGDataGifts).Rows[i].Cells["Qty"], ((UltraGridBase)ULGDataGifts).Rows[i].Cells["Qty"].Value.ToString());
					frmDecimal2.StartPosition = FormStartPosition.CenterScreen;
					frmDecimal2.ShowDialog();
				}
				else
				{
					((UltraGridBase)ULGDataGifts).Rows[i].Cells["Qty"].Value = decimal.Parse(((UltraGridBase)ULGDataGifts).Rows[i].Cells["Qty"].Value.ToString()) + 1m;
				}
				return;
			}
		}
		((UltraGridBase)ULGDataGifts).DisplayLayout.Override.AllowAddNew = (AllowAddNew)6;
		((UltraGridBase)ULGDataGifts).DisplayLayout.Bands[0].AddNew();
		((UltraGridBase)ULGDataGifts).ActiveRow.Cells["CheckDetailID"].Value = ++newID;
		((UltraGridBase)ULGDataGifts).ActiveRow.Cells["ItemID"].Value = ((Control)(UltraButton)sender).Tag;
		DataRow dataRow2 = dtGiftsAndGroups.Select("ItemID=" + ((UltraGridBase)ULGDataGifts).ActiveRow.Cells["ItemID"].Value.ToString())[0];
		((UltraGridBase)ULGDataGifts).ActiveRow.Cells["UnitID"].Value = dataRow2["UnitID"];
		((UltraGridBase)ULGDataGifts).ActiveRow.Cells["TaxID"].Value = dataRow2["TaxID"];
		if (bool.Parse(dataRow2["IsWeight"].ToString()) || bool.Parse(dataRow2["UsePOSNumPad"].ToString()))
		{
			((UltraGridBase)ULGDataGifts).ActiveRow.Cells["Qty"].Value = 0;
			frmDecimal frmDecimal3 = new frmDecimal(((UltraGridBase)ULGDataGifts).ActiveRow.Cells["Qty"], ((UltraGridBase)ULGDataGifts).ActiveRow.Cells["Qty"].Value.ToString());
			frmDecimal3.StartPosition = FormStartPosition.CenterScreen;
			frmDecimal3.ShowDialog();
		}
		else
		{
			((UltraGridBase)ULGDataGifts).ActiveRow.Cells["Qty"].Value = 1;
		}
		if (UsingColors && dataRow2["ItemColorCategoryID"] != DBNull.Value)
		{
			((UltraGridBase)ULGDataGifts).ActiveRow.Cells["ColorID"].Value = DBNull.Value;
			((UltraGridBase)ULGDataGifts).ActiveRow.Cells["ColorID"].ValueList = (IValueList)(object)getColorsValueList(int.Parse(dataRow2["ItemColorCategoryID"].ToString()));
		}
		if (UsingSizes && dataRow2["ItemSizeCategoryID"] != DBNull.Value)
		{
			((UltraGridBase)ULGDataGifts).ActiveRow.Cells["ItemSizeID"].Value = DBNull.Value;
			((UltraGridBase)ULGDataGifts).ActiveRow.Cells["ItemSizeID"].ValueList = (IValueList)(object)getSizesValueList(int.Parse(dataRow2["ItemSizeCategoryID"].ToString()));
		}
		int index = ((UltraGridBase)ULGDataGifts).ActiveRow.Index;
		((UltraGridBase)ULGDataGifts).DisplayLayout.Bands[0].AddNew();
		((UltraGridBase)ULGDataGifts).Rows[index].Activate();
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.POS.Transactions.frmPOSChecksGiftsoffers));
		...
		this.UGBGiftsData = new UltraGroupBox();
		this.UTCGiftsGroups = new UltraTabControl();
		this.ultraTabGifts = new UltraTabSharedControlsPage();
		((System.ComponentModel.ISupportInitialize)base.dtUsersTransactions).BeginInit();
		((System.Windows.Forms.Control)(object)this.pnlItems).SuspendLayout();
		((System.Windows.Forms.Control)(object)this.pnlGifts).SuspendLayout();
		...
		((System.ComponentModel.ISupportInitialize)this.UGBGiftsData).BeginInit();
		((System.Windows.Forms.Control)(object)this.UGBGiftsData).SuspendLayout();
		((System.ComponentModel.ISupportInitialize)this.UTCGiftsGroups).BeginInit();
		((System.Windows.Forms.Control)(object)this.UTCGiftsGroups).SuspendLayout();
		base.SuspendLayout();
		...
		resources.ApplyResources(this.pnlGifts, "pnlGifts");
		this.pnlGifts.AutoScroll = true;
		resources.ApplyResources(this.pnlGifts.ClientArea, "pnlGifts.ClientArea");
		((System.Windows.Forms.Control)(object)this.pnlGifts).Name = "pnlGifts";
		...
		this.ULGDataGifts.ClickCell += new ClickCellEventHandler(ULGData_ClickCell);
		...
		((AppearanceBase)val22).Image = resources.GetObject("appearance28.Image");
		resources.ApplyResources(val22, "appearance28");
		...
		resources.ApplyResources(val23, "appearance29");
		...
		((AppearanceBase)val24).Image = resources.GetObject("appearance30.Image");
		resources.ApplyResources(val24, "appearance30");
		...
		((UltraTabControlBase)this.UTCItemsGroups).Appearance = (AppearanceBase)(object)val25;
		...
		resources.ApplyResources(this.UGBGiftsData, "UGBGiftsData");
		this.UGBGiftsData.CaptionAlignment = (GroupBoxCaptionAlignment)1;
		((System.Windows.Forms.Control)(object)this.UGBGiftsData).Controls.Add((System.Windows.Forms.Control)(object)this.pnlGifts);
		((System.Windows.Forms.Control)(object)this.UGBGiftsData).Name = "UGBGiftsData";
		resources.ApplyResources(this.UTCGiftsGroups, "UTCGiftsGroups");
		resources.ApplyResources(val26, "appearance26");
		((AppearanceBase)val26).TextTrimming = (TextTrimming)6;
		((UltraTabControlBase)this.UTCGiftsGroups).Appearance = (AppearanceBase)(object)val26;
		((System.Windows.Forms.Control)(object)this.UTCGiftsGroups).Controls.Add((System.Windows.Forms.Control)(object)this.ultraTabGifts);
		((System.Windows.Forms.Control)(object)this.UTCGiftsGroups).Name = "UTCGiftsGroups";
		((UltraTabControlBase)this.UTCGiftsGroups).SharedControlsPage = this.ultraTabGifts;
		resources.ApplyResources(this.ultraTabGifts, "ultraTabGifts");
		((System.Windows.Forms.Control)(object)this.ultraTabGifts).Name = "ultraTabGifts";
		resources.ApplyResources(this, "$this");
		base.Controls.Add((System.Windows.Forms.Control)(object)this.UTCGiftsGroups);
		base.Controls.Add((System.Windows.Forms.Control)(object)this.UGBGiftsData);
		...
		base.Name = "frmPOSChecksGiftsoffers";
		...
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.UGBGiftsData, 0);
		base.Controls.SetChildIndex((System.Windows.Forms.Control)(object)this.UTCGiftsGroups, 0);
		((System.ComponentModel.ISupportInitialize)base.dtUsersTransactions).EndInit();
		((System.Windows.Forms.Control)(object)this.pnlItems).ResumeLayout(false);
		((System.Windows.Forms.Control)(object)this.pnlGifts).ResumeLayout(false);
		...
		((System.ComponentModel.ISupportInitialize)this.UGBGiftsData).EndInit();
		((System.Windows.Forms.Control)(object)this.UGBGiftsData).ResumeLayout(false);
		((System.ComponentModel.ISupportInitialize)this.UTCGiftsGroups).EndInit();
		((System.Windows.Forms.Control)(object)this.UTCGiftsGroups).ResumeLayout(false);
