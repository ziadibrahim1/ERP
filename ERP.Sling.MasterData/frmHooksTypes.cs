...
public class frmHooksTypes : frmHeaderDetails
{
	...
	public frmHooksTypes()
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000b: Expected O, but got Unknown
		InitializeComponent();
		TableName = "SLN_HooksTypes";
		IDCol = "HookTypeID";
		NoCol = "HookTypeCode";
		DateCol = "GetDate()";
	}
	...
	public override void PrepareData()
	{
		...
		dtDetails = HooksTypesDetails.SelectByHookTypeID("0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["HookTypeDetailID"].DefaultCellValue = -1;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["HookTypeDetailItemID"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["HookTypeDetailItemID"].Header).Caption = (GlobalVariables.IsArabic ? "اسم الصنف" : "Item Name");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["HookTypeDetailItemID"].ValueList = (IValueList)(object)vlItems;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["HookTypeDetailItemID"].Width = (int)((double)((Control)(object)ULGData).Width * 0.7) - GlobalVariables.ScrollWidth;
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = HooksTypes.Select(RowID, "-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			((Control)(object)txtCode).Text = drMaster["HookTypeCode"].ToString();
			((Control)(object)txtNameAr).Text = drMaster["HookTypeNameAr"].ToString();
			((Control)(object)txtNameEn).Text = drMaster["HookTypeNameEn"].ToString();
			dtDetails = HooksTypesDetails.SelectByHookTypeID(drMaster["HookTypeID"].ToString(), GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
		}
		else
	...
	public override void ClearControls()
	{
		...
		((Control)(object)txtCode).Text = (Adding ? HooksTypes.GetCode(IsFromServer: true) : "");
	...
	public override bool ValidateData()
	{
		...
		if (Main.CheckForValue("SLN_HooksTypes", "HookTypeNameAr", ((Control)(object)txtNameAr).Text, Updating ? drMaster["HookTypeNameAr"].ToString() : "", IsFromServer: true) > 0)
		{
			GlobalVariables.InformationMB.Show(" الاسم بالعربيه متواجد من قبل ", "Arabic Name Already Exist");
			((TextEditorControlBase)txtNameAr).Focus();
			return false;
		}
		...
		if (Main.CheckForValue("SLN_HooksTypes", "HookTypeNameEn", ((Control)(object)txtNameEn).Text, Updating ? drMaster["HookTypeNameEn"].ToString() : "", IsFromServer: true) > 0)
		{
			GlobalVariables.InformationMB.Show(" الاسم بالانجليزيه متواجد من قبل ", "English Name Already Exist");
			((TextEditorControlBase)txtNameEn).Focus();
			return false;
		}
		if (Main.CheckForValue("SLN_HooksTypes", "HookTypeCode", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["HookTypeCode"].ToString(), IsFromServer: true) > 0)
		{
			string code = HooksTypes.GetCode(IsFromServer: true);
			GlobalVariables.QuestionMB.Show("رقم هذا النوع متواجد من قبل \n سوف يتم الحفظ برقم " + code, "The Hook type code Already Exists It Will Be Saved With No. : " + code);
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				if (i != j && ((UltraGridBase)ULGData).Rows[i].Cells["HookTypeDetailItemID"].Value.ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["HookTypeDetailItemID"].Value.ToString())
				{
					GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "إسم الصنف متواجد من قبل" : "Hook Item Already Exist");
					return false;
				}
			}
		}
		...
		{
			if (((UltraGridBase)ULGData).Rows[k].Cells["HookTypeDetailItemID"].Value == DBNull.Value)
			{
				GlobalVariables.InformationMB.Show("برجاء ادخال اسم الصنف  ", "Please Enter Item Name");
				ULGData.ActiveCell = ((UltraGridBase)ULGData).Rows[k].Cells["HookTypeDetailItemID"];
				((UltraGridBase)ULGData).Rows[k].Cells["HookTypeDetailItemID"].DroppedDown = true;
				return false;
			}
	...
	public override void AddData()
	{
		...
		try
		{
			int num = HooksTypes.Insert_Update("-1", ((Control)(object)txtCode).Text, ((Control)(object)txtNameAr).Text, (((Control)(object)txtNameEn).Text == "") ? ((Control)(object)txtNameAr).Text : ((Control)(object)txtNameEn).Text, "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID, IsFromServer: true);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				((UltraGridBase)ULGData).Rows[i].Cells["HookTypeDetailID"].Value = "-1";
				((UltraGridBase)ULGData).Rows[i].Cells["HookTypeID"].Value = num.ToString();
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
			}
			if (((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count > 0)
			{
				HooksTypesDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID, IsFromServer: true);
			}
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = HooksTypes.Insert_Update(drMaster["HookTypeID"].ToString(), ((Control)(object)txtCode).Text, ((Control)(object)txtNameAr).Text, (((Control)(object)txtNameEn).Text == "") ? "Null" : ((Control)(object)txtNameEn).Text, "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID, IsFromServer: true);
			string text = ",";
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["HookTypeDetailID"].Value.ToString() + ",";
				((UltraGridBase)ULGData).Rows[i].Cells["HookTypeID"].Value = num.ToString();
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
			}
			Main.SyncDeleteForUpdate("SLN_HooksTypesDetails", "HookTypeID", num.ToString(), "HookTypeDetailID", text, IsFromServer: true);
			if (((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count > 0)
			{
				HooksTypesDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID, IsFromServer: true);
			}
	...
	public override void DeleteData()
	{
		...
		try
		{
			HooksTypesDetails.DeleteByHookTypeID(drMaster["HookTypeID"].ToString(), GlobalVariables.UserID, IsFromServer: true);
			HooksTypes.Delete(drMaster["HookTypeID"].ToString(), GlobalVariables.UserID, IsFromServer: true);
			Main.EndBulkTrans(FromServer: true);
		}
		catch
		{
	...
	{
		dtSearchResult = SearchFunctions.HooksTypesSearchReport(IsFromServer: true);
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["HookTypeID"].ToString();
			FillData();
		}
	}
	...
	private void btnSelectItems_Click(object sender, EventArgs e)
	{
		...
		{
			if (dtDetails.Select("HookTypeDetailItemID = " + dataTable.Rows[i]["ItemID"].ToString()).Length == 0)
			{
				DataRow dataRow = dtDetails.NewRow();
				dataRow["HookTypeID"] = (Adding ? ((object)(-1)) : drMaster["HookTypeID"]);
				dataRow["HookTypeDetailID"] = -1;
				dataRow["HookTypeDetailItemID"] = dataTable.Rows[i]["ItemID"];
	...
	private void ULGData_KeyDown(object sender, KeyEventArgs e)
	{
		...
		if ((Adding || Updating) && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "HookTypeDetailItemID")
		{
			...
			if (num != 0)
			{
				((UltraGridBase)ULGData).ActiveRow.Cells["HookTypeDetailItemID"].Value = num;
			}
		}
		e.Handled = true;
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sling.MasterData.frmHooksTypes));
