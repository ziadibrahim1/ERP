...
public class frmWareHouseReceipt : frmHeaderDetails
{
	...
	private ValueList vlPercent = new ValueList();
	...
	public frmWareHouseReceipt()
	{
		...
		InitializeComponent();
		TableName = "WH_WareHouseReceipt";
		IDCol = "WareHouseReceiptID";
		NoCol = "WareHouseReceiptNo";
		DateCol = "InvoiceDate";
	}

	public frmWareHouseReceipt(int ID)
		: this()
	{
		RowID = ID.ToString();
	}
	...
	public override void PrepareData()
	{
		...
		dtPercent = Percentages.FillCombo("-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		vlPercent.ValueListItems.Clear();
		for (int j = 0; j < dtPercent.Rows.Count; j++)
		{
			vlPercent.ValueListItems.Add(dtPercent.Rows[j]["PercentageID"], dtPercent.Rows[j]["Percentage"].ToString());
		}
		...
		dtDetails = WareHouseReceiptDetails.SelectByWareHouseReceiptID("0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["WareHouseReceiptDetailID"].DefaultCellValue = -1;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Percentage"].ValueList = (IValueList)(object)vlPercent;
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = WareHouseReceipt.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((Control)(object)txtCode).Text = drMaster["WareHouseReceiptNo"].ToString();
			...
			dtDetails = WareHouseReceiptDetails.SelectByWareHouseReceiptID(drMaster["WareHouseReceiptID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
	...
	public override void ClearControls()
	{
		...
		((Control)(object)txtCode).Text = (Adding ? WareHouseReceipt.GetCodeByBranchID(GlobalVariables.CurrentBranchID) : "");
	...
	public override bool ValidateData()
	{
		...
		if (Main.CheckForValueByBranchIDAndFiscalYearID("WH_WareHouseReceipt", "WareHouseReceiptNo", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["WareHouseReceiptNo"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = WareHouseReceipt.GetCodeByBranchID(GlobalVariables.CurrentBranchID);
	...
	public override void UpdateData()
	{
		...
		try
		{
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				...
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
				((UltraGridBase)ULGData).Rows[i].Cells["VoucherDate"].Value = dtpDate.DateTime;
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["WareHouseReceiptDetailID"].Value.ToString() + ",";
			}
			ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			Main.DeleteForUpdate("WH_WareHouseReceiptDetails", "WareHouseReceiptID", drMaster["WareHouseReceiptID"].ToString(), "WareHouseReceiptDetailID", text);
			WareHouseReceiptDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
			Main.EndBulkTrans(FromServer: false);
		}
		catch
	...
	public override void DeleteData()
	{
		...
		try
		{
			WareHouseReceipt.DeleteVirtual(drMaster["WareHouseReceiptID"].ToString(), GlobalVariables.UserID);
			WareHouseReceiptDetails.DeleteVirtualByWareHouseReceiptID(drMaster["WareHouseReceiptID"].ToString(), GlobalVariables.UserID);
			string text = MessageLog.SelectByVoucherIDAndTransType(drMaster["WareHouseReceiptID"].ToString(), "GRN", "GRN", GlobalVariables.IsArabic ? "1" : "0");
			Main.EndBulkTrans(FromServer: false);
		}
		catch
		{
	...
	{
		dtSearchResult = SearchFunctions.WareHouseReceiptSearchReport(-1, 0);
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["WareHouseReceiptID"].ToString();
			FillData();
		}
	}
	...
	public override void btnDeleteClick()
	{
		...
		if (Convert.ToInt32(WareHouseReceipt.CheckForUpdate(RowID).Rows[0]["IssueRowsCount"]) > 0)
		{
			GlobalVariables.InformationMB.Show("تم صرف بعض اصناف هذا الاذن لا يمكن الحذف", "Cannot Delete This Transaction Because Some Items of this voucher have been issued");
			return;
		}
		if (!CanDelete)
		{
	...
	public override void btnUpdateClick()
	{
		...
		if (Convert.ToInt32(WareHouseReceipt.CheckForUpdate(RowID).Rows[0]["IssueRowsCount"]) > 0)
		{
			GlobalVariables.InformationMB.Show("تم صرف بعض اصناف هذا الاذن لا يمكن التعديل", "Cannot Update This Transaction Because Some Items of this voucher have been issued");
			return;
		}
		if (!CanUpdate)
		{
	...
	public override void SelectFullRow(object sender, EventArgs e)
	{
		...
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "StoreID" && ((UltraToggleEditorBase)chkStore).Checked)
		{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.WareHouse.Transactions.frmWareHouseReceipt));
