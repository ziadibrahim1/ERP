...
public class frmSafeInSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmSafeInSearchReport()
	{
		InitializeComponent();
	}

	public frmSafeInSearchReport(DataTable dt, string idColumnName, int ApprovedValue, int DeletedValue)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Safe In No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInDate"].Header).Caption = (GlobalVariables.IsArabic ? "التاريخ" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SafeInDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
