...
public class frmCustodySearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmCustodySearchReport()
	{
		InitializeComponent();
	}

	public frmCustodySearchReport(DataTable dt, string idColumnName, int ApprovedValue, int DeletedValue)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Custody No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustodyDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SubAccountName"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["SubAccountName"].Header).Caption = (GlobalVariables.IsArabic ? "اسم الموظف" : "Employee");
