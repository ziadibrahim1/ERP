...
public class frmProductDeliveredToClient : frmPosted
{
	private IContainer components = null;

	public frmProductDeliveredToClient()
	{
		InitializeComponent();
	...
	{
		dtsource = ProductMaintenance.SelectByDeliveredToClient(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ProductMaintenanceNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ProductMaintenanceDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpectedDeliveryDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpectedDeliveryDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ التسليم" : "Delivery Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpectedDeliveryDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClientDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ تسليم العميل" : "Client Delivery Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClientDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClient"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClient"].Header).Caption = "";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClient"].Width = (int)((double)((Control)(object)ULGData).Width * 0.05);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClient"].AllowRowFiltering = (DefaultableBoolean)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClient"].Header.CheckBoxVisibility = (HeaderCheckBoxVisibility)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DeliveredToClient"].Header.CheckBoxAlignment = (HeaderCheckBoxAlignment)2;
	}

	public override void SelectFullRow()
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "DeliveredToClient" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "DeliveredToClientDate")
		{
	...
	public override void SaveData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["DeliveredToClient"].Value.Equals(true))
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["ProductMaintenanceID"].Value.ToString() + ",";
				string productMaintenanceID = ((UltraGridBase)ULGData).Rows[i].Cells["ProductMaintenanceID"].Value.ToString();
				if (((UltraGridBase)ULGData).Rows[i].Cells["DeliveredToClientDate"].Value == DBNull.Value)
				{
					GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "برجاء ادخال التاريخ" : "Please insert the date of delivery");
					ULGData.ActiveCell = ((UltraGridBase)ULGData).Rows[i].Cells["DeliveredToClientDate"];
					return;
				}
				ProductMaintenance.SetDeliveredToClient(productMaintenanceID, "1", ((UltraGridBase)ULGData).Rows[i].Cells["DeliveredToClientDate"].Value.ToString(), GlobalVariables.UserID);
			}
		}
		if (text != ",")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد إصلاحات لإعتمادها " : "There are No Requests to Approve");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmProductMaintenance frmProductMaintenance2 = new frmProductMaintenance(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ProductMaintenanceID"].Value.ToString()));
			frmProductMaintenance2.Size = new Size(base.Width, base.Height);
			frmProductMaintenance2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmProductMaintenance2.lblTitle).Text = (GlobalVariables.IsArabic ? "الاصـلاحــــات" : "Product Maintenance");
			frmProductMaintenance2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.ProductMaintenanceReport(1, 1, 1, 1, 0, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["ProductMaintenanceID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["ProductMaintenanceID"].Value.ToString())
				{
					((UltraGridBase)ULGData).Rows[j].Cells["DeliveredToClient"].Value = true;
				}
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Production.Transactions.frmProductDeliveredToClient));
		...
		base.Name = "frmApprovingProductMaintenanceDeliveredToClient";
