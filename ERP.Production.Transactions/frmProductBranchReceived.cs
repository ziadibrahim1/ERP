...
public class frmProductBranchReceived : frmPosted
{
	private IContainer components = null;

	public frmProductBranchReceived()
	{
		InitializeComponent();
	...
	{
		dtsource = ProductMaintenance.SelectByBranchReceived(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceived"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceived"].Header).Caption = "";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceived"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceived"].AllowRowFiltering = (DefaultableBoolean)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceived"].Header.CheckBoxVisibility = (HeaderCheckBoxVisibility)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["BranchReceived"].Header.CheckBoxAlignment = (HeaderCheckBoxAlignment)2;
	}

	public override void SelectFullRow()
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "BranchReceived" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "BranchReceivedDate")
		{
	...
	public override void SaveData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["BranchReceived"].Value.Equals(true))
			{
				...
				ProductMaintenance.SetBranchReceived(productMaintenanceID, "1", ((UltraGridBase)ULGData).Rows[i].Cells["BranchReceivedDate"].Value.ToString(), GlobalVariables.UserID);
			}
		}
		if (text != ",")
		{
	...
	{
		DataTable dataTable = SearchFunctions.ProductMaintenanceReport(1, 1, 1, 0, 0, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				...
				{
					((UltraGridBase)ULGData).Rows[j].Cells["BranchReceived"].Value = true;
				}
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Production.Transactions.frmProductBranchReceived));
		...
		base.Name = "frmApprovingProductMaintenanceSentToMaintenance";
