...
public class frmProductMaintenanceReceived : frmPosted
{
	private IContainer components = null;

	public frmProductMaintenanceReceived()
	{
		InitializeComponent();
	...
	{
		dtsource = ProductMaintenance.SelectByMaintenanceReceived(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpectedDeliveryDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ التسليم المتوقع" : "Expected Delivery Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpectedDeliveryDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["MaintenanceReceived"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["MaintenanceReceived"].Header).Caption = "";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["MaintenanceReceived"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["MaintenanceReceived"].AllowRowFiltering = (DefaultableBoolean)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["MaintenanceReceived"].Header.CheckBoxVisibility = (HeaderCheckBoxVisibility)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["MaintenanceReceived"].Header.CheckBoxAlignment = (HeaderCheckBoxAlignment)2;
	}

	public override void SelectFullRow()
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "MaintenanceReceived" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "MaintenanceReceivedDate")
		{
	...
	public override void SaveData()
	{
		...
		{
			if (((UltraGridBase)ULGData).Rows[i].Cells["MaintenanceReceived"].Value.Equals(true))
			{
				...
				ProductMaintenance.SetMaintenanceReceived(productMaintenanceID, "1", ((UltraGridBase)ULGData).Rows[i].Cells["MaintenanceReceivedDate"].Value.ToString(), GlobalVariables.UserID);
			}
		}
		if (text != ",")
		{
		...
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد إصلاحات لإعتمادها " : "There are No Requests to be Approved");
		}
	}
	...
	{
		DataTable dataTable = SearchFunctions.ProductMaintenanceReport(1, 0, 0, 0, 0, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				...
				{
					((UltraGridBase)ULGData).Rows[j].Cells["MaintenanceReceived"].Value = true;
				}
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Production.Transactions.frmProductMaintenanceReceived));
		...
		base.Name = "frmApprovingProductMaintenanceMaintenanceReceived";
