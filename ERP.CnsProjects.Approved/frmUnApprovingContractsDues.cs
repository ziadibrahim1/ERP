...
namespace ERP.CnsProjects.Approved;

public class frmUnApprovingContractsDues : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingContractsDues()
	{
		InitializeComponent();
	...
	{
		dtsource = ContractsDues.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueCode"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueCode"].Header).Caption = (GlobalVariables.IsArabic ? "رقم المستخلص" : "ContractDue No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueCode"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ المستخلص" : "ContractDue Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDueDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractCode"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractCode"].Header).Caption = (GlobalVariables.IsArabic ? "المقايسه" : "Contract");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["ContractDueID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			ContractsDues.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا يوجد مستخلصات  لفك إعتمادها " : "There are No Contract Dues to be UnApproved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmContractsDues frmContractsDues2 = new frmContractsDues(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ContractDueID"].Value.ToString()));
			frmContractsDues2.Size = new Size(base.Width, base.Height);
			frmContractsDues2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmContractsDues2.lblTitle).Text = (GlobalVariables.IsArabic ? "المستخلص" : "Contract Due");
	...
	{
		DataTable dataTable = SearchFunctions.CnsContractsDuesReport(1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["ContractDueID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["ContractDueID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.CnsProjects.Approved.frmUnApprovingContractsDues));
		...
		base.Name = "frmUnApprovingContractsDues";
