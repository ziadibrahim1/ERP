...
public class frmUnApprovingContracts : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingContracts()
	{
		InitializeComponent();
		NoCol = "ContractCode";
	...
	{
		dtsource = Contracts.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractCode"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractCode"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ" : "Contract Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ContractDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["ContractID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			Contracts.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا يوجد مقايسات لإعتمادها " : "There are No Contracts to be Approved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmContracts frmContracts2 = new frmContracts(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ContractID"].Value.ToString()));
			frmContracts2.Size = new Size(base.Width, base.Height);
			frmContracts2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmContracts2.lblTitle).Text = (GlobalVariables.IsArabic ? "المقايسه" : "Contract");
	...
	{
		DataTable dataTable = SearchFunctions.CnsContractsReport(1, 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["ContractID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["ContractID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.CnsProjects.Approved.frmUnApprovingContracts));
		...
		base.Name = "frmUnApprovingContracts";
