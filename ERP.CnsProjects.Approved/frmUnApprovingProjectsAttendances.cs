...
public class frmUnApprovingProjectsAttendances : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingProjectsAttendances()
	{
		InitializeComponent();
	...
	{
		dtsource = ProjectsAttendances.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ProjectAttendanceCode"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ProjectAttendanceCode"].Header).Caption = (GlobalVariables.IsArabic ? "رقم" : "No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ProjectAttendanceCode"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AttendanceDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AttendanceDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ" : "Attendance Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AttendanceDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["ProjectAttendanceID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			ProjectsAttendances.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا يوجد حضور  لفك إعتماده " : "There are No Attendance to be UnApproved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmProjectAttendance frmProjectAttendance2 = new frmProjectAttendance(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ProjectAttendanceID"].Value.ToString()));
			frmProjectAttendance2.Size = new Size(base.Width, base.Height);
			frmProjectAttendance2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmProjectAttendance2.lblTitle).Text = (GlobalVariables.IsArabic ? "حضور العمال" : "Project Attendance");
			frmProjectAttendance2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.CnsProjectAttendanceReport(1, 0, IsFromServer: false);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["ProjectAttendanceID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["ProjectAttendanceID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.CnsProjects.Approved.frmUnApprovingProjectsAttendances));
		...
		base.Name = "frmUnApprovingProjectsAttendances";
