...
public class frmCustomersReservationsSearch : frmSearch
{
	private IContainer components = null;

	public frmCustomersReservationsSearch()
	{
		InitializeComponent();
	}

	public frmCustomersReservationsSearch(DataTable dt, string idColumnName, int ApprovedValue, int DeletedValue)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Invoice No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["CustomerReservationNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReceivingDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReceivingDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ التسليم" : "Receiving Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ReceivingDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
