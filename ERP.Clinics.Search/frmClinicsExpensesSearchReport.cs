...
public class frmClinicsExpensesSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmClinicsExpensesSearchReport()
	{
		InitializeComponent();
	}

	public frmClinicsExpensesSearchReport(DataTable dt, string idColumnName, int ApprovedValue, int DeletedValue)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpenseNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Expense No");
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpenseDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpenseDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpenseDate"].MaskInput = "dd/mm/yyyy hh:mm:ss tt";
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ExpenseDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.13);
