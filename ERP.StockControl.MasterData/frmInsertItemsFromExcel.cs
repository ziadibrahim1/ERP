...
public class frmInsertItemsFromExcel : frmBase
{
	...
	public frmInsertItemsFromExcel()
	{
		InitializeComponent();
	}
	...
	private void btnSave_Click(object sender, EventArgs e)
	{
		...
		dtXYItems.Columns.Add("BarCode", typeof(string));
		for (int i = 0; i < dtXYCrossGrid.Rows.Count; i++)
		{
			for (int j = 1; j < dtXYCrossGrid.Columns.Count; j++)
			{
				if (dtXYCrossGrid.Rows[i][j] != DBNull.Value && dtXYCrossGrid.Rows[i][j].ToString() != "" && dtXYCross.Select(" X= " + dtXYCrossGrid.Rows[i]["SPH/CYL"].ToString() + " And Y= " + dtXYCrossGrid.Columns[j].ToString()).Length != 0)
				{
					dtXYItems.Rows.Add(int.Parse(dtXYCross.Select(" X= " + dtXYCrossGrid.Rows[i]["SPH/CYL"].ToString() + " And Y= " + dtXYCrossGrid.Columns[j].ToString())[0]["ItemID"].ToString()), dtXYCrossGrid.Rows[i][j].ToString());
				}
			}
		}
		for (int k = 0; k < dtXYItems.Rows.Count; k++)
		{
			if (dtXYItems.Rows[k]["BarCode"].ToString().Trim().Contains(char.Parse(GlobalFunctions.GetDefault("BarcodeQuantitySeparator")).ToString()))
			{
			...
			if (dtXYItems.Rows[k]["BarCode"].ToString().Trim().Contains(char.Parse(GlobalFunctions.GetDefault("BarcodeBatchNoSeparator")).ToString()))
			{
			...
			if (dtXYItems.Rows[k]["BarCode"].ToString().Trim().Contains(char.Parse(GlobalFunctions.GetDefault("BarcodeSizeSeparator")).ToString()))
			{
			...
			if (dtXYItems.Rows[k]["BarCode"].ToString().Trim().Contains(char.Parse(GlobalFunctions.GetDefault("BarcodeColorSeparator")).ToString()))
			{
			...
			for (int l = 0; l < dtXYItems.Rows.Count; l++)
			{
				if (k != l && dtXYItems.Rows[k]["BarCode"].ToString().Trim() == dtXYItems.Rows[l]["BarCode"].ToString().Trim())
				{
					GlobalVariables.InformationMB.Show("لا يمكن تكرار الباركود ", "Cannot Duplicate The Same BarCode");
					return;
				}
			}
		}
		DataTable dataTable = Items.CheckForBarcodes(dtXYItems, GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		if (dataTable.Rows.Count > 0 && int.Parse(dataTable.Rows[0]["DuplicateCount"].ToString()) > 0)
		{
			string text = (GlobalVariables.IsArabic ? " يوجد تكرار للباركود ف الاصناف الاتيه" : "There Are Duplicate Barcodes :");
			for (int m = 0; m < dataTable.Rows.Count; m++)
			{
				DataRow dataRow = dtXYCross.Select("ItemID = " + dataTable.Rows[m]["ItemID"].ToString())[0];
				text = text + " \n " + (GlobalVariables.IsArabic ? string.Concat("لل Sph = ", dataRow["X"], " و Cly =  ", dataRow["Y"]) : string.Concat(" For Item With Sph = ", dataRow["X"], " And Cly = ", dataRow["Y"]));
			}
			GlobalVariables.InformationMB.Show(text, text);
			return;
		}
		string text2 = "";
		for (int n = 0; n < dtXYItems.Rows.Count; n++)
		{
			text2 = text2 + " Update SC_Items Set ItemBarCode= '" + dtXYItems.Rows[n]["BarCode"].ToString() + "' Where ItemID= " + dtXYItems.Rows[n]["ItemID"].ToString() + ";";
			text2 = text2 + " exec SP_Trans_Log " + GlobalVariables.UserID + ",'SC_Items'," + dtXYItems.Rows[n]["ItemID"].ToString() + ",'U' ;";
		}
		if (!(text2 != ""))
		{
			return;
		...
		try
		{
			...
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "تم الحفظ بنجاح  " : "Data Saved SuccessFully");
		}
		catch
		{
	...
	{
		base.PrepareData();
		dtItems = Items.FillComboDirectParents(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
	...
	public void GenerateCross()
	{
		...
		{
			dtXYCross = Items.XYCross(((TextEditorControlBase)cboItemName).Value.ToString(), ((TextEditorControlBase)cboSph).Value.ToString(), ((TextEditorControlBase)cboCyl).Value.ToString(), IsFromServer: true);
			...
			{
				dtXYCrossGrid.Columns.Add(dataTable2.Rows[i]["Y"].ToString(), typeof(string));
			}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.MasterData.frmInsertItemsFromExcel));
		...
		base.Name = "frmInsertItemsFromExcel";
