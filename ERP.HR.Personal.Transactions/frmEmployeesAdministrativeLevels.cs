...
public class frmEmployeesAdministrativeLevels : frmDetails
{
	private DataTable dtEmployees;

	private DataTable dtAdministrativeLevels;

	private DataTable dtEmployeeData;

	private ValueList vlAdministrativeLevel = new ValueList();

	private IContainer components = null;

	private UltraComboEditor cboEmployeeCode;

	public frmEmployeesAdministrativeLevels()
	{
	...
	public override void PrepareData()
	{
		...
		dtAdministrativeLevels = AdministrativeLevels.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		vlAdministrativeLevel.ValueListItems.Clear();
		for (int i = 0; i < dtAdministrativeLevels.Rows.Count; i++)
		{
			vlAdministrativeLevel.ValueListItems.Add(dtAdministrativeLevels.Rows[i]["AdministrativeLevelID"], dtAdministrativeLevels.Rows[i]["AdministrativeLevelName"].ToString());
		}
		dtDetails = EmployeesAdministrativeLevels.SelectBySubAccountID("0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		InitGrid();
	}
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["EmployeeAdministrativeLevelID"].DefaultCellValue = -1;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AdministrativeLevelID"].Header).Caption = (GlobalVariables.IsArabic ? "المستوى الادارى" : "Administrative level");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AdministrativeLevelID"].Hidden = false;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AdministrativeLevelID"].ValueList = (IValueList)(object)vlAdministrativeLevel;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AdministrativeLevelID"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
	...
	public override void DisplayData()
	{
		...
		((TextEditorControlBase)cboEmployeeCode).ValueChanged += cboEmployeeCode_ValueChanged;
		dtDetails = EmployeesAdministrativeLevels.SelectBySubAccountID(((TextEditorControlBase)cboHeader).Value.ToString(), GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		...
		if (dtDetails.Rows.Count == 0 && dtEmployeeData.Rows.Count > 0)
		{
			...
			dataRow["EmployeeAdministrativeLevelID"] = "-1";
			dataRow["SubAccountID"] = ((TextEditorControlBase)cboHeader).Value;
			dataRow["AdministrativeLevelID"] = dtEmployeeData.Rows[0]["AdministrativeLevelID"];
	...
	public override bool ValidateData()
	{
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			if (((UltraGridBase)ULGData).Rows[i].Cells["AdministrativeLevelID"].Value == DBNull.Value)
			{
				GlobalVariables.InformationMB.Show("برجاء إختيار المستوى الادارى", "Please Select Administrative Level");
				((GridItemBase)((UltraGridBase)ULGData).Rows[i].Cells["AdministrativeLevelID"]).Selected = true;
				return false;
			}
		}
	...
	private void InitializeComponent()
	{
		...
		((System.Windows.Forms.Control)(object)base.cboHeader).Location = new System.Drawing.Point(502, 68);
		((System.Windows.Forms.Control)(object)base.cboHeader).Size = new System.Drawing.Size(227, 24);
		...
		((System.Windows.Forms.Control)(object)this.cboEmployeeCode).Location = new System.Drawing.Point(399, 68);
		...
		base.Name = "frmEmployeesAdministrativeLevels";
