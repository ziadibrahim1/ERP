...
public class frmEmployeesDegrees : frmDetails
{
	private DataTable dtEmployees;

	private DataTable dtDegrees;
	...
	public override void PrepareData()
	{
		...
		dtDegrees = Degrees.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		vlDegrees.ValueListItems.Clear();
		for (int i = 0; i < dtDegrees.Rows.Count; i++)
		{
			vlDegrees.ValueListItems.Add(dtDegrees.Rows[i]["DegreeID"], dtDegrees.Rows[i]["DegreeName"].ToString());
		}
		dtDetails = EmployeesDegrees.SelectBySubAccountID("0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		InitGrid();
	}
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["EmployeeDegreeID"].DefaultCellValue = -1;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DegreeID"].Header).Caption = (GlobalVariables.IsArabic ? "الدرجة الوظيفية" : "Degree Name");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DegreeID"].Hidden = false;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DegreeID"].ValueList = (IValueList)(object)vlDegrees;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["DegreeID"].Width = (int)((double)((Control)(object)ULGData).Width * 0.2);
	...
	public override void DisplayData()
	{
		...
		((TextEditorControlBase)cboEmployeeCode).ValueChanged += cboEmployeeCode_ValueChanged;
		dtDetails = EmployeesDegrees.SelectBySubAccountID(((TextEditorControlBase)cboHeader).Value.ToString(), GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		...
		if (dtDetails.Rows.Count == 0 && dtEmployeeData.Rows.Count > 0)
		{
			...
			dataRow["EmployeeDegreeID"] = "-1";
			dataRow["SubAccountID"] = ((TextEditorControlBase)cboHeader).Value;
			dataRow["DegreeID"] = dtEmployeeData.Rows[0]["DegreeID"];
	...
	public override bool ValidateData()
	{
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			if (((UltraGridBase)ULGData).Rows[i].Cells["DegreeID"].Value == DBNull.Value)
			{
				GlobalVariables.InformationMB.Show("برجاء إختيار الدرجة الوظيفية", "Please Select Degree Name");
				((GridItemBase)((UltraGridBase)ULGData).Rows[i].Cells["DegreeID"]).Selected = true;
				return false;
			}
		}
	...
	private void InitializeComponent()
	{
		...
		((System.Windows.Forms.Control)(object)base.cboHeader).Location = new System.Drawing.Point(468, 68);
		...
		((System.Windows.Forms.Control)(object)this.cboEmployeeCode).Location = new System.Drawing.Point(365, 68);
