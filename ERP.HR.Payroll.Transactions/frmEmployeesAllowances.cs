...
public class frmEmployeesAllowances : frmDetails
{
	private DataTable dtEmployees;

	private DataTable dtAllowances;
	...
	public frmEmployeesAllowances()
	{
		//IL_0001: Unknown result type (might be due to invalid IL or missing references)
		//IL_000b: Expected O, but got Unknown
		InitializeComponent();
		((Control)(object)lblHeader).Text = (GlobalVariables.IsArabic ? "البدل" : "Allowance");
	}

	public override void PrepareData()
	{
		dtAllowances = Allowances.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		GlobalFunctions.FillCombo(cboHeader, dtAllowances, "AllowanceID", "AllowanceName");
		...
		dtDetails = EmployeesAllowances.SelectByAllowanceID("0", rbIsSalary.Checked ? "1" : "0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		InitGrid();
	}
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["EmployeeAllowanceID"].DefaultCellValue = -1;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["EmployeeAllowanceID"].DefaultCellValue = -1;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IsByDate"].DefaultCellValue = false;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AllowanceID"].DefaultCellValue = ((cboHeader.SelectedIndex == -1) ? DBNull.Value : ((TextEditorControlBase)cboHeader).Value);
	...
	public override void DisplayData()
	{
		...
		{
			base.DisplayData();
			dtDetails = EmployeesAllowances.SelectByAllowanceID(((TextEditorControlBase)cboHeader).Value.ToString(), rbIsSalary.Checked ? "1" : "0", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
			InitGrid();
			CalculateAmount();
		}
	}
	...
	public override void SaveData()
	{
		...
		try
		{
			...
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				((UltraGridBase)ULGData).Rows[i].Cells["AllowanceID"].Value = ((TextEditorControlBase)cboHeader).Value.ToString();
				((UltraGridBase)ULGData).Rows[i].Cells["BranchID"].Value = GlobalVariables.CurrentBranchID;
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["EmployeeAllowanceID"].Value.ToString() + ",";
			}
			Main.SyncDeleteForUpdate("HR_EmployeesAllowances", "AllowanceID", ((TextEditorControlBase)cboHeader).Value.ToString(), "EmployeeAllowanceID", rbIsSalary.Checked ? " And  IsSalary=1 " : " And  IsSalary=0 ", text, IsFromServer: true);
			if (((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count > 0)
			{
				EmployeesAllowances.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID, IsFromServer: true);
			}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.HR.Payroll.Transactions.frmEmployeesAllowances));
		...
		base.Name = "frmEmployeesAllowances";
