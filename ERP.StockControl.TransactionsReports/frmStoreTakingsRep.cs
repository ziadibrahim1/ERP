...
public class frmStoreTakingsRep : frmReportTree2010
{
	private IContainer components = null;

	public frmStoreTakingsRep()
	{
		TreeItems2ParentIDCol = "ParentID";
		TreeItems2IDCol = "StoreTakingID";
		TreeItems2NameCol = "StoreTackingNo";
	...
	{
		GetBranches();
		dtItems2 = StoreTakings.FillRepTree(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"));
		if (dtItems2 != null)
		{
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SC_StoreTakings_A.rpt" : "Rep_SC_StoreTakings_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SC_StoreTakings_A_nologo.rpt" : "Rep_SC_StoreTakings_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		GlobalVariables.ReportDocument.SetParameterValue("@StoreTakingIDs", Items2);
		...
		if (GroupNamePath != "")
		{
			...
			if (text.IndexOf(TreeItems2IDCol) >= 0)
			{
				string s = GroupNamePath.Replace(text, "").Replace(",", "").Replace("[", "")
					.Replace("]", "");
				frmStoreTakings frmStoreTakings2 = new frmStoreTakings(int.Parse(s));
				frmStoreTakings2.StartPosition = FormStartPosition.CenterParent;
				((Control)(object)frmStoreTakings2.lblTitle).Text = (GlobalVariables.IsArabic ? "جــــــرد الـــمــخــازن" : "Store Takings");
	...
	public override void btnItems2Search_Click(object sender, EventArgs e)
	{
		...
		{
			TreeItems2.GetNodeByKey(dtSearchResult.Rows[i]["StoreTakingID"].ToString()).CheckedState = CheckState.Checked;
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.TransactionsReports.frmStoreTakingsRep));
