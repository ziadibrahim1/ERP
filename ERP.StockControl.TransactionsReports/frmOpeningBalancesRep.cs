...
public class frmOpeningBalancesRep : frmReportTree2010
{
	private IContainer components = null;

	public frmOpeningBalancesRep()
	{
		TreeItems2ParentIDCol = "ParentID";
		TreeItems2IDCol = "OpeningBalanceID";
		TreeItems2NameCol = "OpeningBalanceCode";
	...
	{
		GetBranches();
		dtItems2 = OpeningBalances.FillRepTree(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"));
		if (dtItems2 != null)
		{
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SC_OpeningBalances_A.rpt" : "Rep_SC_OpeningBalances_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SC_OpeningBalances_A_nologo.rpt" : "Rep_SC_OpeningBalances_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		GlobalVariables.ReportDocument.SetParameterValue("@OpeningBalanceIDs", Items2);
		...
		if (GroupNamePath != "")
		{
			...
			if (text.IndexOf(TreeItems2IDCol) >= 0)
			{
				string s = GroupNamePath.Replace(text, "").Replace(",", "").Replace("[", "")
					.Replace("]", "");
				frmOpeningBalances frmOpeningBalances2 = new frmOpeningBalances(int.Parse(s));
				frmOpeningBalances2.StartPosition = FormStartPosition.CenterParent;
				((Control)(object)frmOpeningBalances2.lblTitle).Text = (GlobalVariables.IsArabic ? "الارصـدة الافـتـتـاحيـة" : "Opening Balances");
	...
	public override void btnItems2Search_Click(object sender, EventArgs e)
	{
		...
		{
			TreeItems2.GetNodeByKey(dtSearchResult.Rows[i]["OpeningBalanceID"].ToString()).CheckedState = CheckState.Checked;
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.TransactionsReports.frmOpeningBalancesRep));
		...
		base.Name = "frmOpeningBalancesRep";
