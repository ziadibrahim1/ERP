...
public class frmStoresSettlementVouchersRep : frmReportTree2010
{
	private IContainer components = null;

	public frmStoresSettlementVouchersRep()
	{
		TreeItems2ParentIDCol = "ParentID";
		TreeItems2IDCol = "SettlementVoucherID";
		TreeItems2NameCol = "SettlementVoucherNo";
	...
	{
		GetBranches();
		dtItems2 = StoresSettlementVouchers.FillRepTree(Branches, dtpFromDate.DateTime.Date.ToString(GlobalVariables.DateShortFormate), dtpToDate.DateTime.ToString("MM/dd/yyyy 23:59:59"));
		if (dtItems2 != null)
		{
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SC_StoresSettlementVouchers_A.rpt" : "Rep_SC_StoresSettlementVouchers_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_SC_StoresSettlementVouchers_A_nologo.rpt" : "Rep_SC_StoresSettlementVouchers_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		GlobalVariables.ReportDocument.SetParameterValue("@StoresSettlementVoucherIDs", Items2);
		...
		if (GroupNamePath != "")
		{
			...
			if (text.IndexOf(TreeItems2IDCol) >= 0)
			{
				string s = GroupNamePath.Replace(text, "").Replace(",", "").Replace("[", "")
					.Replace("]", "");
				frmStoresSettlementVouchers frmStoresSettlementVouchers2 = new frmStoresSettlementVouchers(int.Parse(s));
				frmStoresSettlementVouchers2.StartPosition = FormStartPosition.CenterParent;
				((Control)(object)frmStoresSettlementVouchers2.lblTitle).Text = (GlobalVariables.IsArabic ? "اذونــــات تــســـويــة" : "Stores Settlement Vouchers");
	...
	public override void btnItems2Search_Click(object sender, EventArgs e)
	{
		...
		{
			TreeItems2.GetNodeByKey(dtSearchResult.Rows[i]["SettlementVoucherID"].ToString()).CheckedState = CheckState.Checked;
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.TransactionsReports.frmStoresSettlementVouchersRep));
		...
		base.Name = "frmStoresSettlementVouchersRep";
