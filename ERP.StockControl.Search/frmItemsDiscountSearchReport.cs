...
public class frmItemsDiscountSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmItemsDiscountSearchReport()
	{
		InitializeComponent();
	}

	public frmItemsDiscountSearchReport(DataTable dt, string idColumnName)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemDiscountNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemDiscountNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن" : "NO");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["ItemDiscountNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.13) - GlobalVariables.ScrollWidth;
