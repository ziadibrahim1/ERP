...
public class frmWareHouseReceiptSearchReport : frmSearchReport
{
	private IContainer components = null;

	public frmWareHouseReceiptSearchReport()
	{
		InitializeComponent();
	}

	public frmWareHouseReceiptSearchReport(DataTable dt, string idColumnName)
		: this()
	{
		dtSource = dt;
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["WareHouseReceiptNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["WareHouseReceiptNo"].Header).Caption = (GlobalVariables.IsArabic ? "NO" : "NO");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["WareHouseReceiptNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
