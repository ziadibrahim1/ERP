...
public class frmUnApprovingInvoices : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingInvoices()
	{
		InitializeComponent();
		NoCol = "InvoiceNo";
	...
	{
		dtsource = Invoices.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["InvoiceNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Invoice No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["InvoiceNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.125) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["InvoiceDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["InvoiceDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["InvoiceDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotaionNo"].Header).Caption = (GlobalVariables.IsArabic ? "عرض سعر" : "Quotation No");
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["InvoiceID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			Invoices.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد فواتير لفك إعتمادها " : "There are No Invoices to UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmInvoices frmInvoices3 = new frmInvoices(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["InvoiceID"].Value.ToString()));
			frmInvoices3.Size = new Size(base.Width, base.Height);
			frmInvoices3.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmInvoices3.lblTitle).Text = (GlobalVariables.IsArabic ? "الفواتير " : "Invoices");
			frmInvoices3.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.SLNInvoicesReport("-1", 0);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["InvoiceID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["InvoiceID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sling.Approved.frmUnApprovingInvoices));
		...
		base.Name = "frmUnApprovingInvoices";
