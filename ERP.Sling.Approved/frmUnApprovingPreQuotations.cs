...
public class frmUnApprovingPreQuotations : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingPreQuotations()
	{
		InitializeComponent();
	...
	{
		dtsource = PreQuotations.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PreQuotationNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Quotation No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PreQuotationNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.125) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PreQuotationDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PreQuotationDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["PreQuotationDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.075);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["PreQuotationID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			PreQuotations.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد عروض أسعار لفك إعتمادها " : "There are No PreQuotations to be UnApproved");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmSLNPreQuotations frmSLNPreQuotations2 = new frmSLNPreQuotations(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["PreQuotationID"].Value.ToString()));
			frmSLNPreQuotations2.Size = new Size(base.Width, base.Height);
			frmSLNPreQuotations2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmSLNPreQuotations2.lblTitle).Text = (GlobalVariables.IsArabic ? "عــرض ســعـر " : "Pre Quotations");
			frmSLNPreQuotations2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.SLNPreQuotationsReport(1, 0, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["PreQuotationID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["PreQuotationID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sling.Approved.frmUnApprovingPreQuotations));
		...
		base.Name = "frmUnApprovingPreQuotations";
