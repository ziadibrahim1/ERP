...
using ERP.Classes;
using ERP.Sling.Transactions;
using Infragistics.Shared;
using Infragistics.Win;
using Infragistics.Win.UltraWinEditors;
using Infragistics.Win.UltraWinGrid;

namespace ERP.Sling.Approved;

public class frmIssuingQuotations : frmPosted
{
	private IContainer components = null;

	public frmIssuingQuotations()
	{
		InitializeComponent();
		NoCol = "QuotaionNo";
	}
	...
	{
		dtsource = Quotations.SelectByIssued(GlobalVariables.BranchIDs, "0", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["QuotaionNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.15) - GlobalVariables.ScrollWidth;
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IssueDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IssueDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الصرف" : "Issue Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["IssueDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
		...
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Issued"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Issued"].Header).Caption = (GlobalVariables.IsArabic ? "تم الصرف" : "Issue");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Issued"].Width = (int)((double)((Control)(object)ULGData).Width * 0.05);
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Issued"].AllowRowFiltering = (DefaultableBoolean)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Issued"].Header.CheckBoxVisibility = (HeaderCheckBoxVisibility)2;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["Issued"].Header.CheckBoxAlignment = (HeaderCheckBoxAlignment)2;
	}

	public override void SelectFullRow()
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "Issued" && ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key != "IssueDate")
		{
	...
	public override void btnPost_Click(object sender, EventArgs e)
	{
		...
		{
			if (!((UltraGridBase)ULGData).Rows[i].Cells["Issued"].Value.Equals(true))
			{
				continue;
			}
			...
			try
			{
				MessageLog.DeleteByVoucherIDAndTransType(((UltraGridBase)ULGData).Rows[i].Cells["QuotationID"].Value.ToString(), "SlnMIV", "SlnMMIV");
				Quotations.SetIssued("1", DateTime.Parse(((UltraGridBase)ULGData).Rows[i].Cells["IssueDate"].Value.ToString()).ToString(GlobalVariables.DateLongFormate), "," + ((UltraGridBase)ULGData).Rows[i].Cells["QuotationID"].Value.ToString() + ",");
				string text = MessageLog.SelectByVoucherIDAndTransType(((UltraGridBase)ULGData).Rows[i].Cells["QuotationID"].Value.ToString(), "SlnMIV", "SlnMMIV", GlobalVariables.IsArabic ? "1" : "0");
				if (text != "")
				{
	...
	public override void ClickCellButton()
	{
		...
		{
			frmSLNQuotations frmSLNQuotations2 = new frmSLNQuotations(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["QuotationID"].Value.ToString()));
			frmSLNQuotations2.Size = new Size(base.Width, base.Height);
			frmSLNQuotations2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmSLNQuotations2.lblTitle).Text = (GlobalVariables.IsArabic ? "Order" : "Order");
			frmSLNQuotations2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.SLNQuotationsReport(0, 0, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["QuotationID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["QuotationID"].Value.ToString())
				{
					((UltraGridBase)ULGData).Rows[j].Cells["Issued"].Value = true;
				}
			}
		}
	}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.Sling.Approved.frmIssuingQuotations));
		...
		base.Name = "frmIssuingQuotations";
