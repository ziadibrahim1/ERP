...
public class frmOpeningBalances : frmHeaderDetails
{
	...
	public frmOpeningBalances()
	{
		...
		InitializeComponent();
		TableName = "SC_OpeningBalances";
		IDCol = "OpeningBalanceID";
		NoCol = "OpeningBalanceCode";
		DateCol = "OpeningBalanceDate";
	}

	public frmOpeningBalances(int ID)
		: this()
	{
		RowID = ID.ToString();
	}
	...
	public override void PrepareData()
	{
		...
		dtDetails = OpeningBalancesDetails.SelectByOpeningBalanceID("0", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["OpeningBalanceDetailID"].DefaultCellValue = -1;
	...
	public override void FillData()
	{
		...
		{
			DataTable dataTable = OpeningBalances.Select(RowID, GlobalVariables.BranchIDs, GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void DisplayData()
	{
		...
		if (drMaster != null)
		{
			...
			((Control)(object)txtCode).Text = drMaster["OpeningBalanceCode"].ToString();
			dtpDate.Value = (DateTime)drMaster["OpeningBalanceDate"];
			((Control)(object)txtNotes).Text = drMaster["Notes"].ToString();
			dtDetails = OpeningBalancesDetails.SelectByOpeningBalanceID(drMaster["OpeningBalanceID"].ToString(), GlobalVariables.IsArabic ? "1" : "0");
			((UltraGridBase)ULGData).DataSource = dtDetails;
			InitGrid();
	...
	public override void SetControls(bool NavMode)
	{
		...
		((Control)(object)btnSelectItems).Visible = Adding;
	...
	public override void ClearControls()
	{
		...
		((Control)(object)txtCode).Text = (Adding ? OpeningBalances.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID) : "");
	...
	public override bool ValidateData()
	{
		...
		if (Main.CheckForValueByBranchIDAndFiscalYearID("SC_OpeningBalances", "OpeningBalanceCode", ((Control)(object)txtCode).Text, Adding ? "0" : drMaster["OpeningBalanceCode"].ToString(), GlobalVariables.CurrentBranchID, DateCol, dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), "") > 0)
		{
			string codeByBranchID = OpeningBalances.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				...
				{
					GlobalVariables.InformationMB.Show(" لا يمكن تكرار الصنف  مع نفس المخزن", "Cannot Duplicate The Same Item With the Same Store");
					ULGData.ActiveCell = ((UltraGridBase)ULGData).Rows[i].Cells["ItemID"];
					return false;
				}
			}
	...
	public override void AddData()
	{
		...
		try
		{
			int num = OpeningBalances.Insert_Update("-1", ((Control)(object)txtCode).Text, dtpDate.DateTime.AddSeconds(1.0).ToString(GlobalVariables.DateLongFormate), ((Control)(object)txtNotes).Text, "0", "0", "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				((UltraGridBase)ULGData).Rows[i].Cells["OpeningBalanceDetailID"].Value = -1;
				((UltraGridBase)ULGData).Rows[i].Cells["OpeningBalanceID"].Value = num;
				...
				((UltraGridBase)ULGData).Rows[i].Cells["VoucherDate"].Value = dtpDate.DateTime.AddSeconds(1.0);
			}
			OpeningBalancesDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
	...
	public override void UpdateData()
	{
		...
		try
		{
			int num = OpeningBalances.Insert_Update(drMaster["OpeningBalanceID"].ToString(), ((Control)(object)txtCode).Text, dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), ((Control)(object)txtNotes).Text, bool.Parse(drMaster["Approved"].ToString()) ? "1" : "0", "1", bool.Parse(drMaster["Deleted"].ToString()) ? "1" : "0", GlobalVariables.CurrentBranchID, GlobalVariables.UserID);
			MessageLog.DeleteByVoucherIDAndTransType(num.ToString(), "OPN", "OPN");
			string text = ",";
			for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
			{
				((UltraGridBase)ULGData).Rows[i].Cells["OpeningBalanceID"].Value = num;
				...
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["OpeningBalanceDetailID"].Value.ToString() + ",";
			}
			Main.DeleteForUpdate("SC_OpeningBalancesDetails", "OpeningBalanceID", drMaster["OpeningBalanceID"].ToString(), "OpeningBalanceDetailID", text);
			OpeningBalancesDetails.Insert_UpdateByTable((DataTable)((UltraGridBase)ULGData).DataSource, GlobalVariables.UserID);
			string text2 = MessageLog.SelectByVoucherIDAndTransType(num.ToString(), "OPN", "OPN", GlobalVariables.IsArabic ? "1" : "0");
			if (text2 != "")
			{
	...
	public override void DeleteData()
	{
		...
		try
		{
			MessageLog.DeleteByVoucherIDAndTransType(drMaster["OpeningBalanceID"].ToString(), "OPN", "OPN");
			OpeningBalances.DeleteVirtual(drMaster["OpeningBalanceID"].ToString(), GlobalVariables.UserID);
			OpeningBalancesDetails.DeleteVirtualByOpeningBalanceID(drMaster["OpeningBalanceID"].ToString(), GlobalVariables.UserID);
			string text = MessageLog.SelectByVoucherIDAndTransType(drMaster["OpeningBalanceID"].ToString(), "OPN", "OPN", GlobalVariables.IsArabic ? "1" : "0");
			if (text != "")
			{
	...
	public override void btnPrintClick()
	{
		if (RowID != "")
		{
			...
			{
				GlobalVariables.ReportDocument.Load(GlobalVariables.ReportsPath + (GlobalVariables.IsArabic ? "Rep_SC_OpeningBalances_A.rpt" : "Rep_SC_OpeningBalances_E.rpt"));
			}
			frmReporViwer frmReporViwer2 = new frmReporViwer();
			GlobalVariables.ReportDocument.SetParameterValue("@OpeningBalanceIDs", "," + RowID + ",");
	...
	{
		dtSearchResult = SearchFunctions.OpeningBalancesReport(-1);
		if (dtSearchResult.Rows.Count > 0)
		{
			RowID = dtSearchResult.Rows[0]["OpeningBalanceID"].ToString();
			FillData();
		}
	}
	...
	public override void btnUpdateClick()
	{
		...
		if (!ClosedPeriod)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				DataView dataView = new DataView(dtMinAllowedTransDate);
				dataView.RowFilter = " StoreID= " + ((UltraGridBase)ULGData).Rows[j].Cells["StoreID"].Value.ToString();
				if (dataView.ToTable().Rows.Count <= 0 || dtpDate.DateTime < DateTime.Parse(dataView[0]["Date"].ToString()))
				{
				}
			}
		}
	...
	private void btnSelectItems_Click(object sender, EventArgs e)
	{
		...
		DataTable dataTable = OpeningBalancesDetails.SelectByItemIDs((((UltraToggleEditorBase)chkStore).Checked && cboStore.SelectedIndex > -1) ? ((TextEditorControlBase)cboStore).Value.ToString() : "-1", text, GlobalVariables.IsArabic ? "1" : "0");
		...
		else
		{
			...
			{
				if (dtDetails.Select(" ItemID =" + dataTable.Rows[j]["ItemID"].ToString()).Length == 0)
				{
	...
	{
		if (((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "Average" || ((KeyedSubObjectBase)ULGData.ActiveCell.Column).Key == "Qty")
		{
	...
	{
		if (e.Row.IsAddRow && e.Row.Index > 0 && e.Row.Cells["ItemID"].Value == DBNull.Value && !((UltraGridBase)ULGData).Rows[e.Row.Index - 1].Cells["ItemID"].Value.Equals(DBNull.Value))
		{
			e.Row.Cells["ItemID"].Value = ((UltraGridBase)ULGData).Rows[e.Row.Index - 1].Cells["ItemID"].Value;
		}
	}
	...
	private void dtpDate_ValueChanged(object sender, EventArgs e)
	{
		if (Adding)
		{
			((Control)(object)txtCode).Text = OpeningBalances.GetCodeByBranchID(dtpDate.DateTime.ToString(GlobalVariables.DateShortFormate), GlobalVariables.CurrentBranchID);
		}
	}
		...
		for (int k = 0; k < frmXYSelection2.dtXYItems.Rows.Count; k++)
		{
			for (int l = 1; l < frmXYSelection2.dtXYItems.Columns.Count; l++)
			{
				if (frmXYSelection2.dtXYItems.Rows[k]["ItemID"].ToString() != "" && decimal.Parse(frmXYSelection2.dtXYItems.Rows[k]["Qty"].ToString()) > 0m && ((DataTable)((UltraGridBase)ULGData).DataSource).Select(" ItemID= " + frmXYSelection2.dtXYItems.Rows[k]["ItemID"].ToString()).Length == 0)
				{
					...
					((UltraGridBase)ULGData).ActiveRow.Cells["Average"].Value = frmXYSelection2.Price;
					if (UsingBatchNoAndValidityPeriod)
					{
						ValueList batchsValueList2 = getBatchsValueList(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["ItemID"].Value.ToString()));
						((UltraGridBase)ULGData).ActiveRow.Cells["BatchID"].ValueList = (IValueList)(object)batchsValueList2;
	...
	public override void ImportGridData()
	{
		...
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			if (dataTable.Rows[i][GlobalVariables.IsArabic ? "الباركود" : "BarCode"].ToString().Length == 0 || dataTable.Rows[i][GlobalVariables.IsArabic ? "الكمية" : "Qty"] == DBNull.Value || !(decimal.Parse(dataTable.Rows[i][GlobalVariables.IsArabic ? "الكمية" : "Qty"].ToString()) > 0m))
			{
				continue;
			}
		...
		for (int j = 0; j < dataTable.Rows.Count; j++)
		{
			if (dataTable.Rows[j][GlobalVariables.IsArabic ? "الباركود" : "BarCode"].ToString().Length != 0 && dataTable.Rows[j][GlobalVariables.IsArabic ? "الكمية" : "Qty"] != DBNull.Value && decimal.Parse(dataTable.Rows[j][GlobalVariables.IsArabic ? "الكمية" : "Qty"].ToString()) > 0m)
			{
				((UltraGridBase)ULGData).DisplayLayout.Override.AllowAddNew = (AllowAddNew)1;
				((UltraGridBase)ULGData).DisplayLayout.Bands[0].AddNew();
				...
				if (dataTable.Columns.Contains(GlobalVariables.IsArabic ? "المتوسط" : "Average") && decimal.TryParse(dataTable.Rows[j][GlobalVariables.IsArabic ? "المتوسط" : "Average"].ToString(), out var _))
				{
					((UltraGridBase)ULGData).ActiveRow.Cells["Average"].Value = dataTable.Rows[j][GlobalVariables.IsArabic ? "المتوسط" : "Average"];
				}
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.Transactions.frmOpeningBalances));
