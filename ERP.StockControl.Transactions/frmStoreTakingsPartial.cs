...
public class frmStoreTakingsPartial : frmHeaderDetails
{
	...
	private DataTable dtStoreItemsBalance = new DataTable();
	...
	public frmStoreTakingsPartial(int ID)
		: this()
	{
		RowID = ID.ToString();
	}
	...
	public override void SetControls(bool NavMode)
	{
		...
		((EditorButtonControlBase)txtBarCode).ReadOnly = NavMode || Updating;
	...
	public override bool ValidateData()
	{
		...
		if (num > 0)
		{
			GlobalVariables.QuestionMB.Show("برجاء الانتباه يوجد \r\n" + num + "\r\n صنف\r\n رصيدهم الفعلي صفر\r\nهل انت متأكد للحفظ؟", "There is " + num + " Item With Zero Balance Are You Sure To Save?");
			if (GlobalVariables.MessageBoxResult != 'Y')
			{
				return false;
			}
	...
	{
		object value = ((TextEditorControlBase)cboStore).Value;
		UsingColors = GlobalFunctions.GetOption("UsingItemsColors");
		UsingSizes = GlobalFunctions.GetOption("UsingItemsSizes");
		if (UsingColors)
		{
		...
		GlobalFunctions.FillCombo(cboStore, dataView.ToTable(), "StoreID", "StoreName");
		((TextEditorControlBase)cboStore).ValueChanged -= cboStore_ValueChanged;
		((TextEditorControlBase)cboStore).Value = value;
		((TextEditorControlBase)cboStore).ValueChanged += cboStore_ValueChanged;
		dtItems = Items.FillCombo("-1", "-1", "0", "-1", "-1", "0", "-1", "-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: false);
		vlItems.ValueListItems.Clear();
		vlBarCode.ValueListItems.Clear();
		for (int l = 0; l < dtItems.Rows.Count; l++)
		{
			if (dtItems.Rows[l]["ItemID"].ToString() == "288")
			{
	...
	private void cboStore_ValueChanged(object sender, EventArgs e)
	{
		...
		if (cboStore.SelectedIndex > -1)
		{
			...
			{
				dtStoreItemsBalance = StoreTakingsDetails.SelectItemBalanceByStoreID(((TextEditorControlBase)cboStore).Value.ToString(), "-1", dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), GlobalVariables.IsArabic ? "1" : "0");
				dtItemBalance = dtStoreItemsBalance.Clone();
				((UltraGridBase)ULGData).DataSource = dtItemBalance;
				InitGrid();
				((Control)(object)txtTotalQty).Text = "0";
			}
	...
	private bool ValidateItem(string ItemID)
	{
		if (cboStore.SelectedIndex > -1)
		{
			if (dtStoreItemsBalance != null)
			{
				bool flag = dtStoreItemsBalance.Select("ItemID = " + ItemID).Length != 0;
				if (!flag)
				{
					GlobalVariables.InformationMB.Show("هذا الصنف لم يتم إدخاله الى المخزن من قبل", "This Item Never Been In The Store Before.");
				}
				return flag;
			}
			dtStoreItemsBalance = StoreTakingsDetails.SelectItemBalanceByStoreID(((TextEditorControlBase)cboStore).Value.ToString(), "-1", dtpDate.DateTime.ToString(GlobalVariables.DateLongFormate), GlobalVariables.IsArabic ? "1" : "0");
			bool flag2 = dtStoreItemsBalance.Select("ItemID = " + ItemID).Length != 0;
			if (!flag2)
			{
				GlobalVariables.InformationMB.Show("هذا الصنف لم يتم إدخاله الى المخزن من قبل", "This Item Never Been In The Store Before.");
			}
		...
		GlobalVariables.InformationMB.Show("برجاء إختيار مخزن أولا", "Please Select Store First.");
		return false;
	}
	...
	public void AddItemInGid()
	{
		...
		if (flag)
		{
			...
			string text6 = dtItems.Select(" ItemBarCode = '" + text + "'")[0]["ItemID"].ToString();
			if (ValidateItem(text6))
			{
				...
				object value = (((UltraGridBase)ULGData).ActiveRow.Cells["ItemBarCode"].Value = text6);
				obj.Value = value;
				...
				((UltraGridBase)ULGData).ActiveRow.Cells["Average"].Value = 0;
				if (text4 != "")
				{
					((UltraGridBase)ULGData).ActiveRow.Cells["BatchID"].Value = text4;
				}
				((UltraGridBase)ULGData).ActiveRow.Cells["Qty"].Value = decimal.Parse(dtStoreItemsBalance.Select("ItemID = " + text6 + " And ColorID = " + text2 + " And ItemSizeID = " + text3 + ((text4 == "") ? "" : (" And BatchID = " + text4)) + " And UnitID = " + text5)[0]["Qty"].ToString());
				((UltraGridBase)ULGData).ActiveRow.Cells["OriginalQty"].Value = decimal.Parse(dtStoreItemsBalance.Select("ItemID = " + text6 + " And ColorID = " + text2 + " And ItemSizeID = " + text3 + ((text4 == "") ? "" : (" And BatchID = " + text4)) + " And UnitID = " + text5)[0]["OriginalQty"].ToString());
				ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
		...
		string text7 = dataRow2["ItemID"].ToString();
		if (ValidateItem(text7))
		{
			((UltraGridBase)ULGData).DisplayLayout.Override.AllowAddNew = (AllowAddNew)1;
			((UltraGridBase)ULGData).DisplayLayout.Bands[0].AddNew();
			ULGData.AfterCellUpdate -= new CellEventHandler(ULGData_AfterCellUpdate);
			UltraGridCell obj3 = ((UltraGridBase)ULGData).ActiveRow.Cells["ItemID"];
			object value = (((UltraGridBase)ULGData).ActiveRow.Cells["ItemBarCode"].Value = text7);
			obj3.Value = value;
			((UltraGridBase)ULGData).ActiveRow.Cells["ColorID"].Value = text2;
			((UltraGridBase)ULGData).ActiveRow.Cells["ItemSizeID"].Value = text3;
			...
			((UltraGridBase)ULGData).ActiveRow.Cells["Qty"].Value = decimal.Parse(dtStoreItemsBalance.Select("ItemID = " + text7 + " And ColorID = " + text2 + " And ItemSizeID = " + text3 + ((text4 == "") ? "" : (" And BatchID = " + text4)))[0]["Qty"].ToString());
			((UltraGridBase)ULGData).ActiveRow.Cells["OriginalQty"].Value = decimal.Parse(dtStoreItemsBalance.Select("ItemID = " + text7 + " And ColorID = " + text2 + " And ItemSizeID = " + text3 + ((text4 == "") ? "" : (" And BatchID = " + text4)))[0]["OriginalQty"].ToString());
			ULGData.AfterCellUpdate += new CellEventHandler(ULGData_AfterCellUpdate);
			rowIndex = ((UltraGridBase)ULGData).ActiveRow.Index;
			((UltraGridBase)ULGData).DisplayLayout.Override.AllowAddNew = (AllowAddNew)2;
			((UltraGridBase)ULGData).UpdateData();
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.StockControl.Transactions.frmStoreTakingsPartial));
		...
		base.Name = "frmStoreTakingsPartial";
