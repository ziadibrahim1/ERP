...
using ERP.Classes;
using ERP.FixedAssets.Transactions;
using Infragistics.Shared;
using Infragistics.Win;
using Infragistics.Win.UltraWinEditors;
using Infragistics.Win.UltraWinGrid;

namespace ERP.FixedAssets.Approved;

public class frmUnApprovingAssetsAcquisitions : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingAssetsAcquisitions()
	{
		InitializeComponent();
	...
	{
		dtsource = AssetsAcquisitions.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	{
		base.InitGrid();
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetAcquisitionNo"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetAcquisitionNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Asset Acquisitions No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetAcquisitionNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetAcquisitionDate"].Hidden = false;
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetAcquisitionDate"].Header).Caption = (GlobalVariables.IsArabic ? "تاريخ الاذن" : "Date");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetAcquisitionDate"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1);
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["AssetAcquisitionID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			AssetsAcquisitions.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا يوجد أذونات إقتناء لفك إعتمادها " : "There are No Acquisitions to UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmAssetsAcquisitions frmAssetsAcquisitions2 = new frmAssetsAcquisitions(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["AssetAcquisitionID"].Value.ToString()));
			frmAssetsAcquisitions2.Size = new Size(base.Width, base.Height);
			frmAssetsAcquisitions2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmAssetsAcquisitions2.lblTitle).Text = (GlobalVariables.IsArabic ? "إقتناء الأصول" : "Assets Acquisitions");
			frmAssetsAcquisitions2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.AssetsAcquisitionsReport(1, 0, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["AssetAcquisitionID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["AssetAcquisitionID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.FixedAssets.Approved.frmUnApprovingAssetsAcquisitions));
		...
		base.Name = "frmUnApprovingAssetsAcquisitions";
