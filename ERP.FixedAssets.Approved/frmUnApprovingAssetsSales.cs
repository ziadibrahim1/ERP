...
public class frmUnApprovingAssetsSales : frmPosted
{
	private IContainer components = null;

	public frmUnApprovingAssetsSales()
	{
		InitializeComponent();
		NoCol = "AssetSaleNo";
	...
	{
		dtsource = AssetsSales.SelectByApproved(GlobalVariables.BranchIDs, "1", GlobalVariables.IsArabic ? "1" : "0");
	...
	public override void InitGrid()
	{
		...
		((HeaderBase)((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetSaleNo"].Header).Caption = (GlobalVariables.IsArabic ? "رقم الاذن " : "Asset Sales No");
		((UltraGridBase)ULGData).DisplayLayout.Bands[0].Columns["AssetSaleNo"].Width = (int)((double)((Control)(object)ULGData).Width * 0.1) - GlobalVariables.ScrollWidth;
	...
	public override void SaveData()
	{
		...
		for (int i = 0; i < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; i++)
		{
			...
			{
				text = text + ((UltraGridBase)ULGData).Rows[i].Cells["AssetSaleID"].Value.ToString() + ",";
			}
		}
		if (text != ",")
		{
			AssetsSales.SetApprove("0", text);
			FillGrid();
		}
		else
		{
			GlobalVariables.InformationMB.Show(GlobalVariables.IsArabic ? "لا توجد مبيعات لفك إعتمادها " : "There are No Sales to UnApprove");
		}
	}
	...
	public override void ClickCellButton()
	{
		...
		{
			frmAssetsSales frmAssetsSales2 = new frmAssetsSales(int.Parse(((UltraGridBase)ULGData).ActiveRow.Cells["AssetSaleID"].Value.ToString()));
			frmAssetsSales2.Size = new Size(base.Width, base.Height);
			frmAssetsSales2.StartPosition = FormStartPosition.CenterParent;
			((Control)(object)frmAssetsSales2.lblTitle).Text = (GlobalVariables.IsArabic ? "مبيعات الأصول" : "Assets Sales");
			frmAssetsSales2.ShowDialog();
		}
	}

	public override void Search()
	{
		DataTable dataTable = SearchFunctions.AssetsSalesReport(1, 0, -1);
		for (int i = 0; i < dataTable.Rows.Count; i++)
		{
			for (int j = 0; j < ((DisposableObjectCollectionBase)((UltraGridBase)ULGData).Rows).Count; j++)
			{
				if (dataTable.Rows[i]["AssetSaleID"].ToString() == ((UltraGridBase)ULGData).Rows[j].Cells["AssetSaleID"].Value.ToString())
				{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.FixedAssets.Approved.frmUnApprovingAssetsSales));
		...
		base.Name = "frmUnApprovingAssetsSales";
