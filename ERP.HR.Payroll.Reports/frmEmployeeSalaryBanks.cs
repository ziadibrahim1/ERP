...
public class frmEmployeeSalaryBanks : frmReportTree2010
{
	private IContainer components = null;

	public frmEmployeeSalaryBanks()
	{
		dtItems = Banks.FillCombo(GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		TreeItemsIDCol = "BankID";
		TreeItemsNameCol = "BankName";
		dtItems2 = SubAccounts.FillReportTreeEmployeesByAdministrativeStructureIDs(GlobalVariables.EmployeeSubAccountTypeIDs, "-1", "-1", GlobalVariables.IsArabic ? "1" : "0", IsFromServer: true);
		...
		cboReportType.Items.Add((object)1, GlobalVariables.IsArabic ? "كشف مرتبات البنك" : "Salary List Bank");
	...
	public override string GetReportName()
	{
		if (((TextEditorControlBase)cboReportType).Value.ToString() == "1")
		{
			...
			{
				return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_HR_EmployeesSalaryHistoryBank_A.rpt" : "Rep_HR_EmployeesSalaryHistoryBank_E.rpt");
			}
			return GlobalVariables.ReportsPath + (((UltraToggleEditorBase)chkIsArabic).Checked ? "Rep_HR_EmployeesSalaryHistoryBank_A_nologo.rpt" : "Rep_HR_EmployeesSalaryHistoryBank_E_nologo.rpt");
		}
	...
	public override void ShowReport()
	{
		...
		if (Items.Equals(","))
		{
			GlobalVariables.InformationMB.Show("لم تقم باختيار اى بنك  ", "There is no chosen Bank to be shown in the report, please check Bank to be shown in report");
			return;
		}
		if (Items2.Equals(","))
		{
	...
	{
		dtSearchResult = SearchFunctions.BanksReport(IsFromServer: true);
		for (int i = 0; i < dtSearchResult.Rows.Count; i++)
		{
	...
	private void InitializeComponent()
	{
		...
		System.ComponentModel.ComponentResourceManager resources = new System.ComponentModel.ComponentResourceManager(typeof(ERP.HR.Payroll.Reports.frmEmployeeSalaryBanks));
		...
		base.Name = "frmEmployeeSalaryBanks";
